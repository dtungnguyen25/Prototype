...
namespace SciFiShipController
{
    ...
    public class PlayerInputModule : MonoBehaviour
    {
        ...
        /// <summary>
        /// Attempt to disable or cancel AI-assisted docking or undocking 
        /// </summary>
        public void DisableAIDocking()
        {
            if (isShipAIModeEnabled)
            {
                ...
                if (shipDocking != null)
                {
                    ...
                    // If we are currently docking or undocking then set the ship to undocked
                    if (dockingStateInt == ShipDocking.dockingInt || dockingStateInt == ShipDocking.undockingInt)
                    {
                        ...
                        if (shipDocking.GetStateInt() == ShipDocking.notDockedInt)
                        {
                            ...
                            if (aiShip != null)
                            {
                                aiShip.SetState(AIState.idleStateID);
                    ...
                    {
                        Debug.LogWarning("[WARNING] Attempting PlayerInputModule.DisableAIDocking on " + name + " in " + gameObject.scene.name + " while in docking state " + shipDocking.GetState());
                    }
                }
            }
        }
        ...
        /// Attempt to enter Ship AI-assisted mode, then attempt docking or undocking depending upon the current DockingState.
        /// </summary>
