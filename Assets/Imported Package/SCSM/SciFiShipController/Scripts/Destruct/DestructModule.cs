...
namespace SciFiShipController
{
    ...
    public class DestructModule : MonoBehaviour
    {
        ...
        /// <summary>
        /// The fragments move in the direction the ship was when the destruction occurred.
        /// </summary>
        public bool isAddShipVelocity = false;
        ...
        /// <summary>
        /// When Add Ship Velocity is enabled, the retained additional force applied to each fragment, relative to its mass, in the direction of travel.
        /// </summary>
        [Range(0.1f, 100f)] public float retainedVelocityForce = 5f;
        ...
        /// <summary>
        /// This is the total mass of all the fragments in kilograms
        /// </summary>
                ...
                if (destructFragmentList != null)
                {
                    ...
                    for (int fIdx = 0; fIdx < numFragments; fIdx++)
                    {
                        ...
                        if (destructFragment != null)
                        {
                            ...
                            if (!TryGetComponent(out rBody) && isAddRigidBodiesEnabled)
                            {
            ...
            if (disableRigidbodyMode != DisableRigidbodyMode.DontDisable)
            {
                for (int fIdx = 0; fIdx < numFragments; fIdx++)
                {
                    ...
                    if (destructFragment != null && destructFragment.isDynamic && !destructFragment.isDespawned && destructFragment.rBody != null)
                    {
                        ...
                        #if UNITY_6000_0_OR_NEWER
                        if (destructFragment.rBody.linearVelocity.sqrMagnitude < sqrUnmovingVelocity)
                        #else
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (disableRigidbodyMode != DisableRigidbodyMode.DontDisable && !isDynamic)
            {
                ...
                #if UNITY_6000_0_OR_NEWER
                if (otherRBody != null && otherRBody.linearVelocity.sqrMagnitude > sqrUnmovingVelocity)
                #else
        ...
        private void SetUpRigidbody(DestructFragment destructFragment, bool rUseGravity, float rDrag, float rAngularDrag, RigidbodyInterpolation rInterpolation, CollisionDetectionMode rCollisionDetection)
        {
            if (destructFragment != null && destructFragment.mRen != null)
            {
                ...
                #if UNITY_6000_0_OR_NEWER
                destructFragment.rBody.linearDamping = rDrag;
                destructFragment.rBody.angularDamping = rAngularDrag;
                #else
                ...
                for (int fIdx = 0; fIdx < numFragments; fIdx++)
                {
                    ...
                    if (destructFragment != null)
                    {
                        if (destructFragment.rBody != null)
                        {
                            ...
                            if (isAddShipVelocity)
                            {
                                destructFragment.rBody.AddRelativeForce(dstParms.velocity * (destructFragment.mass * retainedVelocityForce));
                            }
                        }
                    }
                }
