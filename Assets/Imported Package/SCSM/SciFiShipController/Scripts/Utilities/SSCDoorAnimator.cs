...
namespace SciFiShipController
{
    ...
    public class SSCDoorAnimator : MonoBehaviour
    {
        ...
        /// <summary>
        /// An optional array of audio sources. By default all doors share the same audio source attached
        /// to this gameobject. However, if you provide one audio source slot for each door, audio will play
        /// on the matching audio source. If the slot is empty, no audio will play for that door.
        /// </summary>
        public AudioSource[] audioSources;

        /// <summary>
        /// Verify the animator parameters on awake. False for backward compatibility.
        /// May incur GC so only use testing.
        /// </summary>
        public bool isVerifyParms = false;
        ...
        void Awake()
        {
            ...
            if (TryGetComponent(out _animator))
            {
                ...
                if (numOpenParms < 1)
                {
                    ...
                    if (isVerifyParms)
                    {
                        SSCUtils.VerifyAnimParameter(_animator, "isOpen", AnimatorControllerParameterType.Bool);
                    }
                    ...
                    {
                        Debug.LogWarning("[ERROR] SSCDoorAnimator.Awake() on " + name + " " + ex.Message);
                    }
                }
                else
                {
                    ...
                    for (int pIdx = 0; pIdx < numOpenParms; pIdx++)
                    {
                        if (isVerifyParms)
                        {
                            SSCUtils.VerifyAnimParameter(_animator, openParamNames[pIdx], useTriggers ? AnimatorControllerParameterType.Trigger : AnimatorControllerParameterType.Bool);
                        }
                    ...
                    if (useTriggers)
                    {
                        ...
                        for (int pIdx = 0; pIdx < numCloseParms; pIdx++)
                        {
                            if (isVerifyParms)
                            {
                                SSCUtils.VerifyAnimParameter(_animator, closeParamNames[pIdx], AnimatorControllerParameterType.Trigger);
                            }
        ...
        /// <summary>
        /// Attempt to play opening audio clip for a door
        /// </summary>
        /// <param name="doorIndex"></param>
        private void PlayOpeningAudioClip(int doorIndex)
        {
            if (isAudioAvailable && openingAudioClip != null)
            {
                if (numAudioSources > 0)
                {
                    AudioSource audioSource = audioSources[doorIndex];
                    if (audioSource != null)
                    {
                        audioSource.clip = openingAudioClip;
                        audioSource.volume = maxAudioVolume * openingClipVolume;
                        if (!audioSource.isActiveAndEnabled) { audioSource.enabled = true; }
        ...
        /// <summary>
        /// Attempt to play opening audio clip for all doors
        /// </summary>
        private void PlayOpeningAudioClips()
        {
            if (isAudioAvailable)
            {
                if (numAudioSources > 0)
                {
                    for (int asIdx = 0; asIdx < numAudioSources; asIdx++)
                    {
                        PlayOpeningAudioClip(asIdx);
                    }
                }
                else
                {
                    PlayOpeningAudioClip(0);
                }
            }
        }

        /// <summary>
        /// Attempt to play closing audio clip for a door
        /// </summary>
        /// <param name="doorIndex"></param>
        private void PlayClosingAudioClip(int doorIndex)
        {
            if (isAudioAvailable && closingAudioClip != null)
            {
                if (numAudioSources > 0)
                {
                    ...
                    if (audioSource != null)
                    {
                        ...
                        audioSource.volume = maxAudioVolume * closingClipVolume;
                        if (!audioSource.isActiveAndEnabled) { audioSource.enabled = true; }
        ...
        /// Attempt to play opening audio clip for all doors
        /// </summary>
        private void PlayClosingAudioClips()
        {
            if (isAudioAvailable)
            {
                if (numAudioSources > 0)
                {
                    for (int asIdx = 0; asIdx < numAudioSources; asIdx++)
                    {
                        PlayClosingAudioClip(asIdx);
                    }
                }
                else
                {
                    PlayClosingAudioClip(0);
                }
            }
        }
        ...
        /// </summary>
        /// <param name="doorIndex"></param>
        private void PlayIsLockedAudioClip(int doorIndex)
        {
            if (isAudioAvailable && isLockedAudioClip != null)
            {
                if (numAudioSources > 0)
                {
                    ...
                    if (audioSource != null)
                    {
                        audioSource.volume = maxAudioVolume * isLockedClipVolume;
                        if (!audioSource.isActiveAndEnabled) { audioSource.enabled = true; }
        ...
        /// <summary>
        /// Play the IsLocked or deny clip just once (don't loop) for all doors
        /// </summary>
        private void PlayIsLockedAudioClips()
        {
            if (isAudioAvailable)
            {
                if (numAudioSources > 0)
                {
                    for (int asIdx = 0; asIdx < numAudioSources; asIdx++)
                    {
                        PlayIsLockedAudioClip(asIdx);
                    }
                }
                else
                {
                    PlayIsLockedAudioClip(0);
                }
            }
        }
            ...
            if (_animator != null && numOpenParms > 0 && (!useTriggers || numCloseParms > 0))
            {
                if (!IsDoorLockedByIndex(0))
                {
                    ...
                    try
                    {
                        ...
                        PlayOpeningAudioClips();

                        if (onOpening != null) { onOpening.Invoke(scriptInstanceID, 0, Vector3.zero); }
                    }
                    catch (Exception ex)
                    {
                        Debug.LogWarning("[ERROR] SSCDoorAnimator.OpenDoors() on " + name + " " + ex.Message);
                    }
                }
                else
                {
                    PlayIsLockedAudioClips();
                }
            }
        }
        ...
        public void OpenDoorsInstantly (int doorIndex, int stateId)
        {
            if (_animator != null)
            {
                if (doorIndex >= 0 && doorIndex < numOpenParms && (!useTriggers || doorIndex < numCloseParms))
                {
                    ...
                    {
                        Debug.LogWarning("[ERROR] SSCDoorAnimator.OpenDoorsInstantly() on " + name + " " + ex.Message);
                    }
                }
                #if UNITY_EDITOR
                else
        ...
        public void CloseDoors()
        {
            if (_animator != null && numOpenParms > 0 && (!useTriggers || numCloseParms > 0) && !IsDoorLockedByIndex(0))
            {
                ...
                try
                {
                    ...
                    PlayClosingAudioClips();

                    if (onClosing != null) { onClosing.Invoke(scriptInstanceID, 0, Vector3.zero); }
                }
                catch (Exception ex)
                {
                    Debug.LogWarning("[ERROR] SSCDoorAnimator.CloseDoors() on " + name + " " + ex.Message);
                }
            }
        }
                ...
                if (doorIndex >= 0 && doorIndex < numOpenParms && (!useTriggers || doorIndex < numCloseParms))
                {
                    try
                    {
                        if (!IsDoorLockedByIndex(doorIndex))
                        {
                            ...
                            PlayClosingAudioClip(doorIndex);
        ...
        /// <summary>
        /// Close a specific door or set of doors given the zero-based index in the array of open param names.
        /// Close them instantly rather than playing the animation. Do not play any audio. Ignore lock status.
        /// Do not invoke any closing events.
        /// To get the the stateId call SSCUtils.GetAnimationStateID(stateName). Cache the stateId to avoid GC.
        /// e.g., int stateId = SSCUtils.GetAnimationStateID("RearRamp_Raise");
        /// </summary>
        /// <param name="doorIndex"></param>
        /// <param name="stateId"></param>
        public void CloseDoorsInstantly (int doorIndex, int stateId)
        {
            if (_animator != null)
            {
                if (doorIndex >= 0 && doorIndex < numOpenParms && (!useTriggers || doorIndex < numCloseParms))
                {
                    ...
                    {
                        Debug.LogWarning("[ERROR] SSCDoorAnimator.CloseDoorsInstantly() on " + name + " " + ex.Message);
                    }
                }
                #if UNITY_EDITOR
                else
                {
                    Debug.LogWarning("[ERROR] SSCDoorAnimator.CloseDoorsInstantly() - the doorIndex of " + doorIndex + " is invalid. Check the number of openParamNames" + (useTriggers ? " or closeParamNames." : "."));
                }
                #endif
            }
        }
        ...
        public void ResetAudioSettings()
        {
            ...
            numAudioSources = IsInitialised ? numAudioSources : audioSources == null ? 0 : audioSources.Length;

            // If there are audio sources, make sure they match the number of doors.
            if (numAudioSources > 0 && numAudioSources != numDoors)
            {
                System.Array.Resize(ref audioSources, numDoors);
            ...
            else if (numAudioSources == 0 && (openingAudioClip != null || closingAudioClip != null || isLockedAudioClip != null))
            {
                #if UNITY_EDITOR
            ...
            {
                // Set the max audio to the first non-null audio source
                for (int asIdx = 0; asIdx < numAudioSources; asIdx++)
                {
                    AudioSource _audioSource = audioSources[asIdx];
                    if (_audioSource != null)
                    {
                        maxAudioVolume = _audioSource.volume;
                        isAudioAvailable = true;
                        break;
                    }
                }
