...
namespace SciFiShipController
{
    ...
    public class SSCDoorControl : MonoBehaviour
    {
        ...
        /// If the first door is opened, or is opening, highlight the Open button.
        /// If the first door is closed, or is closing, highlight the Close button.
        /// </summary>
        public void UpdateStatus()
        {
            if (isInitialised)
            {
                if (GetLockedStatus(doorIndexes[0]))
                {
                    ShowLocked();
                }
                else
                {
                    if (GetIsOpen()) { SwitchMaterial(openMaterial); }
                    else { SwitchMaterial(closeMaterial); }
                } 
            }
        }
