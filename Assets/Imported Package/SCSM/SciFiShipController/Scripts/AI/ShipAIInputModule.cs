    ...
    public class ShipAIInputModule : MonoBehaviour
    {
        ...
        void UpdateLoop (float deltaTime)
        {
            ...
            if (isInitialised && isAIEnabled && shipControlModule.ShipIsEnabled())
            {
                ...
                shipInput.yaw = yawPIDController.RequiredInput(targetYaw, currentYaw, deltaTime);
                ...
                else
                {
                    ...
                    shipInput.pitch = pitchPIDController.RequiredInput(targetPitch, currentPitch, deltaTime);
                    shipInput.roll = rollPIDController.RequiredInput(targetRoll, currentRoll, deltaTime);
                }
                ...
                shipInput.horizontal = horizontalPIDController.RequiredInput(desiredLocalVelocity.x, currentLocalVelocity.x, deltaTime);
                shipInput.vertical = verticalPIDController.RequiredInput(desiredLocalVelocity.y, currentLocalVelocity.y, deltaTime);
                shipInput.longitudinal = longitudinalPIDController.RequiredInput(desiredLocalVelocity.z, currentLocalVelocity.z, deltaTime);
                    ...
                    for (int wpIdx = 0; wpIdx < numWeapons; wpIdx++)
                    {
                        ...
                        if (weapon != null)
                        {
                            ...
                            if (((weapon.weaponType == Weapon.WeaponType.TurretProjectile && weapon.turretPivotY != null && weapon.turretPivotX != null)
                                || isGuidedToTarget) && weapon.projectilePrefab != null && !weapon.isAutoTargetingEnabled)
                            {
        ...
        public void Initialise()
        {
            ...
            timingUpdateInt = shipControlModule.shipInstance.timingUpdateLoopInt;
        ...
        public void TelePort(Vector3 delta, bool resetVelocity)
        {
            ...
            Vector3 _origVelo = isInitialised ? shipControlModule.shipInstance.WorldVelocity : Vector3.zero;
            Vector3 _origAVelo = isInitialised ? shipControlModule.shipInstance.WorldAngularVelocity : Vector3.zero;
            ...
            if (!resetVelocity && isInitialised)
            {
                Rigidbody rBody = shipControlModule.ShipRigidbody;
                ...
                shipControlModule.shipInstance.UpdatePositionAndMovementData(transform, rBody);
            }
        }
