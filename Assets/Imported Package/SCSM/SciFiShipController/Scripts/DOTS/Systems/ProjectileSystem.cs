...
namespace SciFiShipController
{
    ...
    /// U2022.3 - Entities 1.2.3 (SSC 1.4.7+)
    /// </summary>
    [DisableAutoCreation]
    ...
    #endif
    {
        ...
        // ECS 1.0
        //private Unity.Physics.PhysicsWorld physicsWorld;
        #else
        ...
        #region Projectile TelePort Job
        ...
        partial struct ProjectilePhysicsRayJob : IJobEntity
        #else
        ...
        #endif
        {
            ...
            #if UNITY_2022_3_OR_NEWER
            public void Execute(in LocalTransform position, ref Projectile projectile)
            #else
            ...
            #endif
            {
                Unity.Physics.RaycastInput raycastInput = new Unity.Physics.RaycastInput
                {
                    ...
                    End = position.Position + (projectile.velocity * deltaTime),
                    #else
        ...
        protected override void OnCreate()
        {
            ...
            // SystemAPI cannot be called outside a system.
        ...
        #endif
        {
            ...
            #if UNITY_2022_2_OR_NEWER
            // ECS 1.0
            /// TODO - NOT SURE HOW WE DO THIS IN ECS 1.0+
            //SystemAPI.GetSingleton<Unity.Physics.PhysicsWorldSingleton>()
            //this.Dependency = JobHandle.CombineDependencies(this.Dependency, physicsWorld.GetOutputDependency());
            #elif UNITY_2020_1_OR_NEWER
            ...
            for (int chunkIndex = 0; chunkIndex < chunksLength; chunkIndex++)
            {
                ...
                for (int entityIndex = 0; entityIndex < chunkSize; entityIndex++)
                {
                    ...
                    raycastCommand.direction = SSCMath.Normalise(velo);
            ...
            if (physicsraycastResults.IsCreated)
            {
                ...
                // SystemAPI cannot be called outside a system.
                Unity.Physics.CollisionWorld collisionWorld = SystemAPI.GetSingleton<Unity.Physics.PhysicsWorldSingleton>().PhysicsWorld.CollisionWorld;

                var projectilePhysicsRayJob = new ProjectilePhysicsRayJob()
                {
                    collisionWorldInJob = collisionWorld,
                    raycastResultsInJob = physicsraycastResults,
                    deltaTime = deltaTime
                };
                /// TODO - NOT SURE IF THIS IS CORRECT
                ///
                // ERROR is raised if ScheduleParallel is used.
                // System.IndexOutOfRangeException: ReadWriteBuffers are restricted to only read & write the element at the job index.
                ...
                //projectilePhysicsRayJob.ScheduleParallel(projectileEntityQuery, this.Dependency).Complete();

                projectilePhysicsRayJob.Schedule(projectileEntityQuery, this.Dependency).Complete();
                #elif UNITY_2020_1_OR_NEWER
                ...
                #if UNITY_2022_3_OR_NEWER
                numPhysicsBodies = collisionWorld.NumBodies;
                //Debug.Log("[DEBUG] NumBodies: " + numPhysicsBodies + " physics ray hits: " + numPhysicsRaycastHits);
                #else
            ...
            for (int chunkIndex = 0; chunkIndex < chunksLength; chunkIndex++)
            {
                ...
                for (int entityIndex = 0; entityIndex < chunkSize; entityIndex++)
                {
                    ...
                    if (other != null)
                    {
                        ...
                        if (projectile.shieldEffectsObjectPrefabID >= 0 && ProjectileModule.CheckShipHit(hitInfo, projectile.damageAmount, (ProjectileModule.DamageType)projectile.damageTypeInt, projectile.sourceShipId, projectile.sourceSquadronId, projectile.projectilePrefabID, projectile.shieldPenetration, out shipControlModule))
                        {
                        ...
                        else if (projectile.shieldEffectsObjectPrefabID < 0 && ProjectileModule.CheckShipHit(hitInfo, projectile.damageAmount, (ProjectileModule.DamageType)projectile.damageTypeInt, projectile.sourceShipId, projectile.sourceSquadronId, projectile.projectilePrefabID, projectile.shieldPenetration))
                        {
        ...
        internal void TelePortProjectiles(Vector3 deltaPosition)
        {
            ...
            // For some reason, we need to make this this job has completed so avoid it interfering with the
            // other projectile jobs.
