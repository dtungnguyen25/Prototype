...
namespace SciFiShipController
{
    ...
    public class SSCUtils
    {
        ...
        /// <returns></returns>
        public static int VerifyAnimParameter (Animator animator, string parameterName, AnimatorControllerParameterType parameterType)
        {
            int hashCode = 0;

            if (animator.parameterCount > 0)
            {
                foreach(AnimatorControllerParameter parm in animator.parameters)
                {
                    if (parm.name == parameterName)
                    {
                        if (parm.type == AnimatorControllerParameterType.Bool && parameterType == AnimatorControllerParameterType.Bool ||
                            parm.type == AnimatorControllerParameterType.Float && parameterType == AnimatorControllerParameterType.Float ||
                            parm.type == AnimatorControllerParameterType.Int && parameterType == AnimatorControllerParameterType.Int ||
                            parm.type == AnimatorControllerParameterType.Trigger && parameterType == AnimatorControllerParameterType.Trigger
                        )
                        {
                ...
                if (hashCode == 0)
                {
                    Debug.LogWarning("SSCUtils.VerifyAnimParameter " + parameterName + " does not exist in animator controller.");
                }
                #endif
            }
        ...
        /// <summary>
        /// Pre-calculate steps 1 and 2 of the (ship) drag calculations.
        /// </summary>
        /// <param name="physDragData"></param>
        public static void CalculateDragMetrics (ref PhysDragData physDragData)
        {
            ...
            // NOTE: dragReferenceAreas needs to be pre-calculated. For a ship this is done in the Aero tab with "Calculate Drag Properties".

            physDragData.dragLength = (float)Math.Sqrt(physDragData.dragReferenceAreas.x * physDragData.dragReferenceAreas.y / physDragData.dragReferenceAreas.z);
            physDragData.dragWidth = physDragData.dragReferenceAreas.y / physDragData.dragLength;
            physDragData.dragHeight = physDragData.dragReferenceAreas.z / physDragData.dragWidth;

            // Step 2: Calculate the angular drag moment on each axis
            // NOTE: centreOfDragXMoment, centreOfDragYMoment, centreOfDragZMoment need to be pre-calculated.
            // For a ship this is done in the Aero tab with "Calculate Drag Properties".

            // TODO: Re-comment this section
            physDragData.quarticLengthXProj = SSCMath.IntPowS0((physDragData.dragLength / 2) + physDragData.centreOfMass.z - physDragData.centreOfDragYMoment.z, 4) + SSCMath.IntPowS0((physDragData.dragLength / 2) - physDragData.centreOfMass.z + physDragData.centreOfDragYMoment.z, 4);
            physDragData.quarticLengthYProj = SSCMath.IntPowS0((physDragData.dragLength / 2) + physDragData.centreOfMass.z - physDragData.centreOfDragXMoment.z, 4) + SSCMath.IntPowS0((physDragData.dragLength / 2) - physDragData.centreOfMass.z + physDragData.centreOfDragXMoment.z, 4);
            physDragData.quarticWidthYProj = SSCMath.IntPowS0((physDragData.dragWidth / 2) + physDragData.centreOfMass.x - physDragData.centreOfDragZMoment.x, 4) + SSCMath.IntPowS0((physDragData.dragWidth / 2) - physDragData.centreOfMass.x + physDragData.centreOfDragZMoment.x, 4);
            physDragData.quarticWidthZProj = SSCMath.IntPowS0((physDragData.dragWidth / 2) + physDragData.centreOfMass.x - physDragData.centreOfDragYMoment.x, 4) + SSCMath.IntPowS0((physDragData.dragWidth / 2) - physDragData.centreOfMass.x + physDragData.centreOfDragYMoment.x, 4);
            physDragData.quarticHeightXProj = SSCMath.IntPowS0((physDragData.dragHeight / 2) + physDragData.centreOfMass.y - physDragData.centreOfDragZMoment.y, 4) + SSCMath.IntPowS0((physDragData.dragHeight / 2) - physDragData.centreOfMass.y + physDragData.centreOfDragZMoment.y, 4);
            physDragData.quarticHeightZProj = SSCMath.IntPowS0((physDragData.dragHeight / 2) + physDragData.centreOfMass.y - physDragData.centreOfDragXMoment.y, 4) + SSCMath.IntPowS0((physDragData.dragHeight / 2) - physDragData.centreOfMass.y + physDragData.centreOfDragXMoment.y, 4);
            
            if (physDragData.isPhysicsBasedModel) { physDragData.angularDragMultiplier = 1f; }
            else { physDragData.angularDragMultiplier = (float)Math.Pow(10f, physDragData.angularDragFactor - 1f); }

            physDragData.isCached = true;
        }
        ...
        /// <summary>
        /// Return true if we hit a non-trigger collider along with the RaycastHit.
        /// </summary>
        /// <param name="ray"></param>
        /// <param name="direction"></param>
        /// <param name="lookDistance"></param>
        ...
        /// <returns></returns>
        public static bool GetClosestCollider(Ray ray, Vector3 direction, float lookDistance, int layerMask, out RaycastHit raycastHit)
        {
            ray.direction = direction;
            if (Physics.Raycast(ray, out raycastHit, lookDistance, layerMask, QueryTriggerInteraction.Ignore))
            {
                return true;
            }
            else { return false; }
        ...
        /// <summary>
        /// Attempt to add an event system
        /// NOTE: Currently only works in the editor
        /// Seems to always move focus to object in scene even if it exists, so look for one first
        /// </summary>
        public static void AddEventSystem()
        {
            ...
            List<UnityEngine.EventSystems.EventSystem> evSysList = new List<UnityEngine.EventSystems.EventSystem>(UnityEngine.Object.FindObjectsByType<UnityEngine.EventSystems.EventSystem>(FindObjectsSortMode.None));
            #else
            List<UnityEngine.EventSystems.EventSystem> evSysList = new List<UnityEngine.EventSystems.EventSystem>(UnityEngine.Object.FindObjectsOfType<UnityEngine.EventSystems.EventSystem>());
            #endif

            if (evSysList.Count == 0)
            {
        ...
        /// <param name="sceneHandle">Optional handle of scene</param>
        /// <returns></returns>
        public static Canvas FindCanvas (string canvasName, int sceneHandle = 0)
        {
            Canvas canvas = null;
            ...
            if (!string.IsNullOrEmpty(canvasName))
            {
                ...
                if (canvasList != null) { canvas = canvasList.Find(cv => cv.name == canvasName && (sceneHandle == 0 || sceneHandle == cv.gameObject.scene.handle)); }
            }
        ...
        /// <summary>
        /// Attempt either get or create a CanvasScaler.
        /// Usage: CanvasScaler scaler = SSCUtils.GetOrCreateCanvasScaler(canvas, new Vector2(1920f, 1080f));
        /// </summary>
        /// <param name="canvas"></param>
        /// <param name="sizeInPixels"></param>
        /// <returns></returns>
        public static CanvasScaler GetOrCreateCanvasScaler (Canvas canvas, Vector2 sizeInPixels)
        {
            CanvasScaler scaler = null;

            if (canvas != null)
            {
                if (!canvas.TryGetComponent(out scaler))
                {
                    scaler = canvas.gameObject.AddComponent<CanvasScaler>();
                    scaler.uiScaleMode = CanvasScaler.ScaleMode.ScaleWithScreenSize;
                    scaler.referenceResolution = sizeInPixels;
            ...
            if (parentRectTrfm != null && !string.IsNullOrEmpty(childName))
            {
                ...
                if (childTrfm == null)
                {
                    ...
                    if (_tempGO != null)
                    {
                        ...
                        #if UNITY_EDITOR
                        UnityEditor.Undo.RegisterCreatedObjectUndo(_tempGO, string.Empty);
                        #endif
                    }
                }
                else
        ...
        /// <summary>
        /// Check if the transform scale is uniform (or non-uniform).
        /// Example: if (SSCUtils.IsUniformScale(transform.localScale) { }
        /// </summary>
        /// <param name="scale"></param>
        /// <returns></returns>
        public static bool IsUniformScale(Vector3 scale)
        {
        ...
        #region Material and Shader Methods

        /// <summary>
        /// Attempt to get the propertyID of the shader attached to the material.
        /// Typically, the propertyName is the DisplayName preceded by an underscore.
        /// E.g. Colour1 would be _Colour1.
        /// </summary>
        /// <param name="material"></param>
        /// <param name="propertyName"></param>
        /// <param name="propertyID"></param>
        /// <returns></returns>
        public static bool GetShaderPropertyId(Material material, string propertyName, out int propertyID)
        {
            propertyID = 0;
        ...
        /// <summary>
        /// Create a simple capsule at a world space position
        /// </summary>
        /// <param name="position"></param>
        /// <param name="scale"></param>
        /// <param name="includeCollider"></param>
        /// <returns></returns>
        public static GameObject CreateCapsule(Vector3 position, Vector3 scale, bool includeCollider)
        {
            ...
            if (capsuleGO != null)
            {
                if (!includeCollider)
                {
                    CapsuleCollider goCollider;
                    if (capsuleGO.TryGetComponent(out goCollider))
                    {
                        #if UNITY_EDITOR
                ...
                capsuleGO.transform.localScale = scale.x > Mathf.Epsilon && scale.y > Mathf.Epsilon && scale.z > Mathf.Epsilon ? scale : Vector3.one;
            }
            return capsuleGO;
        }
        ...
        /// Use optional paramTypes if method is overloaded and you get AmbiguousMatchException: Ambiguous match found.
        /// </summary>
        /// <param name="t"></param>
        /// <param name="methodName"></param>
        /// <param name="includePrivate"></param>
        /// <param name="includeStatic"></param>
        /// <param name="paramTypes">An optional array of the parameters used if the method is overloaded</param>
        /// <returns></returns>
        public static System.Reflection.MethodInfo ReflectionGetMethod(Type t, string methodName, bool includePrivate, bool includeStatic, Type[] paramTypes = null)
        {
            ...
            if (paramTypes != null)
            {
                return t.GetMethod(methodName, bindingFlags, null, paramTypes, null);
            }
            else
            {
        ...
        /// <summary>
        /// Sort a list of float scores in ascending order.
        /// For performance reasons, assumes list is not null.
        /// </summary>
        /// <param name="sortList"></param>
        public static void SortAsc(List<SSCSortItemFloat> sortList)
        {
            sortList.Sort(delegate (SSCSortItemFloat item1, SSCSortItemFloat item2)
            {
                ...
                else if (item1.score < item2.score) { return -1; }
        ...
        /// <summary>
        /// Sort a list of float scores in descending order.
        /// For performance reasons, assumes list is not null.
        /// </summary>
        /// <param name="sortList"></param>
        public static void SortDesc(List<SSCSortItemFloat> sortList)
        {
            sortList.Sort(delegate (SSCSortItemFloat item1, SSCSortItemFloat item2)
            {
                // If x < y return 1, if x > y return -1,  if x == y return 0 (this is equivalent to x.CompareTo(y))
    ...
    #region Drag Structures

    /// <summary>
    /// Inputs and outputs to cache drag metrics
    /// </summary>
    public struct PhysDragData
    {
        public bool isCached;
        // Inputs
        public bool isPhysicsBasedModel;
        ...
        public float quarticLengthXProj;
        public float quarticLengthYProj;
        public float quarticWidthYProj;
        public float quarticWidthZProj;
        public float quarticHeightXProj;
        public float quarticHeightZProj;
    ...
    #region Sort Items Structures

    /// <summary>
    /// A structure typically used in a sorted list. The index can be used to reference another
    /// list of class or struct x which may not have a sortable field.
    /// See SSCUtils.SortAsc(List<SSCSortItemFloat> sortList), SortDesc(..).
    /// </summary>
    public struct SSCSortItemFloat
    {
        public int index;
        public float score;
    }
