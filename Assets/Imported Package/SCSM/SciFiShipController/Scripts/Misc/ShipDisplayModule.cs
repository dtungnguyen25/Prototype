...
namespace SciFiShipController
{
    ...
    public class ShipDisplayModule : MonoBehaviour
    {
        ...
        public readonly static string cinematicPanelName = "CinematicPanel";
        public readonly static string cinematicTopPanelName = "CinematicTopPanel";
        public readonly static string cinematicBottomPanelName = "CinematicBottomPanel";
        ...
        /// <summary>
        /// Should the cursor get hidden when the HUD is initialised?
        /// </summary>
        public bool isHideCursorOnInit = false;
        ...
        /// </summary>
        public bool IsInitialised { get => isInitialised; private set => isInitialised = value; }
        ...
        #region Public Variables and Properties - Cinematic

        /// <summary>
        /// Damp the movement of the cinematic effect to make it progress in a non-linear manner.
        /// </summary>
        [Range(0f, 0.5f)] public float cinematicDamping = 0.1f;

        /// <summary>
        /// The length of time in seconds, it takes to fully enable the cinematic mask
        /// </summary>
        [Range(0f, 20f)] public float cinematicInDuration = 2f;

        /// <summary>
        /// The length of time in seconds, it takes to fully disable the cinematic mask
        /// </summary>
        [Range(0f, 20f)] public float cinematicOutDuration = 2f;

        /// <summary>
        /// Smoothly enable the cinematic mask when display first starts
        /// </summary>
        public bool cinematicInOnStart = false;

        /// <summary>
        /// Get or set the progress of the cinematic effect (0.0 to 1.0).
        /// </summary>
        public float CinematicProgress { get { return currentCinematicValue; } set { SetCinematicProgress(value); } }

        /// <summary>
        /// Is the cinematic effect currently being disabled?
        /// </summary>
        public bool IsDisplayCinematicDisabling { get { return isCinematicDisabling; } }

        /// <summary>
        /// Is the cinematic effect currently being enabled?
        /// </summary>
        public bool IsDisplayCinematicEnabling { get { return isCinematicEnabling; } }

        /// <summary>
        /// Is the cinematic effect on or visible?
        /// </summary>
        public bool IsDisplayCinematicOn { get { return isInitialised && cinematicTopImage != null && cinematicTopImage.enabled && currentCinematicValue > 0f; } }

        /// <summary>
        /// Smoothly disable the cinematic mask when display first starts
        /// </summary>
        public bool cinematicOutOnStart = false;
        ...
        /// NOTE: This does not currently work with a Worldspace canvas
        /// </summary>
        ...
        /// e.g., altitude, speed, heading, attitude
        /// </summary>
        ...
        /// <summary>
        /// Show the Altitude metric (m) on the HUD if the Altitude indicator is show.
        /// At runtime use ShowAltitudeMetric() or HideAltitudeMetric(). See also ShowAltitude().
        /// </summary>
        public bool showAltitudeMetric = false;
        ...
        /// <summary>
        /// Show the air speed metric (kmh) on the HUD if the air speed is show.
        /// At runtime use ShowAirSpeedMetric() or HideAirSpeedMetric(). See also ShowAirSpeed().
        /// </summary>
        public bool showAirspeedMetric = false;

        /// <summary>
        /// Show Air Speed in knots
        /// </summary>
        public bool airspeedAsKnots = false;

        /// <summary>
        /// Add or subtract from the actual airspeed. This can be useful to give the impression
        /// the ship is travelling very quickly through space or during a warp when ship is stationary.
        /// </summary>
        public float airspeedOffset = 0f;
        ...
        /// <summary>
        /// Get or set the custom gauge purposes scriptableobject
        /// </summary>
        public SSCGaugePurposes GaugePurposes { get => gaugePurposes; set => gaugePurposes = value; }
        ...
        /// [INTERNAL ONLY]
        /// </summary>
        public bool showCinematicSettingsInEditor = false;
        ...
        #region Private Variables - Cinematic

        /// <summary>
        /// Initial delay when starting the cinematic start or stop process.
        /// </summary>
        [SerializeField, Range(0.01f, 5f)] private float cinematicDelay = 0.4f;

        private bool isCinematicEnabling = false;
        private bool isCinematicDisabling = false;
        [System.NonSerialized] private Image cinematicTopImage = null;
        [System.NonSerialized] private Image cinematicBottomImage = null;
        [System.NonSerialized] WaitForSeconds cinematicWait = null;
        [System.NonSerialized] private Coroutine cinematicStartRoutine = null;
        [System.NonSerialized] private Coroutine cinematicStopRoutine = null;

        /// <summary>
        /// Value between 0.0 (disabled) to 1.0 (enabled)
        /// </summary>
        private float currentCinematicValue = 0f;
        ...
        private Vector3 headingInitPositionLS = Vector3.zero;
        ...
        /// <summary>
        /// The optional ScriptableObject containing gauge purposes.
        /// </summary>
        [SerializeField] SSCGaugePurposes gaugePurposes = null; 
        ...
        void Update()
        {
            ...
            #region Flickering
            if (isInitialised && (isFlickeringEndStateOn || isFlickeringEndStateOff))
            {
            ...
            if ((showAltitude || showAirspeed || isHeadingScrolling || isAttitudeScrolling) && IsHUDShown && sourceShip != null && sourceShip.ShipIsEnabled() && sourceShip.IsInitialised)
            {
                if (showAltitude)
                {
                    ...
                    if (showAltitudeMetric)
                    {
                        altitudeString.AddSpace();
                        altitudeString.Add("m");
                    }
                ...
                {
                    // Show in km/h
                    int airSpeedInt = (int)(sourceShip.shipInstance.LocalVelocity.z * 3.6f * (airspeedAsKnots ? 0.5399568f : 1f) + airspeedOffset);

                    // If airspeed is -1 round up to 0.
                    if (airSpeedInt == -1) { airSpeedInt = 0; }

                    airspeedString.Set(airSpeedInt);
                    //airspeedString.Set((int)(sourceShip.shipInstance.LocalVelocity.z*3.6f * (airspeedAsKnots ? 0.5399568f : 1f)));
                    if (showAirspeedMetric)
                    {
                        airspeedString.AddSpace();
                        airspeedString.Add(airspeedAsKnots ? "kn" : "km/h");
                    }
                ...
                if (isHeadingScrolling)
                {
                    ...
                    // Original (works in screen overlay mode only)
                    ...
                    // v1.4.6 convert to LS so can work in render mode overlay or worldspace 
                    headingScrollImg.transform.localPosition = headingInitPositionLS + new Vector3(Vector3.Angle(new Vector3(_shipFwd.x, 0f, _shipFwd.z), Vector3.forward) * Mathf.Sign(_headingDir) * headingPixelsPerDegree, 0f, 0f);
                }
        ...
        #region Private or Internal Methods - Cinematic

        /// <summary>
        /// Attempt to smoothly disable the cinematic mask
        /// </summary>
        /// <returns></returns>
        private IEnumerator DisableCinematic()
        {
            isCinematicEnabling = false;
            isCinematicDisabling = true;

            if (cinematicOutDuration == 0) { cinematicOutDuration = 0.1f; }

            // Reduce the duration if it is partly enabled already
            float currentTimeOutValue = currentCinematicValue * cinematicOutDuration;

            float height = GetCinematicEdgePanelHeight();

            // Set initial values
            Vector2 topOffsetMin = new Vector2(0f, Mathf.Lerp(1080f - height, 1080f, 1f- currentCinematicValue));
            Vector2 topOffsetMax = new Vector2(0f, Mathf.Lerp(height, 0f, currentCinematicValue));

            Vector2 btmOffsetMin = new Vector2(0f, Mathf.Lerp(-height, 0f, currentCinematicValue));
            Vector2 btmOffsetMax = new Vector2(0f, Mathf.Lerp(-1080f + height, -1080f, 1f- currentCinematicValue));

            RectTransform top = cinematicTopImage.rectTransform;
            RectTransform btm = cinematicBottomImage.rectTransform;

            top.offsetMin = topOffsetMin;
            top.offsetMax = topOffsetMax;

            btm.offsetMin = btmOffsetMin;
            btm.offsetMax = btmOffsetMax;

            cinematicTopImage.enabled = true;
            cinematicBottomImage.enabled = true;

            // Inital delay
            yield return cinematicWait;

            while (currentCinematicValue > 0f)
            {
                // Use Unscaled time so not to be affected by Time.timeScale
                currentTimeOutValue -= Time.unscaledDeltaTime;

                currentCinematicValue = SSCMath.DampValue(currentCinematicValue, currentTimeOutValue / cinematicOutDuration, Time.unscaledDeltaTime, cinematicDamping);

                if (currentCinematicValue < 0f) { currentCinematicValue = 0f; }

                //Debug.Log("[DEBUG] cinematic OUT t: " + currentTimeOutValue + " cValue: " + currentCinematicValue);

                topOffsetMin.y = Mathf.Lerp(1080f - height, 1080f, 1f- currentCinematicValue);
                topOffsetMax.y = Mathf.Lerp(height, 0f, currentCinematicValue);
                top.offsetMin = topOffsetMin;
                top.offsetMax = topOffsetMax;

                btmOffsetMin.y = Mathf.Lerp(-height, 0f, currentCinematicValue);
                btmOffsetMax.y = Mathf.Lerp(-1080f + height, -1080f, 1f- currentCinematicValue);
                btm.offsetMin = btmOffsetMin;
                btm.offsetMax = btmOffsetMax;
            ...
            // turn off when finish transitioning
            cinematicTopImage.enabled = false;
            cinematicBottomImage.enabled = false;

            isCinematicDisabling = false;
        }

        /// <summary>
        /// Get the height in pixels of the cinematic top and bottom edge panels.
        /// </summary>
        /// <returns></returns>
        private float GetCinematicEdgePanelHeight()
        {
            return (1080f - 815f) * 0.5f;
        }

        /// <summary>
        /// Get the normalised (0.0-1.0) height of the cinematic top and bottom edge panels.
        /// </summary>
        /// <returns></returns>
        private float GetCinematicEdgePanelHeightN()
        {
            return (1080f - 815f) * 0.5f / 1080f;
        }

        /// <summary>
        /// Attempt to smoothly enable the cinematic mask
        /// </summary>
        /// <returns></returns>
        private IEnumerator EnableCinematic()
        {
            isCinematicEnabling = true;
            isCinematicDisabling = false;

            if (cinematicInDuration == 0) { cinematicInDuration = 0.1f; }

            // Reduce the duration if it is partly enabled already
            float currentTimeInValue = cinematicInDuration - (currentCinematicValue * cinematicInDuration); ;

            float height = GetCinematicEdgePanelHeight();

            // Set initial values
            Vector2 topOffsetMin = new Vector2(0f, Mathf.Lerp(1080f, 1080f - height, currentCinematicValue));
            Vector2 topOffsetMax = new Vector2(0f, Mathf.Lerp(0f, height, currentCinematicValue));

            Vector2 btmOffsetMin = new Vector2(0f, Mathf.Lerp(-height, 0f, currentCinematicValue));
            Vector2 btmOffsetMax = new Vector2(0f, Mathf.Lerp(-1080f, -1080f + height, currentCinematicValue));
            ...
            while (currentCinematicValue < 1f)
            {
                // Use Unscaled time so not to be affected by Time.timeScale
                currentTimeInValue -= Time.unscaledDeltaTime;

                currentCinematicValue = SSCMath.DampValue(currentCinematicValue, 1f - (currentTimeInValue / cinematicInDuration), Time.unscaledDeltaTime, cinematicDamping);

                if (currentCinematicValue > 1f) { currentCinematicValue = 1f; }

                //Debug.Log("[DEBUG] cinematic t: " + currentTimeInValue + " cValue: " + currentCinematicValue);

                topOffsetMin.y = Mathf.Lerp(1080f, 1080f - height, currentCinematicValue);
                topOffsetMax.y = Mathf.Lerp(0f, height, currentCinematicValue);
                top.offsetMin = topOffsetMin;
                top.offsetMax = topOffsetMax;

                btmOffsetMin.y = Mathf.Lerp(-height, 0f, currentCinematicValue);
                btmOffsetMax.y = Mathf.Lerp(-1080f, -1080f + height, currentCinematicValue);
                btm.offsetMin = btmOffsetMin;
                btm.offsetMax = btmOffsetMax;
        ...
        /// <summary>
        /// Get the cinematic bottom image. Create it if it does not already exist.
        /// </summary>
        /// <returns></returns>
        private Image GetCinematicImageBottom()
        {
            if (cinematicBottomImage == null)
            {
                RectTransform cinematicPanel = GetCinematicPanel();

                if (cinematicPanel != null)
                {
                    // Stretched panel
                    float heightN = GetCinematicEdgePanelHeightN();
                    // The pivot point is the middle. Offset Y is (-1 [bottom] + height) / 2.
                    RectTransform bottomPanel = SSCUtils.GetOrCreateChildRectTransform(cinematicPanel, cvsResolutionFull, cinematicBottomPanelName, 0f, (-1 + heightN)*0.5f, 1f, heightN, 0f, 0f, 1f, 1f);

                    // Add an Image component if one doesn't already exist
                    if (bottomPanel != null && !bottomPanel.TryGetComponent<Image>(out cinematicBottomImage))
                    {
                        cinematicBottomImage = bottomPanel.gameObject.AddComponent<Image>();
                        cinematicBottomImage.raycastTarget = false;
                        cinematicBottomImage.color = Color.black;
                    }
                }
            }
            return cinematicBottomImage;
        }

        /// <summary>
        /// Get the cinematic top image. Create it if it does not already exist.
        /// </summary>
        /// <returns></returns>
        private Image GetCinematicImageTop()
        {
            if (cinematicTopImage == null)
            {
                RectTransform cinematicPanel = GetCinematicPanel();
                ...
                if (cinematicPanel != null)
                {
                    // Stretched panel
                    float heightN = GetCinematicEdgePanelHeightN();
                    // The pivot point is the middle. Offset Y is (+1 [top] - height) / 2.
                    RectTransform topPanel = SSCUtils.GetOrCreateChildRectTransform(cinematicPanel, cvsResolutionFull, cinematicTopPanelName, 0f, (1f-heightN)*0.5f, 1f, heightN, 0f, 0f, 1f, 1f);

                    // Add an Image component if one doesn't already exist
                    if (topPanel != null && !topPanel.TryGetComponent<Image>(out cinematicTopImage))
                    {
                        cinematicTopImage = topPanel.gameObject.AddComponent<Image>();
                        cinematicTopImage.raycastTarget = false;
                        cinematicTopImage.color = Color.black;
                    }
                }
            }
            return cinematicTopImage;
        }

        private void StopCinematicCoroutines()
        {
            if (cinematicStartRoutine != null)
            {
                StopCoroutine(cinematicStartRoutine);
            }

            if (cinematicStopRoutine != null)
            {
                StopCoroutine(cinematicStopRoutine);
            }
        }
        ...
        /// <summary>
        /// Get the fade image. Create it if it does not already exist.
        /// </summary>
        /// <returns></returns>
        private Image GetFadeImage()
        {
            if (fadeImage == null)
            {
                ...
                // Add an Image component if one doesn't already exist
                if (fadePanel != null && !fadePanel.TryGetComponent<Image>(out fadeImage))
                {
        ...
        private void InitialiseHeading(bool isInitialising)
        {
            ...
            {
                // headingScrollImg is a component on the headingScrollPanel.
                // The transform on each are the same transform.
                headingInitPositionLS = headingScrollImg.transform.localPosition;
        ...
        private void UpdateTargetPositions()
        {
            if (isInitialised)
            {
                if (sscRadar == null) { sscRadar = SSCRadar.GetOrCreateRadar(gameObject.scene.handle); }
        ...
        public void Initialise()
        {
            ...
            if (canvas != null && hudRectTfrm != null)
            {
                ...
                if (reticlePanel != null && overlayPanel != null && altitudeTextRectTfrm != null && airspeedTextRectTfrm != null)
                {
                    ...
                    altitudeString = new SCSMString(8);
                    ...
                    airspeedString = new SCSMString(11);
                    ...
                    #if UNITY_EDITOR
                    if (!isMainCameraAssigned) { Debug.LogWarning("ShipDisplayModule.Initialise() - could not find a camera with the MainCamera tag set"); }
                    #endif
                    ...
                    if (isHideCursorOnInit) { ShowOrHideCursor(false); }

                    if (isShowHUDWithFlicker && isShowOnInitialise) { FlickerOn(flickerDefaultDuration); }
                    else  { ShowOrHideHUD(isShowOnInitialise); }

                    SetCinematicDelay(cinematicDelay);

                    if (cinematicInOnStart) { currentCinematicValue = 0f; StartDisplayCinematic(false); }
                    else if (cinematicOutOnStart) { currentCinematicValue = 1f; StopDisplayCinematic(false); }
        ...
        /// Has no effect if already shown.
        /// </summary>
        public void ShowHUD()
        {
            if (!IsHUDShown)
        ...
        /// <summary>
        /// Show the heads-up display. Has no effect if IsInitialised is false.
        /// This is the same as ShowHUD() when Show HUD with Flicker is not enabled.
        /// </summary>
        public void ShowHUDInstantly()
        {
            // Prob makes sense to start in a non-flickering state.
            StopFlickering();
        ...
        /// <summary>
        /// Hide the heads-up display. Has no effect if IsInitialised is false.
        /// This is the same as HideHUD() when Hide HUD with Flicker is not enabled.
        /// </summary>
        public void HideHUDInstantly()
        {
            // Prob makes sense to start in a non-flickering state.
            StopFlickering();
        ...
        public void SetCamera (Camera camera)
        {
            ...
            if (!isInitialised) { canvas = GetCanvas; }

            // Default is ScreenSpaceOverlay.
        ...
        /// <summary>
        /// Get the Cinematic Panel. Create it if it does not already exist.
        /// </summary>
        /// <returns></returns>
        public RectTransform GetCinematicPanel()
        {
            if (hudRectTfrm == null) { hudRectTfrm = GetHUDPanel(); }
            ...
            {
                // Stretched panel
                return SSCUtils.GetOrCreateChildRectTransform(hudRectTfrm, cvsResolutionFull, cinematicPanelName, 0f, 0f, 1f, 1f, 0f, 0f, 1f, 1f);
            }
            else { return null; }
        }
        ...
        /// <summary>
        /// Attempt to show the Altitude metric text (m) when the Altitude indicator is shown.
        /// </summary>
        public void ShowAltitudeMetric()
        {
            showAltitudeMetric = true;
        }
        ...
        /// <summary>
        /// Attempt to turn off the altitude metric text (m) when the Altitude indicator is shown.
        /// </summary>
        public void HideAltitudeMetric()
        {
            showAltitudeMetric = false;
        }
        ...
        /// <summary>
        /// Attempt to show the Air Speed metric text when the Air Speed indicator is show.
        /// </summary>
        public void ShowAirSpeedMetric()
        {
            showAirspeedMetric = true;
        }
        ...
        /// <summary>
        /// Attempt to turn off the Air Speed metric text when the Air Speed indicator is shown.
        /// </summary>
        public void HideAirSpeedMetric()
        {
            showAirspeedMetric = false;
        }

        /// <summary>
        /// Set the airspeed text colour on the heads-up display. Only update the colour if it has changed.
        /// If the module has been initialised, this will also re-colour the HUD with the appropriate brightness.
        /// </summary>
        /// <param name="newColour"></param>
        ...
        #region Public API Methods - Cinematic

        /// <summary>
        /// Halt or interrupt the progress of any cinematic effects
        /// </summary>
        public void CancelDisplayCinematic (bool turnOff)
        {
            if (isInitialised)
            {
                if (turnOff)
                {
                    StopDisplayCinematic(true);
                }
                else
                {
                    // Make sure nothing is running
                    StopCinematicCoroutines();
                }
        ...
        /// <summary>
        /// Set the time a non-instant cinematic effect delays before starting.
        /// This will stop any cinematic effects in progress.
        /// </summary>
        /// <param name="delayTime"></param>
        public void SetCinematicDelay (float delayTime)
        {
            if (delayTime > 0.01f)
            {
                cinematicDelay = delayTime;

                if (isInitialised)
                {
                    StopCinematicCoroutines();

                    if (cinematicWait != null) { cinematicWait = null; }

                    cinematicWait = new WaitForSeconds(cinematicDelay);
                }
            }
            // Make sure it always gets set to something
            else if (isInitialised && cinematicWait == null)
            {
                cinematicWait = new WaitForSeconds(0.01f);
            }
        }

        /// <summary>
        /// Sets the current progress or starting point of the cinematic effect.
        /// Values between 0.0 (off) and 1.0 (fully progressed).
        /// </summary>
        /// <param name="progress"></param>
        public void SetCinematicProgress (float progress)
        {
            if (isInitialised && progress >= 0f && progress <= 1f)
            {
                currentCinematicValue = progress;
            }
        }

        /// <summary>
        /// Start or turn on the cinematic mask overlay.
        /// </summary>
        /// <param name="isInstant"></param>
        public void StartDisplayCinematic (bool isInstant)
        {
            if (isInitialised)
            {
                float height = GetCinematicEdgePanelHeight();
                ...
                if (isInstant) { currentCinematicValue = 1f; }

                cinematicTopImage = GetCinematicImageTop();
                ...
                if (cinematicTopImage != null)
                {
                    ...
                    if (isInstant)
                    {
                        cinematicTopImage.rectTransform.offsetMin = new Vector2(0f, 1080f - height);
                        cinematicTopImage.rectTransform.offsetMax = new Vector2(0f, 0f);
                        cinematicTopImage.enabled = true;
                    }
                }

                cinematicBottomImage = GetCinematicImageBottom();
                ...
                if (cinematicBottomImage != null)
                {
                    ...
                    if (isInstant)
                    {
                        cinematicBottomImage.rectTransform.offsetMin = new Vector2(0f, 0f);
                        cinematicBottomImage.rectTransform.offsetMax = new Vector2(0f, -1080f + height);
                        cinematicBottomImage.enabled = true;
                    }
                }

                if (!isInstant && cinematicTopImage != null && cinematicBottomImage != null)
                {
                    cinematicStartRoutine = StartCoroutine(EnableCinematic());
                }
            }
        }

        /// <summary>
        /// Stop of turn off the cinematic mask overlay
        /// </summary>
        /// <param name="isInstant"></param>
        public void StopDisplayCinematic (bool isInstant)
        {
            if (isInitialised)
            {
                float height = GetCinematicEdgePanelHeight();
                ...
                if (isInstant) { currentCinematicValue = 0f; }
                ...
                if (cinematicTopImage != null)
                {
                    ...
                    if (isInstant)
                    {
                        cinematicTopImage.rectTransform.offsetMin = new Vector2(0f, 1080f);
                        cinematicTopImage.rectTransform.offsetMax = new Vector2(0f, height);
                    }
                }
                ...
                if (cinematicBottomImage != null)
                {
                    ...
                    if (isInstant)
                    {
                        cinematicBottomImage.rectTransform.offsetMin = new Vector2(0f, -height);
                        cinematicBottomImage.rectTransform.offsetMax = new Vector2(0f, -1080f);
                    }
                }

                if (!isInstant && cinematicTopImage != null && cinematicBottomImage != null)
                {
                    cinematicStopRoutine = StartCoroutine(DisableCinematic());
                }
            }
        }
        ...
        /// <summary>
        /// Instantly blank the display with the fade colour.
        /// </summary>
        ...
        public void StartDisplayFade (bool isFadeIn)
        {
            if (isInitialised)
            {
                ...
                if (fadeImage != null)
                {
                    ...
                    if (isFadeIn) { fadeStartAlpha = 1f; fadeTargetValue = 0f; }
        ...
        /// <summary>
        /// Instantly stop blanking or fading the display.
        /// </summary>
        public void UnblankDisplay()
        {
            if (isInitialised)
            {
                if (fadeImage != null)
        ...
        /// <summary>
        /// Attempt to get the first gauge with a matching Gauge Purpose Key.
        /// </summary>
        /// <param name="gaugePurposeKey"></param>
        /// <returns></returns>
        public DisplayGauge GetDisplayGaugeByPurpose (int gaugePurposeKey)
        {
            DisplayGauge displayGauge = null;
            DisplayGauge _tempDG = null;
            ...
            {
                _tempDG = displayGaugeList[dgIdx];
                if (_tempDG != null && _tempDG.gaugePurposeKey == gaugePurposeKey)
                {
                    displayGauge = _tempDG;
                    break;
                }
        ...
        /// <summary>
        /// Get the UI sprite (image) for a Heading Indicator.
        /// See also SetDisplayHeadingIndicatorSprite(..).
        /// </summary>
        /// <returns></returns>
        public Sprite GetDisplayHeadingIndicatorSprite()
        {
            return headingIndicatorSprite;
        }

        /// <summary>
        /// Get the UI sprite (image) for a Heading mask.
        /// See also SetDisplayHeadingMaskSprite(..).
        /// </summary>
        /// <returns></returns>
        public Sprite GetDisplayHeadingMaskSprite()
        {
            return headingMaskSprite;
        }

        /// <summary>
        /// Get the UI sprite (image) for a Heading scolling.
        /// See also SetDisplayHeadingScrollSprite(..).
        /// </summary>
        /// <returns></returns>
        public Sprite GetDisplayHeadingScrollSprite()
        {
            return headingScrollSprite;
        }
        ...
        public void SetDisplayHeadingOffset (float offsetX, float offsetY)
        {
            ...
            if (isInitialised || editorMode)
            {
                ...
                if (headingScrollPanel != null)
                {
                    ...
                    headingInitPositionLS = headingScrollPanel.transform.localPosition;
        ...
        /// <summary>
        /// Attempt to get the display message with the zero-based index
        /// in the list of current messages.
        /// </summary>
        /// <param name="index"></param>
        /// <returns></returns>
        public DisplayMessage GetDisplayMessageByIndex (int index)
        {
            if (index >= 0 && index < GetNumberDisplayMessages)
            {
                return displayMessageList[index];
            }
            else { return null; }
        }
        ...
        /// If fade-in duration is greater than 0 seconds, it will fade in over that period.
        /// </summary>
        /// <param name="guidHash"></param>
        ...
        /// <summary>
        /// Show the Display Message using a zero-based index on the HUD. The HUD will automatically be shown if it is not already visible.
        /// If fade-in duration is greater than 0 seconds, it will fade in over that period.
        /// </summary>
        /// <param name="index"></param>
        public void ShowDisplayMessageByIndex (int index)
        {
            DisplayMessage displayMessage = GetDisplayMessageByIndex(index);
            if (displayMessage != null) { ShowOrHideMessage(displayMessage, true, false); }
            #if UNITY_EDITOR
            else { Debug.LogWarning("WARNING: ShowDisplayMessageByIndex - Could not find message with zero-based index: " + index); }
            #endif
        }
        ...
        /// <summary>
        /// Show the Display Message using a zero-based index on the HUD instantly by ignoring fade settings. The HUD will automatically be shown if it is not already visible.
        /// </summary>
        /// <param name="guidHash"></param>
        public void ShowDisplayMessageByIndexInstant (int index)
        {
            DisplayMessage displayMessage = GetDisplayMessageByIndex(index);
            if (displayMessage != null) { ShowOrHideMessage(displayMessage, true, true); }
            #if UNITY_EDITOR
            else { Debug.LogWarning("WARNING: ShowDisplayMessageByIndexInstant - Could not find message with zero-based index: " + index); }
            #endif
        }
        ...
        /// <summary>
        /// Hide the Display Message using a zero-based index on the HUD. The HUD will automatically be shown if it is not already visible.
        /// If fade-in duration is greater than 0 seconds, it will fade in over that period.
        /// </summary>
        /// <param name="index"></param>
        public void HideDisplayMessageByIndex (int index)
        {
            DisplayMessage displayMessage = GetDisplayMessageByIndex(index);
            if (displayMessage != null) { ShowOrHideMessage(displayMessage, false, false); }
            #if UNITY_EDITOR
            else { Debug.LogWarning("WARNING: HideDisplayMessageByIndex - Could not find message with zero-based index: " + index); }
            #endif
        }
        ...
        /// <summary>
        /// Hide the Display Message using a zero-based index on the HUD instantly by ignoring fade settings. The HUD will automatically be shown if it is not already visible.
        /// </summary>
        /// <param name="guidHash"></param>
        public void HideDisplayMessageByIndexInstant (int index)
        {
            DisplayMessage displayMessage = GetDisplayMessageByIndex(index);
            if (displayMessage != null) { ShowOrHideMessage(displayMessage, false, true); }
            #if UNITY_EDITOR
            else { Debug.LogWarning("WARNING: HideDisplayMessageByIndexInstant - Could not find message with zero-based index: " + index); }
            #endif
        }
                ...
                if (_numDisplayTargets > 0)
                {
                    ...
                    for (int dtIdx = 0; dtIdx < _numDisplayTargets; dtIdx++)
                    {
                        ...
                        if (_tempDT != null)
                        {
                            ...
                            {
                                /// TODO - Find a better Radar solution to avoid GC
                                /// Investigate a pre-allocated list filled with unset (-1?) values
        ...
        #if SCSM_S3D
        /// <summary>
        /// This is a special method that can be called from the Engage Tab on a Sticky3D Controller character when looking at interactive-enabled
        ...
        /// <summary>
        /// You can hook this up to the stickyControlModule.callbackOnChangeLookAtInteractive
        /// delegate so that it is called whenever the player character starts or stops
        /// looking at an interactive-enabled object.
        /// </summary>
        ...
        public void OnInteractiveLookAtChanged (StickyControlModule stickyControlModule, StickyInteractive oldLookAtObject, StickyInteractive newLookAtObject)
        {
            if (stickyControlModule != null)
            {
                ...
                SetDisplayReticleColour(newLookAtObject == null ? stickyControlModule.NonengageColour : stickyControlModule.EngageColour);
            }
        }
