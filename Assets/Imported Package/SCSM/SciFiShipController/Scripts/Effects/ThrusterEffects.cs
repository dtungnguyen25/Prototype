    ...
    public class ThrusterEffects : MonoBehaviour
    {
        ...
        public virtual void UpdateThrusterInput (Thruster thruster, Vector3 shipLocalVelocity)
        {
            if (initialised)
            {
                ...
                // #SMS v1.4.5 check fuelLevel when Min FX Always On. There is a danger that when central fuel is used
                // and user previously set fuelLevel to 0 this will be an issue but I think the risk is minimal.
                // For central fuel, when 0, the throttle is automatically set to 0 when the engines are shutdown in shipcontrolmodule.fixedupdate().
                if (currentThrusterInput > 0f || (!isFXLimited && thruster.isMinEffectsAlwaysOn && thruster.throttle > 0f && thruster.fuelLevel > 0f))
                {
