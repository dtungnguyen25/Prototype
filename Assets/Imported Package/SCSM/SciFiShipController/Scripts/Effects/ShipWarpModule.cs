...
namespace SciFiShipController
{
    ...
    public class ShipWarpModule : MonoBehaviour
    {
        ...
        /// <summary>
        /// Camera clear flags used in BiRP. Not applicable for URP and HDRP.
        /// </summary>
        ...
        /// <summary>
        /// The type of camera used during jumps
        /// </summary>
        public enum SSCWarpCameraType
        {
            Ignore = -1,
            ShipCameraModule = 0,
            StandardUnity = 1
        ...
        /// <summary>
        /// When using built-in render pipeline, should the module attempt to determine what the clear flags should be set to?
        /// </summary>
        public bool isUpdateClearFlags = false;
        ...
        /// <summary>
        /// The amount of boost, in KNewtons, to be applied to the ship when warp is disengaged
        /// </summary>
        public float shipAddBoostOnDisengage = 0f;

        /// <summary>
        /// The rotation Euler angles from forward flight to use when applying boost.
        /// </summary>
        public Vector3 shipBoostRotOnDisengage = Vector3.zero;

        /// <summary>
        /// The time, in seconds, the boost will be applied for when warp is disengaged
        /// </summary>
        [Range(0f, 10f)] public float shipBoostDurationOnDisengage = 1f;

        /// <summary>
        /// Ship movement will be enabled when warp is disengaged
        /// </summary>
        public bool isShipMoveOnDisengage = false;

        /// <summary>
        /// Attempt to snap the camera to the target position and rotation when warp is engaged
        /// </summary>
        public bool isSnapCameraOnEngage = false;
        ...
        /// </summary>
        [Range(0f, 30f)] public float onPostInitialisedEvtDelay = 0f;

        /// <summary>
        /// These methods get called immediately after the module is initialised.
        /// </summary>
        public UnityEvent onPostInitialised = null;
        ...
        /// <summary>
        /// Get or set the ship camera that will be enabled or disabled during jumps
        /// </summary>
        public ShipCameraModule ShipCamera { get { return shipCameraModule; } set { SetShipCameraModule(value); } }
        ...
        /// <summary>
        /// Get the camera type as an int for faster comparison with static integers.
        /// See also GetCameraType().
        /// </summary>
        public int CameraTypeInt { get { return isInitialised ? cameraTypeInt : (int)cameraType; } }

        /// <summary>
        /// The zero-based index of the currently used ShipCameraSetttings.
        /// Will return - 1 if the module is not initialised, or there is no active camera settings applied.
        /// </summary>
        ...
        /// <summary>
        /// Does this warp module have a ship camera assigned to it?
        /// </summary>
        public bool IsShipCameraAssigned { get { return isInitialised ? cameraTypeInt == CameraTypeSSCInt && shipCameraModule != null : cameraType == SSCWarpCameraType.ShipCameraModule && shipCameraModule != null; } }

        /// <summary>
        /// Does this warp module have a camera assigned that is does not use a ship camera module?
        /// </summary>
        public bool IsNonSSCCameraAssigned { get { return isInitialised ? cameraTypeInt == CameraTypeUnityInt && nonSSCCamera != null : cameraType == SSCWarpCameraType.StandardUnity && nonSSCCamera != null; } }

        /// <summary>
        /// Get or set the camera used if not using a Ship Camera Module.
        /// </summary>
        public Camera NonSSCCamera { get { return nonSSCCamera; } set { SetNonSSCCamera(value); } }

        #endregion

        #region Public Properties - Ship

        /// <summary>
        /// Does this warp module have a ship assigned to it?
        /// </summary>
        public bool IsShipAssigned { get { return shipControlModule != null; } }

        #endregion

        #region Public Static Variables

        public static int CameraTypeIgnoreInt = (int)SSCWarpCameraType.Ignore;
        public static int CameraTypeSSCInt = (int)SSCWarpCameraType.ShipCameraModule;
        public static int CameraTypeUnityInt = (int)SSCWarpCameraType.StandardUnity;
        ...
        protected UnityEngine.Rendering.AmbientMode savedAmbientMode;
        protected Color savedAmbientSkyColor;

        protected Color savedCameraBgndColour;
        protected CameraClearFlags savedCameraClearFlags;
        ...
        #region Protected Variables - Identification

        /// <summary>
        /// Unique ID for the WarpModule. When used with SSC X Pack 2, this needs to be unique across all scenes.
        /// If used from a prefab, update the ID after placing it in the scene.
        /// </summary>
        ...
        /// <summary>
        /// The camera type used during the warp
        /// </summary>
        [SerializeField] protected SSCWarpCameraType cameraType = SSCWarpCameraType.ShipCameraModule;

        /// <summary>
        /// The camera used if ship camera module is not in use.
        /// </summary>
        [SerializeField] protected Camera nonSSCCamera = null;
        ...
        /// <summary>
        /// The minimum randomised percentage percentage of the EffectsModule default volume
        /// </summary>
        [SerializeField, Range(0f, 100f)] protected float minSoundVolume = 50f;

        /// <summary>
        /// The maximum randomised percentage of the EffectsModule default volume
        /// </summary>
        [SerializeField, Range(0f, 100f)] protected float maxSoundVolume = 100f;
        ...
        /// <summary>
        /// Is the EffectsModule volume randomised?
        /// </summary>
        ...
        /// <summary>
        /// This is typically called from DisengageWarp()
        /// </summary>
        protected virtual void RestoreRenderSettings()
        {
            RenderSettings.ambientMode = savedAmbientMode;
            RenderSettings.ambientSkyColor = savedAmbientSkyColor;
        }

        protected virtual void UpdateRenderSettings(bool isSaveSettings = true)
        {
        ...
        protected virtual bool ConfigureShipForWarp()
        {
            ...
            {
                #if UNITY_EDITOR
                Debug.LogWarning("[ERROR] ShipWarpModule (" + name + " in scene " + gameObject.scene.name + ") ConfigureShipForWarp - no ship defined on Ship Tab");
                #endif
            }
            else if (!shipControlModule.IsInitialised)
            {
                #if UNITY_EDITOR
                Debug.LogWarning("[ERROR] ShipWarpModule (" + name + " in scene " + gameObject.scene.name + ") ConfigureShipForWarp - " + shipControlModule.name + " on Ship Tab is not initialised");
                #endif
            }
            else
            {
        ...
        /// <summary>
        /// Attempt to update the ship's position and rotation
        /// </summary>
        protected virtual void MoveShip()
        {
            if (isInitialised)
            {
                Vector3 newPos = shipOriginPos + (currentShipRot * currentShipPosOffset);

                // When the ship is kinematic, we need to move the RigidBody.
                shipRBody.MovePosition(newPos);
                shipRBody.MoveRotation(currentShipRot);

                // v1.5.1 Added for U2022.3+ builds
                shipControlModule.transform.SetPositionAndRotation(newPos, currentShipRot);
            }
        }
        ...
        /// <summary>
        /// Configure the main (ship) camera.
        /// </summary>
        /// <param name="isSaveSettings"></param>
        /// <returns></returns>
        protected virtual bool ConfigureCamera (bool isSaveSettings = false)
        {
            ...
            if (camera1 != null)
            {
                if (isSaveSettings)
                {
                    savedCameraBgndColour = camera1.backgroundColor;
                    savedCameraClearFlags = camera1.clearFlags;
                }

                // Infer what the clear flags should be in BiRP (doesn't cater for Depth only)
                if (isUpdateClearFlags)
                {
                    camera1.clearFlags = envAmbientSource == EnvAmbientSource.Skybox ? CameraClearFlags.Skybox : CameraClearFlags.SolidColor;
                }
                camera1.backgroundColor = nightSkyColour;
            ...
            if (isInitialised)
            {
                ...
                if (numEffects > 0)
                {
                    ...
                    else
                    {
                        ...
                        float soundFXVolume = isSoundIntervalRandomised ? sscRandom.Range(defaultVolume * minSoundVolume * 0.01f, defaultVolume * maxSoundVolume * 0.01f) : defaultVolume;
        ...
        protected virtual bool VerifyParticleSystem(ParticleSystem particleSystem)
        {
            ...
            {
                if (particleSystem.transform.IsChildOf(transform))
                {
                ...
                {
                    Debug.LogWarning("[ERROR] ShipWarpModule (" + name + ") VerifyParticleSystem - the particle system must be on a child gameobject of the Warp Module");
                }
            }
        ...
        /// <summary>
        /// Retrieve the camera based on the type.
        /// </summary>
        /// <returns></returns>
        protected Camera GetCameraUsingType()
        {
            if (isInitialised) { cameraTypeInt = (int)cameraType; }

            if (cameraTypeInt == CameraTypeSSCInt) { return shipCameraModule.GetCamera1; }
            else if (cameraTypeInt == CameraTypeUnityInt) { return nonSSCCamera; }
            else { return null; }
        }
        ...
        /// <summary>
        /// Attempt to restore camera settings. Typically called
        /// from DisengageWarp().
        /// </summary>
        protected void RestoreCameraSettings()
        {
            if (camera1 != null)
            {
                camera1.clearFlags = savedCameraClearFlags;
                camera1.backgroundColor = savedCameraBgndColour;
            }
        }

        /// <summary>
        /// Set and verify the (ship) camera
        /// </summary>
        /// <param name="camera"></param>
        /// <returns></returns>
        protected bool VerifyCamera (Camera camera)
        {
            if (camera != null)
            {
                camera1 = camera;
            ...
            {
                Debug.LogWarning("[ERROR] ShipWarpModule (" + name + ") VerifyCamera - camera is null");
                return false;
            }
        }
        ...
        #region Protected and Internal Methods - Events


        protected void DelayOnPostInitialiseEvents()
        {
            onPostInitialised.Invoke();
        }
        ...
        public virtual void DisengageWarp()
        {
            if (isWarpEngaged)
            {
                ...
                if (isShipMoveOnDisengage) { shipControlModule.EnableShipMovement(true); }
                if (shipAddBoostOnDisengage > 0f)
                {
                    shipControlModule.shipInstance.AddBoost(Quaternion.Euler(shipBoostRotOnDisengage) * Vector3.forward, shipAddBoostOnDisengage * 1000f, shipBoostDurationOnDisengage);
                }
        ...
        public virtual void EngageWarp()
        {
            if (!isWarpEngaged)
            {
                ...
                if (ConfigureCamera(true) && ConfigureShipForWarp())
                {
                    ...
                    if (cameraTypeInt == CameraTypeSSCInt)
                    {
                        if (shipCameraModule != null)
                        {
                            ...
                            if (isSnapCameraOnEngage)
                            {
                                shipCameraModule.SnapToTarget();
                            }
                        }
                    }
                    else if (cameraTypeInt == CameraTypeUnityInt)
                    {
                        // Allow camera to move (but not render) Nothing to do here.
                    }
        ...
        public virtual void Initialise()
        {
            ...
            {
                // If the WarpModule doesn't have a unique ID, make one now.
                if (guidHash == 0) { guidHash = SSCMath.GetHashCodeFromGuid(); }

                // v1.4.9 move to EngageWarp() to avoid conflict with Celestials
                //UpdateRenderSettings();

                cameraTypeInt = (int)cameraType;

                if (cameraTypeInt == CameraTypeSSCInt && shipCameraModule == null)
                {
                    #if UNITY_EDITOR
                    Debug.LogWarning("[ERROR] ShipWarpModule (" + name + " in scene " + gameObject.scene.name + ") FX tab - ship camera module is not setup");
                    #endif
                }
                else if (cameraTypeInt == CameraTypeUnityInt && nonSSCCamera == null)
                {
                    #if UNITY_EDITOR
                    Debug.LogWarning("[ERROR] ShipWarpModule (" + name + " in scene " + gameObject.scene.name + ") FX tab - camera is not setup");
                    #endif
                }
                // v1.4.9 move ConfigureCamera to EngageWarp() to avoid conflict with Celestials
                // v1.5.0 allow one or both particle FX to be null.
                else if ((cameraTypeInt == CameraTypeIgnoreInt || VerifyCamera(GetCameraUsingType())) && (innerParticleSystem == null || VerifyParticleSystem(innerParticleSystem)) && (outerParticleSystem == null || VerifyParticleSystem(outerParticleSystem)))
                {
                    DisableParticleFX();
        ...
        public virtual void SetEnvironmentAmbientSource (EnvAmbientSource newEnvAmbientSource)
        {
            ...
            // Updated for v1.4.9 to avoid conflict with Celestials
            if (isWarpEngaged)
            {
                UpdateRenderSettings(false);
                ConfigureCamera();
            }

            //if (camera1 != null) { ConfigureCamera(camera1); }
        }
        ...
        #region Public API Methods - Identification

        /// <summary>
        /// Get the unique identifier code for this Warp Module.
        /// If it returns 0, call SetNewId().
        /// </summary>
        /// <returns></returns>
        public int GetWarpModuleId()
        {
            return guidHash;
        }

        /// <summary>
        /// When placing the prefab in a scene, if used with SSC Expansion Pack 2, it is useful
        /// to have a unique Id across all warp modules used in the project.
        /// </summary>
        ...
        /// <summary>
        /// Attempt to get the ShipControlModule for this warp module.
        /// </summary>
        /// <returns></returns>
        public ShipControlModule GetShipControlModule()
        {
        ...
        {
            if (isInitialised && cameraTypeInt == CameraTypeSSCInt && shipCameraModule != null)
            {
                    ...
                    if (settingIdx >= 0 && settingIdx < numCamSettings)
                    {
                        ...
                        if (camSettings != null)
                        {
                            ...
                            if (camSettings.GetHashCode() != currentCameraSettingsHash)
                            {
                                ...
                                int shipId = shipControlModule != null ? shipControlModule.GetShipId : 0;

                                if (onChangeCameraSettings != null) { onChangeCameraSettings.Invoke(prevCameraSettingsIndex+1, currentCameraSettingsIndex+1, shipId, false); }
                            }
                            break;
                        }
                        else
        ...
        /// <summary>
        /// Get the type of camera type used during the warp
        /// </summary>
        /// <returns></returns>
        public SSCWarpCameraType GetCameraType()
        {
            return cameraType;
        }
        ...
        {
            return isInitialised ? camera1 : cameraType == SSCWarpCameraType.StandardUnity ? nonSSCCamera : cameraType == SSCWarpCameraType.ShipCameraModule && shipCameraModule != null ? shipCameraModule.GetCamera1 : null;
        }

        /// <summary>
        /// Get the camera used if not using a Ship Camera Module.
        /// </summary>
        /// <returns></returns>
        public Camera GetNonSSCCamera()
        {
            return nonSSCCamera;
        }
        ...
        /// <summary>
        /// Rather than use a ShipCameraModule camera, use a different camera setup.
        /// </summary>
        /// <param name="newCamera"></param>
        public void SetNonSSCCamera (Camera newCamera)
        {
            nonSSCCamera = newCamera;
        }
        ...
        /// <summary>
        /// Set the camera type used during the warp
        /// </summary>
        /// <param name="newCameraType"></param>
        public void SetCameraType (SSCWarpCameraType newCameraType)
        {
