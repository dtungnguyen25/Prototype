...
namespace SciFiShipController
{
    ...
    public class ShipDockingStation : MonoBehaviour
    {
        ...
        private void UnDockShipInternal (ShipControlModule dockedShip, ShipDocking shipDocking)
        {
            ...
            // v1.4.4 check AI is actually enabled.
            if (aiShip != null && aiShip.IsInitialised && aiShip.IsAIEnabled) { shipDocking.SetState(ShipDocking.DockingState.Undocking); }
            // This is NOT an AI nor an AI-assisted ship or AI is not enabled, so go straight to not docked state
        ...
        /// <summary>
        /// Attempt to add a ShipDockingPoint to the ship docking station
        /// </summary>
        /// <param name="shipDockingPoint"></param>
        public void AddShipDockingPoint (ShipDockingPoint shipDockingPoint)
        {
            if (shipDockingPoint != null)
            {
                if (shipDockingPointList == null) { shipDockingPointList = new List<ShipDockingPoint>(10); }

                shipDockingPointList.Add(shipDockingPoint);

                numDockingPoints = shipDockingPointList.Count;
            }
        }

        /// <summary>
        /// Add a new ShipDockingPoint to the docking station
        /// </summary>
        /// <returns></returns>
        public ShipDockingPoint AddShipDockingPoint()
        {
            ShipDockingPoint shipDockingPoint = new ShipDockingPoint();
            AddShipDockingPoint(shipDockingPoint);
            return shipDockingPoint;
        }
        ...
        public void DockShip (int dockingPointIndex)
        {
            ...
            if (shipControlModule != null)
            {
                ...
                if (shipDocking != null)
                {
                    ...
                    // v1.4.4 check AI is actually enabled.
                    if (aiShip != null && aiShip.IsInitialised && aiShip.IsAIEnabled) { shipDocking.SetState(ShipDocking.DockingState.Docking); }
        ...
        /// <summary>
        /// Safely retrieve a ShipDockingPoint from the ShipDockingStation using the zero-based index in the list.
        ...
        /// <summary>
        /// Get the world space docking point rotation with a local Euler offset.
        /// </summary>
        /// <param name="shipDockingPoint"></param>
        /// <param name="offsetRotation"></param>
        /// <returns></returns>
        public Quaternion GetDockingPointRotationWithOffset (ShipDockingPoint shipDockingPoint, Vector3 offsetRotation)
        {
            if (shipDockingPoint != null)
            {
                if (offsetRotation.x != 0f || offsetRotation.y != 0f || offsetRotation.z != 0f)
                {
                    return (isStationShipInitialised ? shipControlModuleStation.shipInstance.TransformRotation : transform.rotation) * (Quaternion.Euler(offsetRotation) * Quaternion.Euler(shipDockingPoint.relativeRotation));
                }
                else
                {
                    return (isStationShipInitialised ? shipControlModuleStation.shipInstance.TransformRotation : transform.rotation) * Quaternion.Euler(shipDockingPoint.relativeRotation);
