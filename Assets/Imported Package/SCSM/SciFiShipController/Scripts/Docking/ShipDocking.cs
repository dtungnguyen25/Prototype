...
namespace SciFiShipController
{
    ...
    public class ShipDocking : MonoBehaviour
    {
        ...
        /// <summary>
        /// The dockingAdapter being used to dock or undock with.
        /// Numbers begin at 1.
        /// </summary>
        private int activeAdapterNumber = 1;
        ...
        public void Initialise()
        {
            if (!IsInitialised)
            {
                ...
                if (shipControlModule != null)
                {
                    ...
                    // cache to avoid having to check for null etc in UpdateLoop etc
                    isShipInitialised = shipControlModule.IsInitialised;

                    timingUpdateInt = (int)shipControlModule.shipInstance.timingUpdateLoop;

                    if (timingUpdateInt == Ship.SSCTimingManualInt)
                    {
                        shipControlModule.callbackOnUpdateLoop += UpdateLoop;
                    }

                    shipControlModule.shipInstance.callbackOnTimingUpdateChanged += OnTimingUpdateChanged;
                }
        ...
        private void Update()
        {
            if (timingUpdateInt == Ship.SSCTimingAutoInt)
            {
            ...
            bool hoverPosition, ref Vector3 targetDockingPos, ref Quaternion targetDockingRot)
        {
            ...
            Vector3 adapterDirection = adapterList[activeAdapterNumber - 1].relativeDirection;
            ...
            // Include an optional offset rotation from the ShipDocking adapter.
            targetDockingRot = shipDockingStation.GetDockingPointRotationWithOffset(shipDockingPoint, adapterList[activeAdapterNumber - 1].offsetRelativeRotation);
            ...
            targetDockingPos -= targetDockingRot * adapterList[activeAdapterNumber - 1].relativePosition;
        }
        ...
        #region Private Methods - Timing

        private void OnTimingUpdateChanged(Ship ship, Ship.SSCTiming timing)
        {
            shipControlModule.callbackOnUpdateLoop -= UpdateLoop;

            timingUpdateInt = (int)timing;
        ...
        /// <summary>
        /// Attempt to add a ship docking adapter.
        /// See also GetActiveDockingAdapter() and SetActiveDockingAdapter(..)
        /// </summary>
        /// <returns></returns>
        public ShipDockingAdapter AddDockingAdapter()
        {
            ShipDockingAdapter shipDockingAdapter = new ShipDockingAdapter();
            if (AddDockingAdapter(shipDockingAdapter))
            {
                return shipDockingAdapter;
            }
            else
            {
                return null;
        ...
        /// See also GetActiveDockingAdapter() and SetActiveDockingAdapter(..)
        /// </summary>
        /// <param name="shipDockingAdapter"></param>
        public bool AddDockingAdapter (ShipDockingAdapter shipDockingAdapter)
        {
            if (shipDockingAdapter != null)
            {
                if (!IsInitialised)
                {
                    if (adapterList == null) { adapterList = new List<ShipDockingAdapter>(2); }
                }

                adapterList.Add(shipDockingAdapter);
                return true;
            }
            else
            {
        ...
        /// <summary>
        /// Get the number of the active ship docking adapter.
        /// </summary>
        /// <returns></returns>
        public int GetActiveDockingAdapter()
        {
            return activeAdapterNumber;
        }
        ...
        /// <summary>
        /// Attempt to get the ship docking adapter based on its number.
        /// The numbers begin at 1.
        /// </summary>
        /// <param name="adapterNumber"></param>
        /// <returns></returns>
        public ShipDockingAdapter GetDockingAdapter (int adapterNumber)
        {
            if (adapterNumber > 0 && adapterNumber <= adapterList.Count)
            {
                return adapterList[adapterNumber - 1];
            }
            else
            {
                return null;
        ...
        /// <param name="adapterNumber"></param>
        /// <returns></returns>
        public Quaternion GetDockingAdapterRotation (int adapterNumber)
        {
            if (adapterNumber > 0 && adapterNumber <= adapterList.Count)
            {
        ...
        [System.ObsoleteAttribute("Typo in API - instead use GetDockingAdapterRotation(..)")]
        public Quaternion GetDockingAdapteRotation(int adapterNumber)
        {
            return GetDockingAdapterRotation(adapterNumber);
        }
        ...
        /// <summary>
        /// Set which Ship Docking Adapter the ship will use to dock or undock with.
        /// Numbers begin at 1.
        /// </summary>
        /// <param name="newActiveDockingAdapterNumber"></param>
        public void SetActiveDockingAdapter (int newActiveDockingAdapterNumber)
        {
            if (newActiveDockingAdapterNumber > 0 && newActiveDockingAdapterNumber <= adapterList.Count)
            {
                activeAdapterNumber = newActiveDockingAdapterNumber;
            }
        }

        /// <summary>
        /// Attempt to set the offsetRelativeRotation for the docking adapter. Adapter numbers begin at 1.
        /// This could be useful if you want to approach a ShipDockingStation point from the opposite direction.
        /// e.g. SetDockingAdapterOffsetRotation(1, new Vector3(180f, 0f, 0f)
        /// </summary>
        /// <param name="dockingAdapterNumber"></param>
        /// <param name="eulerOffsetRotation"></param>
        public void SetDockingAdapterOffsetRotation (int dockingAdapterNumber, Vector3 eulerOffsetRotation)
        {
            ShipDockingAdapter adapter = GetDockingAdapter(dockingAdapterNumber);

            if (adapter != null)
            {
                adapter.offsetRelativeRotation = eulerOffsetRotation;
            }
        }
        ...
        public void SetState (DockingState dockingState)
        {
            ...
            if (shipControlModule != null)
            {
                ...
                else if (dockingStateInt == notDockedInt)
                {
                    ...
                    if (previousDockingStateInt != notDockedInt)
                    {
                        ...
                        if (shipControlModule.IsInitialised && dockWithShip != null && dockWithShip.IsInitialised)
                        {
                            ...
                            shipControlModule.ShipRigidbody.linearVelocity = dockWithShip.ShipRigidbody.linearVelocity + (dockWithShip.shipInstance.RigidbodyUp * undockVertVelocity) + (dockWithShip.shipInstance.RigidbodyForward * undockFwdVelocity)
                ...
                else if (dockingStateInt == dockingInt || dockingStateInt == undockingInt)
                {
                    ...
                    {
                        Debug.Log("ERROR: attempting Docking or Undocking without a ShipDockingStation or without correctly initialising it on " + name + " in " + gameObject.scene.name);
                    }            
                    #endif
                }
                #endregion
