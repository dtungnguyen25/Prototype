...
namespace SciFiShipController
{
    ...
    /// This should always be a root gameobject in the scene.
    /// </summary>
    ...
    public class SSCRadar : MonoBehaviour
    {
        ...
        [System.NonSerialized] private static readonly List<SSCRadar> radarList = new List<SSCRadar>(2);
        [System.NonSerialized] private static List<GameObject> tempGameObjectList = new List<GameObject>();
        private static int numberRadars = 0;
        ...
        /// <summary>
        /// Currently only used by GetRadarItemLocation(..)
        /// </summary>
        ...
        private void Start()
        {
            ...
            {
                GetOrCreateRadar(gameObject.scene.handle);
        ...
        /// <summary>
        /// [INTERNAL ONLY] Instead use SSCRadar.GetOrCreateRadar(sceneHandle) 
        /// </summary>
        ...
        private void DisplayResults(bool redraw = false)
        {
            ...
            int outerRadius = (int)(sscRadarDisplay.uiTexWidth / 2f);

            sscRadarDisplay.radarTex = radarImage.texture as Texture2D;

            // If the canvas image has been resized but doesn't match
            // the width of the texture, refresh it.
            if (sscRadarDisplay.uiTexWidth > sscRadarDisplay.radarTex.width)
            {
                RefreshRadarImageStatus();
                return;
            }
            ...
            DrawCircle(sscRadarDisplay.radarTex, ref sscRadarDisplay.uiRimPixels, sscRadarDisplay.uiTexCentreX, sscRadarDisplay.uiTexCentreY, outerRadius, overlayColour, true, redraw);
            DrawCircle(sscRadarDisplay.radarTex, ref sscRadarDisplay.uiInnerPixels, sscRadarDisplay.uiTexCentreX, sscRadarDisplay.uiTexCentreY, outerRadius - sscRadarDisplay.uiRimWidth, backgroundColour, true, redraw);
            ...
            if (sscRadarDisplay.isFollowShip && shipToFollow != null)
            {
            ...
            else if (sscRadarDisplay.isFollowGameObject && gameobjectToFollow != null)
            {
            ...
            {
                if (isCallbackEnabled) { callbackOnDrawBlip(sscRadarDisplay.radarTex, displayUIRotation, factionId, sscRadarResultsList[blipIdx]); }
                else { DrawBlip(sscRadarDisplay, sscRadarQuery, displayUIRotation, factionId, sscRadarResultsList[blipIdx]); }
            }

            // Only apply once all operations have finished
            sscRadarDisplay.radarTex.Apply();
            radarImage.texture = sscRadarDisplay.radarTex;
        }
        ...
        /// </summary>
        /// <param name="radarDisplay"></param>
        /// <param name="radarQuery"></param>
        /// <param name="displayRotation"></param>
        /// <param name="factionId"></param>
        /// <param name="sscRadarBlip"></param>
        private void DrawBlip(SSCRadarDisplay radarDisplay, SSCRadarQuery radarQuery, Quaternion displayRotation, int factionId, SSCRadarBlip sscRadarBlip)
        {
            Vector3 rotatedPosition = displayRotation * (sscRadarBlip.wsPosition - radarQuery.centrePosition);

            int blipOffsetX = (int)((rotatedPosition.x / radarQuery.range) * radarDisplay.uiTexWidth / 2f);
            int blipOffsetZ = (int)((rotatedPosition.z / radarQuery.range) * radarDisplay.uiTexHeight / 2f);

            Color blipColour = radarDisplay.blipNeutralColour;
            ...
            if (factionId != 0 && sscRadarBlip.factionId != 0)
            {
                ...
                if (sscRadarBlip.factionId == factionId) { blipColour = radarDisplay.blipFriendColour; }
                else { blipColour = radarDisplay.blipFoeColour; }
            }

            int blipSize = (int)sscRadarBlip.blipSize;

            // Draw a 3x3, 4x4, 5x5 blip etc
            for (int x = blipOffsetX + radarDisplay.uiTexCentreX - blipSize; x < blipOffsetX + radarDisplay.uiTexCentreX + blipSize; x++)
            {
                for (int y = blipOffsetZ + radarDisplay.uiTexCentreY - blipSize; y < blipOffsetZ + radarDisplay.uiTexCentreY + blipSize; y++)
                {
                    // Clip with display rim width indent
                    if (x > radarDisplay.uiRimWidth + 1 && x < radarDisplay.uiTexWidth - radarDisplay.uiRimWidth - 2 && y > radarDisplay.uiRimWidth + 1 && y < radarDisplay.uiTexHeight - radarDisplay.uiRimWidth - 2)
                    {
                        radarDisplay.radarTex.SetPixel(x, y, blipColour);
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Attempt to add a SSCRadar to the current scene
        /// </summary>
        /// <param name="sceneHandle"></param>
        /// <returns></returns>
        private static SSCRadar AddSSCRadar(int sceneHandle)
        {
            if (sceneHandle != 0)
            {
                ...
                if (activeScene.handle != sceneHandle)
                {
                    ...
                    for (int sIdx = 0; sIdx < numScenes; sIdx++)
                    {
                        ...
                        {
                            // If an additive scene is just loaded it may not be fully loaded until the next frame
                            // The workaround is to place the SSCRadar in the scene at design time.
                            if (_scene.isLoaded)
                            {
                            ...
                            {
                                Debug.LogWarning("[ERROR] SSCRadar.AddSSCRadar() - scene " + _scene.name + " is NOT fully loaded. As a workaround place in SSCRadar in the scene at design time.");
                            }
                            break;
                        }
                    }
            ...
            // Create a new radar in the scene
            GameObject newRadarGameObject = new GameObject("SSC Radar");
            newRadarGameObject.transform.SetPositionAndRotation(Vector3.zero, Quaternion.identity);
            newRadarGameObject.transform.parent = null;
            SSCRadar _sscRadar = newRadarGameObject.AddComponent<SSCRadar>();
            _sscRadar.sceneHandle = newRadarGameObject.scene.handle;
        ...
        /// Typically you should be using GetOrCreateRadar(sceneHandle).
        /// </summary>
        /// <returns></returns>
        ...
        /// USAGE: SSCRadar sscRadar = SSCRadar.GetOrCreateRadar(sceneHandle);
        /// </summary>
        /// <returns></returns>
        public static SSCRadar GetOrCreateRadar (int sceneHandle = 0)
        {
            SSCRadar _sscRadar = null;

            // Attempt to find the handle in the list of current SSCRadars
            for (int mIdx = numberRadars-1; mIdx >= 0; mIdx--)
            {
                SSCRadar _tempRadar = radarList[mIdx];
                if (_tempRadar != null)
                {
                    // If no scene specified or this is the scene, set the radar
                    if (sceneHandle == 0 || _tempRadar.sceneHandle == sceneHandle)
                    {
                        _sscRadar = _tempRadar;
                        break;
                    }
                }
                else
                {
                    // If, for some reason this radar is null, remove this slot from the list
                    radarList.RemoveAt(mIdx);
                    numberRadars--;
                }
            }

            // If there wasn't a radar in the list, see if we can find one in the indicated scene.
            if (_sscRadar == null)
            {
                int numScenes = SceneManager.sceneCount;
                ...
                for (int sIdx = 0; sIdx < numScenes; sIdx++)
                {
                    if (sceneHandle == 0 || SceneManager.GetSceneAt(sIdx).handle == sceneHandle)
                    {
                        ...
                        // Assume SSCRadar is always a root level object
                        for (int rIdx = 0; rIdx < numRootGO; rIdx++)
                        {
                            if (tempGameObjectList[rIdx].TryGetComponent(out _sscRadar))
                            {
                                if (_sscRadar.sceneHandle == 0) { _sscRadar.sceneHandle = sceneHandle == 0 ? _scene.handle : sceneHandle; }
                                radarList.Add(_sscRadar);
                                numberRadars = radarList.Count;
                                break;
                            }
                        }
                ...
                // If this scene does not already have a radar, create one
                if (_sscRadar == null)
                {
                    _sscRadar = AddSSCRadar(sceneHandle);
                    radarList.Add(_sscRadar);
                    numberRadars = radarList.Count;
                }
            }

            // OLD pre-scene-aware code
            ...
            if (_sscRadar != null)
            {
                ...
                if (!_sscRadar.isInitialised) { _sscRadar.Initialise(); }
            }
            #if UNITY_EDITOR
            // If _sscRadar is still null, log a warning to the console
            else
            {
                Debug.LogWarning("SSCRadar GetOrCreateRadar(" + sceneHandle + ") Warning: Could not find or create radar, so returned null.");
            }
            #endif
        ...
        /// <returns>The item index from the radar pool</returns>
        ...
        /// <summary>
        /// Attempt to retrieve a location from the SSCManager in the same scene given a radarItem.
        /// </summary>
        /// <param name="radarItem"></param>
        /// <returns></returns>
        public LocationData GetRadarItemLocation (SSCRadarItem radarItem)
        {
            LocationData location = null;

            if (isInitialised && radarItem != null && radarItem.radarItemType == SSCRadarItem.RadarItemType.Location)
            {
                if (sscManager == null) { sscManager = SSCManager.GetOrCreateManager(sceneHandle); }

                location = sscManager.GetLocation(radarItem.guidHash);
            }
        ...
        /// <summary>
        /// If the module is initialised, refresh or update the colours
        /// used to render the mini-map.
        /// </summary>
        ...
        public void RefreshRadarImageStatus()
        {
            ...
            if (isRadarImageAvailable)
            {
                ...
                ResultCount = 0;
                sscRadarDisplay.uiTexWidth = radarImage.texture.width;
                sscRadarDisplay.uiTexHeight = radarImage.texture.height;
        ...
        #region Public API Member Methods - Texture

        /// <summary>
        /// Attempt to output the results to a texture. For performance, assumes parameters are not null.
        /// See also SampleRadarToMaterial.cs
        /// </summary>
        /// <param name="radarDisplay"></param>
        /// <param name="radarQuery"></param>
        /// <param name="queryResultList"></param>
        /// <param name="redraw"></param>
        public void DrawTexture (SSCRadarDisplay radarDisplay, SSCRadarQuery radarQuery, List<SSCRadarBlip> queryResultList, bool redraw)
        {
            int numBlips = queryResultList.Count;
            int outerRadius = (int)(radarDisplay.uiTexWidth / 2f);

            // Cache the texture pixels to improve performance and prevent GC in each frame
            DrawCircle(radarDisplay.radarTex, ref radarDisplay.uiRimPixels, radarDisplay.uiTexCentreX, radarDisplay.uiTexCentreY, outerRadius, radarDisplay.overlayColour, true, redraw);
            DrawCircle(radarDisplay.radarTex, ref radarDisplay.uiInnerPixels, radarDisplay.uiTexCentreX, radarDisplay.uiTexCentreY, outerRadius - radarDisplay.uiRimWidth, radarDisplay.backgroundColour, true, redraw);

            int factionId = 0; // neutral

            // Get the direction the central ship or gameobject is facing
            if (radarDisplay.isFollowShip && radarDisplay.shipToFollow != null)
            {
                if (radarDisplay.shipToFollow.shipInstance != null)
                {
                    radarDisplay.uiRotation = Quaternion.Euler(0f, radarDisplay.shipToFollow.shipInstance.TransformInverseRotation.eulerAngles.y, 0f);
                    factionId = radarDisplay.shipToFollow.shipInstance.factionId;
                }
                else
                {
                    // Ship may be in the process of being destroyed
                    //FollowShip(null);
                    radarDisplay.isFollowShip = false;
                    radarDisplay.shipToFollow = null;
                    radarDisplay.uiRotation = Quaternion.LookRotation(radarDisplay.defaultFwdDirection);
                }
            }
            else if (radarDisplay.isFollowGameObject && radarDisplay.gameobjectToFollow != null)
            {
                radarDisplay.uiRotation = Quaternion.Euler(0f, Quaternion.Inverse(radarDisplay.gameobjectToFollow.transform.rotation).eulerAngles.y, 0f);
            }
            else { radarDisplay.uiRotation = Quaternion.LookRotation(radarDisplay.defaultFwdDirection); }

            // Populate the UI display with blips
            for (int blipIdx = 0; blipIdx < numBlips; blipIdx++)
            {
                //if (isCallbackEnabled) { callbackOnDrawBlip(sscRadarDisplay.radarTex, displayUIRotation, factionId, sscRadarResultsList[blipIdx]); }
                //else { DrawBlip(sscRadarDisplay.radarTex, displayUIRotation, factionId, sscRadarResultsList[blipIdx]); }
                DrawBlip(radarDisplay, radarQuery, radarDisplay.uiRotation, factionId, queryResultList[blipIdx]);
            }

            // Only apply once all operations have finished
            radarDisplay.radarTex.Apply();
        }
