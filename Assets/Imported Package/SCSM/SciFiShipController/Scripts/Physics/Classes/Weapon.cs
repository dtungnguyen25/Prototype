...
namespace SciFiShipController
{
    ...
    public class Weapon
    {
        ...
        /// <summary>
        /// When there are multiple firing positions,
        /// when the fire command is issued, how should the
        /// weapon respond? By default, all fire positions are
        /// fired at the same time (subject to available ammo).
        /// </summary>
        ...
        public static readonly int FiringTypeAllInt = (int)FiringType.All;
        public static readonly int FiringTypeSingleInt = (int)FiringType.Single;

        public static readonly int FiringButtonNoneInt = (int)FiringButton.None;
        public static readonly int FiringButtonPrimaryInt = (int)FiringButton.Primary;
        public static readonly int FiringButtonSecondaryInt = (int)FiringButton.Secondary;
        public static readonly int FiringButtonAutoInt = (int)FiringButton.AutoFire;
        public static readonly int FiringButtonManualInt = (int)FiringButton.ManualFire;
        ...
        /// <summary>
        /// When there are multiple firing positions, when the fire command is issued, how should the
        /// weapon respond? By default, all fire positions are fired at the same time (subject to available ammo).
        /// Currently this is only available for weapons using missiles from the SSC Expansion Pack 1 (if installed)
        /// </summary>
        ...
        /// </summary>
        [Range(0.01f, 30f)] public float reloadTime;
        /// <summary>
        ...
        /// ManualFire for use with FireIfReady() APIs.
        /// </summary>
        ...
        /// <summary>
        /// Does the beam weapon need to be manually recharged in code
        /// by setting the chargeAmount to a value between 0.0 and 1.0?
        /// </summary>
        public bool isManualRecharge;
        ...
        /// <summary>
        /// The time, in seconds, the turret will stop rotating after it has fired.
        /// This can be used to give the missile(s) a chance to clear the turret.
        /// Currently this is only supported on weapons that fire missiles from
        /// SSC Expansion Pack 1 (if installed).
        /// </summary>
        [Range(0f, 5f)] public float turretMoveDelay;
        ...
        /// If SSC Expansion Pack 1 is installed, also used for turret and fixed missile launchers.
        /// </summary>
        ...
        /// <summary>
        /// If SSC Expansion Pack 1 is installed, used for turret and fixed missile launchers.
        /// When firing, does the missile need to be locked on to a target?
        /// </summary>
        public bool isRequireTargetLock;
        ...
        /// <summary>
        /// Returns true if the weapon has a target (Location, Ship Damage Region, Ship or Gameobject) assigned.
        /// </summary>
        public bool HasTarget { get { return targetguidHash != 0 || target != null; } }
        ...
        /// <summary>
        /// Cached from the projectilePrefab for faster lookup when a projectile is instantiated.
        /// A missile is a guided to target and part of SSC Expansion Pack 1.
        /// </summary>
        public bool IsMissile { get; internal set; }
        ...
        [System.NonSerialized] public int firingTypeInt;
        ...
        /// <summary>
        /// When firing type is Single, which position should be fired?
        /// Positions are zero-based.
        /// </summary>
        internal int currentFiringPosition;
        ...
        /// </summary>
        internal bool isManualFireNow = false;

        /// <summary>
        /// [INTERNAL USE ONLY]
        /// Is the current target a location. Can be used to avoid testing for null of Location.
        /// NOTE: Doesn't validate that the location still exists.
        /// </summary>
        internal bool isTargetLocation;

        /// <summary>
        /// Used with turretMoveDelay. Is set whenever a turret weapon is fired,
        /// turretMoveDelay > 0, and the weapon is firing a missile from SSC
        /// Expansion Pack 1 (if installed).
        /// </summary>
        internal float turretMoveDelayTimer;
        ...
        /// <summary>
        /// [INTERNAL USE ONLY] Call weapon.SetLocation(..), launchModule.SetLocation(..), or ship.SetWeaponLocation(..).
        /// The current location (if any) being targeted when by a missile from SSC Expansion Pack 1.
        /// </summary>
        [System.NonSerialized] internal LocationData targetLocation;
        ...
        public Weapon(Weapon weapon)
        {
            ...
            else
            {
                ...
                this.isManualRecharge = weapon.isManualRecharge;
                ...
                this.turretMoveDelay = weapon.turretMoveDelay;
                ...
                this.isRequireTargetLock = weapon.isRequireTargetLock;
        ...
        /// <param name="isLauncherTurret"></param>
        internal void MoveTurret(Vector3 worldVelocity, bool isSurfaceTurret, bool isLauncherTurret)
        {
            if (turretPivotY != null)
            {
                float dTime = Time.deltaTime;

                // Currently only weapons that fire missiles from the SSC Expansion Pack 1 (if installed)
                // can delay movement after firing.
                bool canMove = turretMoveDelayTimer == 0f;

                // Check if it is ok to rotate the turret
                if (!canMove)
                {
                    turretMoveDelayTimer -= dTime;
                    if (turretMoveDelayTimer <= 0f)
                    {
                        canMove = true;
                        turretMoveDelayTimer = 0f;
                    }
                }

                // +v1.4.5 consider Location for XPack1 missiles
                if (targetguidHash != 0 || target != null)
                {
                    ...
                    // Not a ship but with a guidHash must be a Location
                    else if (targetguidHash != 0)
                    {
                        ...
                        {
                            targetLastPosition = targetLocation.position;
                        }
                    }
                    ...
                    if (targetRelativeSpeed > 0.1f && !isProjectileKGuideToTarget && !IsMissile && weaponTypeInt != TurretBeamInt)
                    {
                    ...
                    if (!isSurfaceTurret && !isLauncherTurret)
                    {
                    ...
                    if (canMove)
                    {
                        ...
                        turretPivotY.localRotation = Quaternion.RotateTowards(turretPivotY.localRotation,
                            Quaternion.Euler(0f, azimuthAngle, 0f), turretMoveSpeed * dTime);
                    }
                    ...
                    if (turretPivotX != null)
                    {
                        ...
                        if (canMove)
                        {
                            ...
                            turretPivotX.localRotation = Quaternion.RotateTowards(turretPivotX.localRotation, Quaternion.Euler(-altitudeAngle, 0f, 0f), turretMoveSpeed * dTime);
                        }
                ...
                else
                {
                    ...
                    if (turretReturnToParkInterval > 0f && !isParked)
                    {
                        ...
                        // If the delayed move after firing is enabled, we still keep the park countdown timer going.
                        if (returnToParkTimer < turretReturnToParkInterval) { returnToParkTimer += dTime; }
                        else
                        {
                            if (canMove)
                            {
                                // Move the barrel(s) up or down towards original rotation
                                turretPivotY.localRotation = Quaternion.RotateTowards(turretPivotY.localRotation, intPivotYLocalRot, turretMoveSpeed * dTime);

                                // Rotate the base left or right towards the initial rotation
                                if (turretPivotX != null)
                                {
                                    turretPivotX.localRotation = Quaternion.RotateTowards(turretPivotX.localRotation, intPivotYLocalRot, turretMoveSpeed * dTime);
                                }
                            }
        ...
        public void SetClassDefaults()
        {
            ...
            this.firePositionList = new List<Vector3>(2);
            this.firePositionList.Add(Vector3.zero);
            this.firingType = FiringType.All;
        ...
        public void Initialise(Quaternion rootObjectInvRotation)
        {
            ...
            #if !SCSM_SSCXP1
            firingType = FiringType.All;
            currentFiringPosition = 0;
            #else
            currentFiringPosition = 0;
            #endif

            firingTypeInt = (int)firingType;

            if (firingButton == FiringButton.AutoFire && !(IsTurretWeapon || (weaponTypeInt == Weapon.FixedProjectileInt && projectilePrefab != null && projectilePrefab.IsMissile)))
            {
                Debug.LogWarning("[ERROR] Init Weapon (" + name + ") does not support AutoFire as only turrets, or fixed projectile weapons (with Missiles from SSC Expansion Pack 1) do. Disabling Fire Button.");
                firingButton = FiringButton.None;
            }
            ...
            if (weaponTypeInt == FixedBeamInt || weaponTypeInt == TurretBeamInt)
            {
                ...
                if (rechargeTime == 0f && !isManualRecharge) { chargeAmount = 1f; }
        ...
        /// <summary>
        /// Set the target location for the weapon. Currently only applies to launch module
        /// from SSC Expansion Pack 1 (if installed).
        /// </summary>
        /// <param name="targetLocationData"></param>
        public void SetTargetLocation (LocationData targetLocationData)
        {
            isLockedOnTarget = false;
            targetShip = null;
            targetShipDamageRegion = null;
            ...
            {
                targetguidHash = targetLocation.guidHash;
        ...
        /// or missiles from SSC Expansion Pack 1 (if installed)
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Get the current target location for this weapon (if any).
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Get the current target ship damage region for this weapon (if any). Only applies to Turret-type weapons and FixedProjectile weapons
        /// with guided projectiles, or missiles from SSC Expansion Pack 1 (if installed).
        /// </summary>
        /// <returns></returns>
        public DamageRegion GetTargetShipDamageRegion()
        {
            return targetShipDamageRegion;
        }
