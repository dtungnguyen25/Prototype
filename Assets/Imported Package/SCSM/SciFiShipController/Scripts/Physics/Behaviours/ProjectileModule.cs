...
namespace SciFiShipController
{
    ...
    public class ProjectileModule : MonoBehaviour
    {
        ...
        /// <summary>
        /// If a projectile (or missile) hits the source ship that fired it, should it incur damage?
        /// </summary>
        public bool isDamageSource = false;
        ...
        /// <summary>
        /// The amount this projectile (or missile from SSC Expansion Pack 1), can penetrate a shield on impact.
        /// </summary>
        [Range(0f, 1f)] public float shieldPenetration = 0f;
        ...
        /// <summary>
        /// The estimated range (in metres) of this projectile or missile assuming it travels at a constant velocity.
        /// </summary>
        public float estimatedRange { get { return CalcRange(); } }

        /// <summary>
        /// A projectile can be Kinematic Guide to Target, or it can be a Missile (from Expansion Pack 1)
        /// </summary>
        public bool IsGuidedToTarget { get { return isKinematicGuideToTarget || IsMissile; } }

        /// <summary>
        /// Current velocity of the projectile or missile.
        ...
        /// <summary>
        /// The world space position of the projectile or missile in the current frame
        /// </summary>
        public Vector3 ThisFramePosition { get { return thisFramePosition; } internal set { thisFramePosition = value; } }

        /// <summary>
        /// The world space position of the projectile or missile in the last frame
        /// </summary>
        ...
        /// <summary>
        /// If a location is being targeted, will return its name.
        /// </summary>
        public string TargetLocationName { get { return targetLocation != null ? (string.IsNullOrEmpty(targetLocation.name) ? "no name" : targetLocation.name) : "-"; } }
        ...
        /// <summary>
        /// Get the local space velocity of the projectile or missile
        /// </summary>
        public virtual Vector3 LocalVelocity { get { return Quaternion.Inverse(transform.rotation) * velocity; } }

        /// <summary>
        /// Is this SSCMissileModule from SSC Expansion Pack 1?
        /// </summary>
        public virtual bool IsMissile { get { return false; } }
        ...
        /// <summary>
        /// From source (eg Ship), the magnitude of the acceleration (in metres per second squared) due to gravity.
        /// </summary>
        protected float gravitationalAcceleration;
        /// <summary>
        /// From source (eg Ship), the direction in world space in which gravity acts upon the ship.
        /// </summary>
        protected Vector3 gravityDirection;

        /// <summary>
        /// From the source (eg Ship), the density of the fluid that the projectile is travelling through.
        /// </summary>
        protected float mediumDensity;

        /// <summary>
        /// The current ship (if any) being targeted when isKinematicGuideToTarget is true,
        /// or is a missile from SSC Expansion Pack 1.
        /// </summary>
        [System.NonSerialized] protected ShipControlModule targetShip = null;

        /// <summary>
        /// The guidHash of the target. Currently only set for ship damage regions or
        /// is a missile from SSC Expansion Pack 1 and a location is targeted.
        /// </summary>
        protected int targetguidHash = 0;

        /// <summary>
        /// The current ship damage region (if any) being targeted when isKinematicGuideToTarget is true,
        /// or is a missile from SSC Expansion Pack 1.
        /// </summary>
        [System.NonSerialized] protected DamageRegion targetShipDamageRegion = null;

        /// <summary>
        /// The current location (if any) being targeted when (FUTURE) isKinematicGuideToTarget is true,
        /// or is a missile from SSC Expansion Pack 1.
        /// </summary>
        [System.NonSerialized] protected LocationData targetLocation = null;

        /// <summary>
        /// The current GameObject (if any) being targeted when isKinematicGuideToTarget is true,
        /// or is a missile from SSC Expansion Pack 1.
        /// </summary>
        [System.NonSerialized] protected GameObject targetGameObject = null;

        /// <summary>
        /// Is a location being targeted?
        /// </summary>
        protected bool isTargetLocation = false;

        /// <summary>
        /// Is a ship being targeted?
        /// </summary>
        protected bool isTargetShip = false;

        /// <summary>
        /// Is a ship's damage region being targeted?
        /// </summary>
        protected bool isTargetShipDamageRegion = false;
        ...
        /// </summary>
        [System.NonSerialized] protected SSCManager sscManager = null;

        // Is there a user-defined CallbackProjectileMoveTo configured for this projectile?
        protected bool isCallbackOnMoveTo = false;
        ...
        protected Vector3 lastFrameLOSN = Vector3.zero;
        protected Vector3 thisFrameLOSN = Vector3.zero;
        protected static readonly float NavConst = 3f;
        ...
        {
            // Avoid repeated property lookups as that may incur a small performance impact
            isMissile = IsMissile;
            ...
            // Medium (fluid) density is inherited from source object that fired it.
            this.mediumDensity = ipParms.mediumDensity;
        ...
        /// <summary>
        /// Calculate the (estimated) range of the projectile or missile
        /// </summary>
        /// <returns></returns>
        protected virtual float CalcRange()
        {
            return startSpeed * despawnTime;
        }
            ...
            if (isKinematicGuideToTarget && (isTargetShip || isTargetShipDamageRegion || targetGameObject != null))
            {
                ...
                else
                {
                    ...
                    if (lastFrameLOSN.x != 0f || lastFrameLOSN.y != 0f || lastFrameLOSN.z != 0f)
                    {
                        ...
                        //Debug.Log("[DEBUG] guided projectile angleRateLOS: " + angleRateLOS.ToString("0.0000") + " dist: " + (targetPosition - transform.position).magnitude);
                        ...
                        // NOTE: Final velocity should have the same magnitude (length) as the original velocity.
        ...
        /// <summary>
        /// Once an object has been hit, this will take the hitInfo and determine what to do with it.
        /// Use the local class-scoped hitInfo instead of passing in a copy of the struct.
        /// NOTE: hitInfo must be pre-populated before calling this method.
        /// If SSC Expansion Pack 1 is installed, missiles also call this method on collision.
        /// </summary>
        protected void ProcessHitInfo()
        {
            bool isShieldHit = false;
            ShipControlModule shipControlModule;

            // Do we need to check for ship shield hits?
            if (shieldEffectsObjectPrefabID >= 0 && CheckShipHit(hitInfo, damageAmount, damageType, sourceShipId, sourceSquadronId, projectilePrefabID, shieldPenetration, isDamageSource, out shipControlModule))
            {
                if (isDamageSource || shipControlModule.GetShipId != sourceShipId)
                {
            ...
            else if (shieldEffectsObjectPrefabID < 0 && CheckShipHit(hitInfo, damageAmount, damageType, sourceShipId, sourceSquadronId, projectilePrefabID, shieldPenetration, isDamageSource))
            {
        ...
        /// Currently does not support DOTS/ECS or kinematic guided projectiles
        /// </summary>
        /// <param name="locationData"></param>
        internal void SetTargetLocation (LocationData locationData)
        {
            if (!useECS && !isKinematicGuideToTarget)
            {
                isTargetShip = false;
                ...
                isTargetLocation = targetLocation != null;

                if (isTargetLocation)
                {
                    targetguidHash = locationData.guidHash;
                }
                else
                {
                    targetguidHash = 0;
        ...
        /// <param name="shieldPenetration">0 to 1.0 value</param>
        /// <param name="damageSource">Should source ship be damaged if hit by own projectile?</param>
        /// <returns></returns>
        public static bool CheckShipHit (RaycastHit raycastHitInfo, float projectileDamageAmount, DamageType projectileDamageType, 
            int sourceShipId, int sourceShipSquadronId, int projectilePrefabID, float shieldPenetration, bool damageSource)
        {
            bool isHit = false;
            ...
            if (hitRigidbody != null)
            {
                ...
                {
                    if (damageSource || shipControlModule.GetShipId != sourceShipId)
                    {
                        // Apply damage to the ship
                        shipControlModule.shipInstance.ApplyNormalDamage(projectileDamageAmount, projectileDamageType, raycastHitInfo.point, shieldPenetration);
        ...
        /// <summary>
        /// Determine if a ship has been hit. Apply damage as required.
        /// Typically called if a shield was hit.
        /// </summary>
        /// <param name="raycastHitInfo"></param>
        /// <param name="projectileDamageAmount"></param>
        /// <param name="projectileDamageType"></param>
        ...
        /// <returns></returns>
        public static bool CheckShipHit (RaycastHit raycastHitInfo, float projectileDamageAmount, DamageType projectileDamageType, 
            int sourceShipId, int sourceShipSquadronId, int projectilePrefabID, float shieldPenetration, bool damageSource, out ShipControlModule shipControlModule)
        {
            bool isHit = false;
            Rigidbody hitRigidbody = raycastHitInfo.rigidbody;
            ...
            int sourceShipId, int sourceShipSquadronId, int projectilePrefabID)
        {
            ...
            if (raycastHitInfo.collider != null)
            {
                ...
                if (raycastHitInfo.collider.TryGetComponent(out  damageReceiver) && damageReceiver.callbackOnHit != null)
                {
