...
namespace SciFiShipController
{
    ...
    public class BeamModule : MonoBehaviour
    {
        ...
        /// </summary>
        [Range(0.01f, 5f)] public float minBurstDuration = 0.5f;
        
        /// <summary>
        /// The maximum amount of time, in seconds, the beam can be active in a single burst
        /// </summary>
        [Range(0.02f, 30f)] public float maxBurstDuration = 5f;
        ...
        /// <summary>
        /// The amount of damage the beam will inflict on the target relative to the distance to the target.
        /// The maximum range is speed times the discharge duration.
        /// </summary>
        public AnimationCurve damageCurve = GetDefaultDamageCurve();
        ...
        public static SSCCurve.CurvePreset DefaultDamageCurvePreset = SSCCurve.CurvePreset.Constant;
        ...
        internal delegate void CallbackOnMove(BeamModule beamModule, float deltaTime);
        ...
        private void FixedUpdate()
        {
            if (isInitialised && isBeamEnabled)
            {
                ...
                if (callbackOnMove != null) { callbackOnMove(this, Time.deltaTime); }
            }
        }
        ...
        public static bool CheckShipHit (RaycastHit raycastHitInfo, BeamModule beamModule, float hitDuration)
        {
            ...
            if (hitRigidbody != null && beamModule != null)
            {
                ...
                {
                    float damageAmount = beamModule.GetDamageOverDistance(beamModule.damageRate * hitDuration, raycastHitInfo.distance);
            ...
            if (raycastHitInfo.collider != null)
            {
                ...
                if (damageReceiver != null && damageReceiver.callbackOnHit != null)
                {
                    ...
                    CallbackOnObjectHitParameters callbackOnObjectHitParameters = new CallbackOnObjectHitParameters
                    {
                        ...
                        projectilePrefab = null,
                        beamPrefab = sscManager.GetBeamPrefab(beamModule.beamPrefabID),
                        damageAmount = beamModule.GetDamageOverDistance(beamModule.damageRate * hitDuration, raycastHitInfo.distance),
                        sourceSquadronId = beamModule.sourceSquadronId
                    };
                    // Call the custom callback
        ...
        /// <summary>
        /// Get the default animation curve for the damage curve
        /// </summary>
        /// <returns></returns>
        public static AnimationCurve GetDefaultDamageCurve()
        {
            return SSCCurve.GetCurveFromPreset(DefaultDamageCurvePreset);
        }
        ...
        /// <summary>
        /// Given an amount of damage to apply, return the damage proportional to the maximum range (speed * discharge duration)
        /// based on the damage curve.
        /// </summary>
        /// <param name="damageAmount"></param>
        /// <param name="distance"></param>
        /// <returns></returns>
        public float GetDamageOverDistance (float damageAmount, float distance)
        {
            return speed > 0 && dischargeDuration > 0 ? damageAmount * damageCurve.Evaluate(distance / (speed * dischargeDuration)) : damageAmount;
        }
