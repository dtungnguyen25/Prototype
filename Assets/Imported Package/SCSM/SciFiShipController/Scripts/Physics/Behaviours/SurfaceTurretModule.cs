...
namespace SciFiShipController
{
    ...
    public class SurfaceTurretModule : MonoBehaviour
    {
        ...
        /// <summary>
        /// The unique model or type of surface turret. Values 1 to 100 are reserved for demo turrets. 0 = Not Set.
        /// </summary>
        ...
        /// <summary>
        /// If SSC Expansion Pack 1 is installed, the density (in kilograms per cubic metre) of the fluid the missile will travel through (usually air).
        /// </summary>
        public float mediumDensity = 1.293f;
        ...
        /// <summary>
        /// Attempt to get or set the Surface Turret's health
        /// </summary>
        public float Health { get { return isInitialised ? weapon.Health : weapon != null ? weapon.startingHealth : 0f; } set { SetHealth(value); } }
        ...
        public void Initialise()
        {
            ...
            if (!isInitialised)
            {
                ...
                if (weapon != null)
                {
                    ...
                    {
                        // If there are DamageReceiver components attached, setup the callback.
                        DamageReceiver.tempDamageReceiverList.Clear();

                        GetComponentsInChildren(true, DamageReceiver.tempDamageReceiverList);

                        for (int drIdx = 0; drIdx < DamageReceiver.tempDamageReceiverList.Count; drIdx++)
                        {
                            DamageReceiver damageReceiver = DamageReceiver.tempDamageReceiverList[drIdx];
        ...
        /// <summary>
        /// This gets called This gets called when the weapon fires and 
        /// from FixedUpdate in BeamModule. It performs the following:
        ...
        private void UpdateWeapon()
        {
            ...
            if (isInitialised && (weapon.projectilePrefabID >= 0 || weapon.beamPrefabID >= 0))
            {
                ...
                {
                    weapon.MoveTurret(worldVelocity, true, false);
                                    ...
                                    if (weaponTypeInt == Weapon.TurretBeamInt)
                                    {
                                        ...
                                        if (weapon.beamItemKeyList[fp].beamSequenceNumber == 0)
                                        {
                                            ...
                                            if (beamModule != null)
                                            {
                                                ...
                                                MoveBeam(beamModule, _deltaTime);
        ...
        /// <summary>
        /// Safely destroy the turret. If there is a destruction prefab, that can also be run.
        /// </summary>
        /// <param name="runDestruction"></param>
        public void DestroySurfaceTurret (bool runDestruction)
        {
            if (!IsDestroyed && callbackOnDestroy != null) { callbackOnDestroy.Invoke(this); }
            IsDestroyed = true;
            ...
            if (runDestruction && !isDestructionEffectsObjectInstantiated && effectsObjectPrefabID >= 0 && destructionEffectsObject != null && sscManager != null)
            {
            ...
            //if (isDestroyOnNoHealth)
            {
                ...
                if (!isDestructObjectActivated && destructObjectPrefabID >= 0 && destructObject != null && sscManager != null)
                {
                    ...
                    // Instantiate the launcher destruct prefab
        ...
        /// <summary>
        /// Attempt to set the healh between 0.0 (no health) and 100.0 (full health)
        /// </summary>
        /// <param name="newHealthLevel"></param>
        public void SetHealth (float newHealthLevel)
        {
            if (!isInitialised && weapon == null)
            {
                Debug.LogWarning("[ERROR] SurfaceTurretModule.SetHealth - could not set health on " + name + " as the weapon is null - PLEASE REPORT");
            }
            else
            {
                if (newHealthLevel < 0f) { weapon.Health = 0f; }
                else if (newHealthLevel > 100f) { weapon.Health = 100f; }
                else { weapon.Health = newHealthLevel; }
                ...
                if (isInitialised)
                {
                    if (weapon.Health <= 0f)
                    {
                        ...
                        if (isDestroyOnNoHealth) { DestroySurfaceTurret(true); }
                    }
                    else
                    {
                        // In case it previously had no health
                        IsDestroyed = false; 
                    }
                }
            } 
