...
namespace SciFiShipController
{
    ...
    public class SSCManager : MonoBehaviour
    {
        ...
        [System.NonSerialized] private static List<SSCManager> managerList = new List<SSCManager>(2);
        ...
        public void Initialise ()
        {
            ...
            for (int lIdx = 0; lIdx < numLocations; lIdx++)
            {
                ...
                {
                    if (sscRadar == null) { sscRadar = SSCRadar.GetOrCreateRadar(sceneHandle); }
                            ...
                            if (weapon.IsProjectileWeapon)
                            {
                                ...
                                else
                                {
                                    ...
                                    Debug.LogWarning("SSCManager UpdateProjectilesAndEffects() Warning: Projectile for a weapon (" 
                                        + weapon.name + ") was null. ShipId: " + shipInstance.shipId + " Ship Model Id: " + shipInstance.modelId + ". This weapon will not be able to fire projectiles.");
                                }
                                #endif
                            }
                        }

                        shipInstance.UpdateMaxAutoTargetRange();
                    }
                    #if UNITY_EDITOR
                    else
                    {
        ...
        /// <summary>
        /// Updates the DestructModule objects lists in the Ship Controller Manager given a DestructModule prefab.
        /// This is currently only used for SSCLauncherModule in SSC Expansion Pack 1 (if installed).
        /// </summary>
        /// <param name="destructObjectPrefab"></param>
        /// <param name="destructObjectPrefabID"></param>
        /// <param name="isDestructObjectActivated"></param>
        public void UpdateDestructObjects (DestructModule destructObjectPrefab, out int destructObjectPrefabID, out bool isDestructObjectActivated)
        {
            ...
            if (isInitialised)
            {
                if (destructObjectPrefab != null)
                {
                    // Get the transform instance ID for this destruct prefab
                    int destructTransformID = destructObjectPrefab.transform.GetInstanceID();
                    // Search the destruct templates list to see if we already have a destruct prefab with the same instance ID
                    int destructTemplateIndex = destructTemplateList.FindIndex(e => e.instanceID == destructTransformID);
                    ...
                    {
                        // If no match was found, create a new destruct template for this prefab
                        destructObjectPrefabID = AddDestructTemplate(destructObjectPrefab, destructTransformID);
                    }
                    else
                    {
                        // Save the destruct template index in the surface turret
                        destructObjectPrefabID = destructTemplateIndex;
                    }
                }
                else { destructObjectPrefabID = -1; }
            ...
            {
                // If the method is called before the manager is initialised, log a warning
                Debug.LogWarning("SSCManager UpdateDestructObjects(DestructModule destructObjectPrefab) Warning: Method was called before the manager was initialised.");
            }
            #endif
        }
        ...
        /// - The damage (destruction) effects object
        /// </summary>
        /// <param name="surfaceTurretModule"></param>
                ...
                if (surfaceTurretModule != null && surfaceTurretModule.weapon != null)
                {
                    ...
                    if (surfaceTurretModule.weapon.IsProjectileWeapon)
                    {
                        if (surfaceTurretModule.weapon.projectilePrefab != null)
                        {
                            ...
                            surfaceTurretModule.weapon.estimatedRange = surfaceTurretModule.weapon.projectilePrefab.estimatedRange;
                        }
                        #if UNITY_EDITOR
                        else
                        {
        ...
        /// <summary>
        /// Updates the effects objects lists in the Ship Controller Manager given a EffectsModule instance,
        /// and return the ID for the effects object prefab (template).
        /// Currently only used for SSCLauncherModule (if SSC Expansion Pack 1 is installed)
        /// </summary>
        /// <param name="effectsModule"></param>
        /// <param name="effectsObjectPrefabID"></param>
        public void UpdateEffects (EffectsModule effectsModule, out int effectsObjectPrefabID)
        {
            ...
            {
                // Get the transform instance ID for this effects object prefab
                int effectsObjectTransformID = effectsModule.transform.GetInstanceID();
                // Search the effects object templates list to see if we already have an 
                // effects object prefab with the same instance ID
                int effectsObjectTemplateIndex = effectsObjectTemplatesList.FindIndex(e => e.instanceID == effectsObjectTransformID);
                ...
                {
                    // If no match was found, create a new effects object template for this prefab
                    effectsObjectPrefabID = AddEffectsObjectTemplate(effectsModule, effectsObjectTransformID);
                }
                else
                {
                    // Update the effect template index to be returned
                    effectsObjectPrefabID = effectsObjectTemplateIndex;
                }
            }
        }
        ...
        private int AddProjectileTemplate (ProjectileModule projectilePrefab, int projectileTransformID)
        {
            ...
            if (projectileTemplate.projectilePrefab.usePooling)
            {
                ...
                for (int i = 0; i < projectileTemplate.currentPoolSize; i++)
                {
                    ...
                    #if SCSM_SSCXP1
                    if (_projectile.IsMissile)
                    {
                        SSCMissileModule _missileModule = (SSCMissileModule)_projectile;
                        _missileModule.ConfigureMissile();
                        _missileModule.Deactivate();
                    }
                    else
                    {
        ...
        internal void UpdateWeaponProjectileAndEffects(Weapon weapon)
        {
            ...
            if (weapon.projectilePrefab.IsMissile)
            {
                weapon.IsMissile = true;
            }
            else
            {
                // Only missiles from SSC Expansion Pack 1 support pausing turret movement after firing.
                weapon.IsMissile = false;
                weapon.turretMoveDelay = 0f;
            }
        }
        ...
        private static SSCManager AddSSCManager(int sceneHandle)
        {
                ...
                if (activeScene.handle != sceneHandle)
                {
                    ...
                    for (int sIdx = 0; sIdx < numScenes; sIdx++)
                    {
                        ...
                        if (_scene.handle == sceneHandle)
                        {
                            ...
                            // The workaround is to place the SSCManager in the scene at design time.                            
                            if (_scene.isLoaded)
                            {
                            ...
                            {
                                Debug.LogWarning("[ERROR] SSCManager.AddSSCManager() - scene " + _scene.name + " is NOT fully loaded. As a workaround place in SSCManager in the scene at design time.");
                            }
                            break;
                        }
                    }
            ...
            #if UNITY_EDITOR
            UnityEditor.Undo.SetCurrentGroupName("New SSCManager");
            ...
            #if UNITY_EDITOR
            UnityEditor.Undo.RegisterCreatedObjectUndo(newManagerGameObject, string.Empty);
            SSCManager _manager = UnityEditor.Undo.AddComponent(newManagerGameObject, typeof(SSCManager)) as SSCManager;
        ...
        private void OnDestroy()
        {
            ...
            if (projectileSystem != null)
            {
                //sscWorld.DestroySystemManaged(projectileSystem);
            ...
            DOTSHelper.DestroyWorld(ref sscWorld);
        ...
        {            
            ...
            #if DEBUG_SSCMANAGER
            Debug.Log("[DEBUG] GetOrCreateManager with sceneHandle: " + sceneHandle + " T:" + Time.time);
            #endif
            ...
            for (int mIdx = numberManagers-1; mIdx >= 0; mIdx--)
            {
                ...
                {
                    #if DEBUG_SSCMANAGER
                    Debug.Log("[DEBUG] GetOrCreateManager checking " + _tempManager.name + " in " + _tempManager.gameObject.scene.name + " sceneHandle " + _tempManager.sceneHandle + " T:" + Time.time);
                    #endif
                    ...
                    if (sceneHandle == 0 || _tempManager.sceneHandle == sceneHandle)
                    {
                        ...
                        #if DEBUG_SSCMANAGER
                        Debug.Log("[DEBUG] GetOrCreateManager Found matching " + _tempManager.name + " in " + _tempManager.gameObject.scene.name + " sceneHandle " + _tempManager.sceneHandle + " T:" + Time.time);
                        #endif
                ...
                else
                {
                    ...
                    #if DEBUG_SSCMANAGER
                    Debug.Log("[ERROR] GetOrCreateManager removed null manager at index " + mIdx + " T:" + Time.time);
                    #endif
                }
            }
            ...
            {
                #if DEBUG_SSCMANAGER
                Debug.Log("[ERROR] GetOrCreateManager ADDING NEW MANAGER T:" + Time.time);
                #endif
            ...
            {
                Debug.LogWarning("SSCManager GetOrCreateManager(" + sceneHandle + ") Warning: Could not find or create manager, so returned null.");
            }
            #endif
        ...
        #region Public Member Methods API - Instantiate Beam, Destruct, Projectile and Effects Objects
        ...
        public void InstantiateProjectile(int projectilePrefabID, Vector3 position, Vector3 fwdDirection, Vector3 upDirection, Vector3 weaponVelocity, float gravity, Vector3 gravityDirection, int shipId, int squadronId)
        {
            InstantiateProjectileParameters ipParms = new InstantiateProjectileParameters()
            {
                ...
                // aerodynamics are not supported with this older method
                mediumDensity = 0
            };
                ...
                if (ipParms.projectilePrefabID >= 0 && ipParms.projectilePrefabID < projectileTemplatesList.Count)
                {
                    ...
                    if (projectileTemplate.projectilePrefab.useECS)
                    {
                        ...
                        (
                            ...
                            projectileTemplate.projectilePrefab.shieldPenetration,
                        ...
                        {                           
                            ...
                            if (firstInactiveIndex == -1)
                            {
                                ...
                                if (projectileTemplate.currentPoolSize < projectileTemplate.projectilePrefab.maxPoolSize)
                                {
                                    ...
                                    if (_projectile.IsMissile)
                                    {
                                        // Pre-configure the new missile
                                        SSCMissileModule _missileModule = (SSCMissileModule)_projectile;
                                        _missileModule.ConfigureMissile();
                                    }
                                    #endif

                                    // Initialise the projectile (or missile)                                
        ...
        /// WARNING: This has not been performance optimised. Might need to check GC.
        /// and revise with slightly slower for-loop instead of list.Find().
        /// </summary>
        ...
        /// NOTE: When possible, use GetPath(guidHash) or GetPathByNumber(pathNumber).
        /// </summary>
        ...
        /// <summary>
        /// Get a path given the number in the list of Paths in the inspector.
        /// Numbers begin at 1.
        /// </summary>
        /// <param name="pathNumber"></param>
        /// <returns></returns>
        public PathData GetPathByNumber (int pathNumber)
        {
            int numPaths = pathDataList == null ? 0 : pathDataList.Count;

            if (numPaths > 0 && pathNumber > 0 && pathNumber <= numPaths)
            {
                return pathDataList[pathNumber - 1];
            }
            else
            {
                Debug.LogWarning("[ERROR] SSCManager.GetPathByNumber(" + pathNumber + ") - invalid path number");
                return null;
            }
        }
    ...
    public class ProjectileTemplate
    {
        ...
        public ProjectileTemplate (ProjectileModule prefab, int id)
        {
            ...
            #if SSC_ENTITIES
            #region Construct Entity Prefab and Archetype

            if (prefab.useECS)
            {
                // NOTE: ConvertGameObjectHierarchy has been REMOVED in ECS 1.0 for U2022.2
                ...
                entityManager.AddComponentData(projectilePrefabEntity, new LocalTransform { Rotation = Quaternion.identity, Scale = 1 });
                entityManager.AddComponentData(projectilePrefabEntity, new Projectile());
                entityManager.AddComponentData(projectilePrefabEntity, new Prefab());
                ...
                if (prefabRootTransform.childCount > 0)
                {
                    entityManager.AddComponentData(projectilePrefabEntity, new LocalToWorld());

                    NativeList<Entity> linkedEntityList = new NativeList<Entity>(prefabRootTransform.childCount, Allocator.Temp);

                    linkedEntityList.Add(projectilePrefabEntity);
                    ...
                    foreach (Transform childTfrm in prefabRootTransform)
                    {
                        ...
                        if (childTfrm.TryGetComponent(out mRen) && childTfrm.TryGetComponent(out mFilter))
                        {
                            ...
                            entityManager.AddComponentData(childEntity, new Parent() { Value = projectilePrefabEntity });
                            entityManager.AddComponentData(childEntity, new Prefab());
                            ...
                            if (!SSCUtils.IsUniformScale(mRen.transform.localScale))
                            {
                                #if UNITY_2022_3_OR_NEWER
                                // ECS 1.2.3
                                entityManager.AddComponentData(childEntity, new PostTransformMatrix { Value = float4x4.Scale(mRen.transform.localScale) });
                                #else
                                // ECS 1.0
                                //entityManager.AddComponentData(childEntity, new PostTransformScale { Value = float3x3.Scale(mRen.transform.localScale) });
                                #endif
                            }
                        }
                    }
                    ...
                    if (linkedEntityList.Length > 1)
                    {
                        ...
                        DynamicBuffer<LinkedEntityGroup> linkedEntityGroup = entityManager.AddBuffer<LinkedEntityGroup>(projectilePrefabEntity);
                    ...
                    if (linkedEntityList.IsCreated) { linkedEntityList.Dispose(); }
                }
                ...
                #if UNITY_2022_2_OR_NEWER
                ComponentType[] archetypeComponentTypeArray = new ComponentType[2];
                #else
    ...
    public struct InstantiateProjectileParameters
    {
        ...
        public int shipId;
        /// <summary>
        /// Squadron of the ship or weapon that fired the projectile, else -1.
        /// </summary>
        ...
        /// <summary>
        /// The unique guidHash for the target. Currently only used for ship damage regions or locations.
        /// If unset value is 0.
        /// </summary>
        ...
        /// <summary>
        /// The density of the fluid (usually air) the projectile will fly through.
        /// If Expansion Pack 1 is installed, this may affect missile flight aerodynamics.
        /// </summary>
    ...
    public struct InstantiateDestructParameters
    {
        ...
        /// <summary>
        /// The local-space velocity of the parent object when it was destroyed
        /// </summary>
