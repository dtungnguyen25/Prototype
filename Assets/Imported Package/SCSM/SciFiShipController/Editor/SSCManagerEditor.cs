...
namespace SciFiShipController
{
    ...
    public class SSCManagerEditor : Editor
    {
        ...
        private readonly static GUIContent optionGizmoScaleContent = new GUIContent("Gizmo Scale", "The relative scale of the gizmos in scene view.");
        ...
        {
            SSCManager mgr = SSCManager.GetOrCreateManager(SceneManager.GetActiveScene().handle);
                        ...
                        for (int lnIdx = 0; lnIdx < numLocations; lnIdx++)
                        {
                            ...
                            if (locationDataItem != null)
                            {
                                if (locationDataItem.showGizmosInSceneView)
                                {
                                    ...
                                    float itemHandleSize = (sscManager.isAutosizeLocationGizmo ? HandleUtility.GetHandleSize(itemHandlePosition) * 0.1f : 0.5f) * sscManager.gizmoScale;
                    ...
                    using (new Handles.DrawingScope(sscManager.defaultPathControlGizmoColour))
                    {
                        ...
                        if (pathData != null && pathData.showGizmosInSceneView)
                        {
                            ...
                            for (int pldIdx = 0; pldIdx < numPathLocationDatas; pldIdx++)
                            {
                                ...
                                {
                                    float controlHandleSize = (sscManager.isAutosizeLocationGizmo ? HandleUtility.GetHandleSize(pathLocationData.inControlPoint) * 0.07f : 0.4f) * sscManager.gizmoScale;
        ...
        public override void OnInspectorGUI()
        {
            ...
            else if (selectedTabInt == 2)
            {
                ...
                EditorGUILayout.PropertyField(gizmoScaleProp, optionGizmoScaleContent);
