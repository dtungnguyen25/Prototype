...
namespace SciFiShipController
{
    ...
    public class ShipCameraSettingsEditor : Editor
    {
        ...
        #region GUIContent - Special Purpose

        private readonly static GUIContent isSnapToTargetContent = new GUIContent("Snap to Target", "When applying these settings, snap the camera to the target location and rotation. NOTE: When false, this can still be overridden by the Ship Camera Changer with Always Snap to Target.");
        ...
        #region Serialized Properties - Special Purpose

        private SerializedProperty isSnapToTargetProp;
        #endregion
        ...
        private void OnEnable()
        {
            ...
            #region Find Properties - Special Purpose
            isSnapToTargetProp = serializedObject.FindProperty("isSnapToTarget");
            #endregion
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(isOrbitAvailableProp, ShipCameraModuleEditor.isOrbitAvailableContentSCM);
            if (EditorGUI.EndChangeCheck() && isOrbitEnabledProp.boolValue && !isOrbitAvailableProp.boolValue)
            {
            ...
            {
                EditorGUILayout.PropertyField(isOrbitEnabledProp, ShipCameraModuleEditor.isOrbitEnabledContentSCM);
            }

            EditorGUILayout.PropertyField(orbitDistanceProp, ShipCameraModuleEditor.orbitDistanceContentSCM);
            EditorGUILayout.PropertyField(orbitSpeedProp, ShipCameraModuleEditor.orbitSpeedContentSCM);
            EditorGUILayout.PropertyField(orbitHorizDampingProp, ShipCameraModuleEditor.orbitHorizDampingContentSCM);
            EditorGUILayout.PropertyField(orbitVertDampingProp, ShipCameraModuleEditor.orbitVertDampingContentSCM);
            EditorGUILayout.PropertyField(isOrbitGroundDetectionProp, ShipCameraModuleEditor.isOrbitGroundDetectionContentSCM);

            if (isOrbitGroundDetectionProp.boolValue)
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(ShipCameraModuleEditor.orbitGroundMaskContentSCM, GUILayout.Width(defaultEditorLabelWidth - 58f));
                if (GUILayout.Button(ShipCameraModuleEditor.resetBtnContentSCM, buttonCompact, GUILayout.MaxWidth(50f)))
                {
                ...
                EditorGUILayout.PropertyField(orbitGroundDetectOffsetProp, ShipCameraModuleEditor.orbitGroundDetectOffsetContentSCM);
            }
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            EditorGUILayout.PropertyField(isSnapToTargetProp, isSnapToTargetContent);
            EditorGUILayout.EndVertical();
            #endregion
