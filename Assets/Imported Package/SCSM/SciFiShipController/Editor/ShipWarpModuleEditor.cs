...
namespace SciFiShipController
{
    ...
    public class ShipWarpModuleEditor : Editor
    {
        ...
        protected readonly static GUIContent cpyBtnContent = new GUIContent("Cp", "Copy to clipboard");
        protected readonly static GUIContent newBtnContent = new GUIContent("New", "Get a new Id for this warp module");
        protected readonly static GUIContent guidHashContent = new GUIContent(" Warp Module Id", "When used with SSC X Pack 2, this needs to be unique across all scenes. If used from a prefab, update the ID after placing it in the scene.");
        #endregion
        ...
        protected readonly static GUIContent shipAddBoostOnDisengageContent = new GUIContent(" Add Boost on Disengage", "The amount of boost, in KNewtons, to be applied to the ship when warp is disengaged");
        protected readonly static GUIContent shipBoostRotOnDisengageContent = new GUIContent(" Boost Euler Angles", "The rotation Euler angles from forward flight to use when applying boost.");
        protected readonly static GUIContent shipBoostDurationOnDisengageContent = new GUIContent(" Boost Duration", "The time, in seconds, the boost will be applied for when warp is disengaged.");
        protected readonly static GUIContent isShipMoveOnDisengageContent = new GUIContent(" Movement on Disengage", "Ship movement will be enabled when warp is disengaged");
        #endregion

        #region GUIContent - Camera
        protected readonly static GUIContent cameraTypeContent = new GUIContent(" Camera Type", "The type of camera used during the warp");
        protected readonly static GUIContent nonSSCCameraContent = new GUIContent(" Camera", "The camera used if ship camera module is not in use.");
        ...
        protected readonly static GUIContent isUpdateClearFlagsContent = new GUIContent(" Update Clear Flags (BiRP)", "When using built-in render pipeline, should the module attempt to determine what the clear flags should be set to? Default: Off");
        protected readonly static GUIContent isSnapCameraOnEngageContent = new GUIContent(" Snap on Engage", "Attempt to snap the camera to the target position and rotation when the warp is engaged. Default: Off");
        ...
        protected readonly static GUIContent isSoundFXPausedContent = new GUIContent(" Pause Sound FX", "Are the sound effects currently paused. No new sounds will play until it is unpaused.");
        ...
        protected readonly static GUIContent isSoundIntervalRandomisedContent = new GUIContent(" Randomise Sound Volume", "Is the EffectsModule volume randomised?");
        protected readonly static GUIContent soundVolumeRangeContent = new GUIContent(" Volume Range", "The randomised percentage of the EffectsModule default volume");

        #endregion

        #region GUIContent - Events
        protected readonly static GUIContent onPostInitialisedEvtDelayContent = new GUIContent("  On Post Init Event Delay", "The number of seconds to delay firing the onPostInitialised event methods after the module has been initialised.");
        protected readonly static GUIContent onPostInitialisedContent = new GUIContent("On Post Initialised");
        ...
        #region Serialized Properties - Identification

        protected SerializedProperty guidHashProp;
        ...
        protected SerializedProperty shipAddBoostOnDisengageProp;
        protected SerializedProperty shipBoostRotOnDisengageProp;
        protected SerializedProperty shipBoostDurationOnDisengageProp;
        protected SerializedProperty isShipMoveOnDisengageProp;

        #endregion

        #region Serialized Properties - Camera
        protected SerializedProperty cameraTypeProp;
        protected SerializedProperty shipCameraModuleProp;
        protected SerializedProperty nonSSCCameraProp;
        protected SerializedProperty isUpdateClearFlagsProp;
        ...
        protected SerializedProperty isSnapCameraOnEngageProp;
        ...
        protected SerializedProperty minSoundVolumeProp;
        protected SerializedProperty maxSoundVolumeProp;

        #endregion

        #region Serialized Properties - Events
        protected SerializedProperty onPostInitialisedEvtDelayProp;
        protected SerializedProperty onPostInitialisedProp;
        ...
        protected virtual void OnEnable()
        {
            ...
            #region Find Properties - Identification

            guidHashProp = serializedObject.FindProperty("guidHash");
            ...
            shipAddBoostOnDisengageProp = serializedObject.FindProperty("shipAddBoostOnDisengage");
            shipBoostRotOnDisengageProp = serializedObject.FindProperty("shipBoostRotOnDisengage");
            shipBoostDurationOnDisengageProp = serializedObject.FindProperty("shipBoostDurationOnDisengage");
            isShipMoveOnDisengageProp = serializedObject.FindProperty("isShipMoveOnDisengage");
            #endregion

            #region Find Properties - Camera
            cameraTypeProp = serializedObject.FindProperty("cameraType");
            nonSSCCameraProp = serializedObject.FindProperty("nonSSCCamera");
            shipCameraModuleProp = serializedObject.FindProperty("shipCameraModule");
            isUpdateClearFlagsProp = serializedObject.FindProperty("isUpdateClearFlags");
            ...
            isSnapCameraOnEngageProp = serializedObject.FindProperty("isSnapCameraOnEngage");
            #endregion
            ...
            minSoundVolumeProp = serializedObject.FindProperty("minSoundVolume");
            maxSoundVolumeProp = serializedObject.FindProperty("maxSoundVolume");
            #endregion
        ...
        protected virtual void DrawCameraSettings()
        {
            ...
            EditorGUILayout.PropertyField(cameraTypeProp, cameraTypeContent);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(shipWarpModule, "Change Camera Type");
                serializedObject.ApplyModifiedProperties();
                shipWarpModule.SetCameraType((ShipWarpModule.SSCWarpCameraType)cameraTypeProp.intValue);
                serializedObject.Update();
            }

            if (cameraTypeProp.intValue == ShipWarpModule.CameraTypeSSCInt)
            {
                ...
                EditorGUILayout.PropertyField(isSnapCameraOnEngageProp, isSnapCameraOnEngageContent);
            }
            else if (cameraTypeProp.intValue == ShipWarpModule.CameraTypeUnityInt)
            {
                EditorGUILayout.PropertyField(nonSSCCameraProp, nonSSCCameraContent);
            }

            if (cameraTypeProp.intValue != ShipWarpModule.CameraTypeIgnoreInt)
            {
                EditorGUILayout.PropertyField(isUpdateClearFlagsProp, isUpdateClearFlagsContent);
            }
        ...
        /// </summary>
        protected virtual void DrawIdentificationSettings()
        {
            SSCEditorHelper.DrawUILine(separatorColor, 2, 6);

            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(guidHashContent, GUILayout.Width(defaultEditorLabelWidth-72f));

            if (GUILayout.Button(newBtnContent, GUILayout.Width(36)))
            {
                guidHashProp.intValue = SSCMath.GetHashCodeFromGuid();
            }

            if (GUILayout.Button(cpyBtnContent, GUILayout.Width(28)))
            {
                EditorGUIUtility.systemCopyBuffer = guidHashProp.intValue.ToString();
            }

            EditorGUILayout.LabelField(guidHashProp.intValue.ToString());
            EditorGUILayout.EndHorizontal();
        }
        ...
        protected virtual void DrawShipSettings()
        {
            ...
            EditorGUILayout.PropertyField(isShipMoveOnDisengageProp, isShipMoveOnDisengageContent);
            EditorGUILayout.PropertyField(shipAddBoostOnDisengageProp, shipAddBoostOnDisengageContent);
            if (shipAddBoostOnDisengageProp.floatValue >0f)
            {
                EditorGUILayout.PropertyField(shipBoostRotOnDisengageProp, shipBoostRotOnDisengageContent);
                EditorGUILayout.PropertyField(shipBoostDurationOnDisengageProp, shipBoostDurationOnDisengageContent);
            }
            #endregion
        }
        ...
        protected virtual void DrawSoundFXSettings()
        {
            ...
            if (isSoundIntervalRandomisedProp.boolValue)
            {
                SSCEditorHelper.DrawSSCMinMaxSlider(minSoundVolumeProp, maxSoundVolumeProp, soundVolumeRangeContent, 0f, 100f, defaultEditorLabelWidth);
            }
        }
        ...
        /// Only applies when mouse is over the inspector.
        /// </summary>
        /// <returns></returns>
