...
namespace SciFiShipController
{
    ...
    public class ShipControlModuleEditor : Editor
    {
        ...
        private bool debugIsShowAero = false;
        ...
        private readonly static GUIContent isCheckMomentValidContent = new GUIContent(" Check Valid Moment", "In some custom configurations " +
            "the moment may return NaN. Enabling this will reset the moment to zero to avoid errors. Do not enable unless you are getting NaN errors.");
        ...
        private readonly static GUIContent counteractGravityContent = new GUIContent(" Counteract Gravity", " The ship will glide or hover " +
            "when thrust is not being applied. When disabled, you will need to add a wing to provide lift.");
        ...
        private readonly static GUIContent timingHeaderContent = new GUIContent("<b>Timing</b>\n\nManual timing supports custom update rates like when using a 3rd party rigidbody.");
        private readonly static GUIContent timingPhysicsLoopContent = new GUIContent(" Physics Timing", "The timing method used with the physics update loop");
        private readonly static GUIContent timingUpdateLoopContent = new GUIContent(" Update Timing", "The timing method used with the update loop");
        private readonly static GUIContent moveDataUpdateContent = new GUIContent(" Move Data Update", "The way cached move data is updated");
        #endregion
        ...
        private readonly static GUIContent pitchDampingContent = new GUIContent(" Pitch Damping", "Controls how much damping is used in pitch control to prevent overshoot of the target pitch to match. Increase this value if the ship oscillates (moves back and forth) near the target pitch angle. (Default 1.0).");
        ...
        private readonly static GUIContent rollDampingContent = new GUIContent(" Roll Damping", "Controls how much damping is used in roll control to prevent overshoot of the target roll to match. Increase this value if the ship oscillates (moves back and forth) near the target roll angle (Default 1.0).");
        ...
        private readonly static GUIContent autoCentrePitchContent = new GUIContent(" Auto Centre Pitch", "When there is no pitch input, pitch will return to a neutral position");
        ...
        private readonly static GUIContent maxGroundMatchAccelerationFactorContent = new GUIContent(" Max Acceleration", "The limit to " +
            ...
            "ship to match the Target Distance more closely but may look less natural. Keep the value as low as possible.");
        ...
        private readonly static GUIContent weaponIsManualRechargeContent = new GUIContent("Manual Recharge", "Does the beam weapon need to be manually recharged in code by setting the chargeAmount to a value between 0.0 and 1.0?");
        private readonly static GUIContent weaponFiringButtonContent = new GUIContent("Firing Button", "The firing button or mechanism to use for this weapon. Auto-fire only works with Turrets when a target is selected. Fixed projectile weapons also support Auto-Fire if they have a Missile assigned to them from SSC Expansion Pack 1 (if installed).");
        ...
        #if SCSM_SSCXP1
        private readonly static GUIContent weaponTurretMoveDelayContent = new GUIContent("Turret Move Delay", "The time, in seconds, the turret will stop rotating after it has fired. This can be used to give the missile(s) a chance to clear the turret.");
        private readonly static GUIContent weaponCheckLineOfSightContent = new GUIContent("Check Line Of Sight", "Whether the weapon checks line of sight before firing (in order to prevent friendly fire) each frame. Since this uses raycasts it can lead to reduced performance. Currently has no effect if the target is a Location.");
        #else
        private readonly static GUIContent weaponCheckLineOfSightContent = new GUIContent("Check Line Of Sight", "Whether the weapon checks line of sight before firing (in order to prevent friendly fire) each frame. Since this uses raycasts it can lead to reduced performance.");
        #endif
        ...
        private readonly static GUIContent modelIdContent = new GUIContent("Model Id", "The unique model or type of ship. Values 1 to 100 are reserved for demo ships. 0 = Not Set.");
        ...
        private readonly static GUIContent debugShipIdContent = new GUIContent("Ship Id");
        ...
        private readonly static GUIContent debugIsShowAeroContent = new GUIContent("Show Aerodynamics");
        private readonly static GUIContent debugAeroRefAreasContent = new GUIContent("  Drag Area (m2)");
        ...
        private readonly static GUIContent debugWeaponEstRangeContent = new GUIContent("   Range (Estimated)");
        private readonly static GUIContent debugWeaponIsAutoTargetingContent = new GUIContent("   Auto Targeting");
        private readonly static GUIContent debugWeaponIsMissileContent = new GUIContent("   Missile");
        ...
        private SerializedProperty timingPhysicsLoopProp;
        private SerializedProperty timingUpdateLoopProp;
        private SerializedProperty moveDataUpdateProp;
        ...
        private SerializedProperty weaponIsManualRechargeProp;
        ...
        private void SceneGUI (SceneView sv)
        {
            if (shipControlModule != null && shipControlModule.gameObject.activeInHierarchy)
            {
                ...
                bool isPhysicsModel = shipControlModule.shipInstance.shipPhysicsModel == Ship.ShipPhysicsModel.PhysicsBased;

                float gizmoScale = shipControlModule.gizmoScale;
                ...
                using (new Handles.DrawingScope(Color.grey))
                {
                    ...
                    if (shipControlModule.shipInstance.showCOMGizmosInSceneView)
                    {
                        ...
                        {
                            Handles.ArrowHandleCap(0, centreOfMassHandlePosition, centreOfMassHandleRotation, 2f*gizmoScale, EventType.Repaint);
                        }
                        ...
                        if (shipControlModule.shipInstance.setCentreOfMassManually)
                        {
                            ...
                            if (Handles.Button(centreOfMassHandlePosition, Quaternion.identity, 1f * gizmoScale, 0.5f * gizmoScale, Handles.SphereHandleCap))
                            {
                                if (isCentreOfMassSelected)
                                {
                                    DeselectAllComponents();
                        ...
                        else
                        {
                            ...
                            if (Event.current.type == EventType.Repaint)
                            {
                                ...
                                Handles.SphereHandleCap(0, centreOfMassHandlePosition, Quaternion.identity, 1f * gizmoScale, EventType.Repaint);
                            }
                        }
                    }
                    #endregion
                ...
                using (new Handles.DrawingScope(Color.cyan))
                {
                        ...
                        if (wingComponent.showGizmosInSceneView)
                        {
                            ...
                            if (wingComponent.selectedInSceneView)
                            {
                                ...
                                else if (Tools.current == Tool.Scale)
                                {
                                    ...
                                    componentHandleScale = Handles.ScaleHandle(componentHandleScale, componentHandlePosition, sceneViewShipRotation, HandleUtility.GetHandleSize(componentHandlePosition) * gizmoScale );
                    ...
                    if (shipControlModule.shipInstance.showCOLGizmosInSceneView && Event.current.type == EventType.Repaint)
                    {
                        ...
                        {
                            Handles.SphereHandleCap(0, centreOfLiftHandlePosition, Quaternion.identity, 0.75f * gizmoScale, EventType.Repaint);
                            Handles.ArrowHandleCap(0, centreOfLiftHandlePosition, Quaternion.LookRotation(directionOfLiftHandleVector), 2f * gizmoScale, EventType.Repaint);
                            Handles.DrawDottedLine(centreOfLiftHandlePosition - (directionOfLiftHandleVector * 100f),
                                centreOfLiftHandlePosition + (directionOfLiftHandleVector * 100f), 4f * gizmoScale);
                        }
                    }
                ...
                using (new Handles.DrawingScope(weaponGizmoColour))
                {
                    for (int wpi = 0; wpi < numWeapons; wpi++)
                    {
                        ...
                        if (weaponComponent.showGizmosInSceneView)
                        {
                            ...
                            handleDistanceScale = HandleUtility.GetHandleSize(componentHandlePosition) * gizmoScale;
                                    ...
                                    for (int fposIdx = 0; fposIdx < numCompWeaponFirePositions; fposIdx++)
                                    {
                                        ...
                                        if (weaponComponent.weaponType == Weapon.WeaponType.TurretProjectile && weaponComponent.turretPivotX != null)
                                        {
                                            Handles.SphereHandleCap(0, ((shipControlModule.transform.rotation *
                                                        ...
                                                        shipControlModule.transform.position, componentHandleRotation, 0.2f * gizmoScale, EventType.Repaint);
                                        ...
                                        else
                                        {
                                            ...
                                            // v1.1.9+
                                            Handles.SphereHandleCap(0, weaponWorldFirePosition, componentHandleRotation, 0.2f * gizmoScale, EventType.Repaint);
                                        }
                                    }
                                }
                        ...
                        if (damageRegionCommponent.showGizmosInSceneView)
                        {
                            ...
                            if (damageRegionCommponent.selectedInSceneView)
                            {
                                ...
                                else if (Tools.current == Tool.Scale)
                                {
                                    ...
                                    Vector3 scale = Handles.ScaleHandle(damageRegionCommponent.size, componentHandlePosition, componentHandleRotation, HandleUtility.GetHandleSize(componentHandlePosition) * gizmoScale);
                ...
                using (new Handles.DrawingScope(Color.magenta))
                {
                    ...
                    for (int ti = 0; ti < numThrusters; ti++)
                    {
                        ...
                        if (thrusterComponent.showGizmosInSceneView)
                        {
                            ...
                            if (Event.current.type == EventType.Repaint)
                            {
                                Handles.ArrowHandleCap(0, gizmoPosition, componentHandleRotation, 1f * gizmoScale, EventType.Repaint);
                            }
                            ...
                            if (Handles.Button(gizmoPosition, Quaternion.identity, 0.5f * gizmoScale, 0.25f * gizmoScale, Handles.SphereHandleCap))
                            {
                                if (thrusterComponent.selectedInSceneView)
                                {
                                    DeselectAllComponents();
                    ...
                    if (shipControlModule.shipInstance.showCOTGizmosInSceneView && Event.current.type == EventType.Repaint)
                    {
                        ...
                        {
                            Handles.SphereHandleCap(0, centreOfThrustHandlePosition, Quaternion.identity, 0.5f * gizmoScale, EventType.Repaint);
                            Handles.ArrowHandleCap(0, centreOfThrustHandlePosition, Quaternion.LookRotation(-directionOfThrustHandleVector), 2f * gizmoScale, EventType.Repaint);
                            Handles.DrawDottedLine(centreOfThrustHandlePosition - (directionOfThrustHandleVector * 100f),
                                centreOfThrustHandlePosition + (directionOfThrustHandleVector * 100f), 4f * gizmoScale);
                        }
                    }
                ...
                using (new Handles.DrawingScope(Color.red))
                {
                    ...
                    if (Event.current.type == EventType.Repaint)
                    {
                        Handles.SphereHandleCap(0, shipControlModule.transform.TransformPoint(shipControlModule.shipInstance.centreOfDragXMoment), Quaternion.identity, 0.25f * gizmoScale, EventType.Repaint);
                    }
                }
                ...
                using (new Handles.DrawingScope(Color.green))
                {
                    ...
                    if (Event.current.type == EventType.Repaint)
                    {
                        Handles.SphereHandleCap(0, shipControlModule.transform.TransformPoint(shipControlModule.shipInstance.centreOfDragYMoment), Quaternion.identity, 0.25f * gizmoScale, EventType.Repaint);
                    }
                }
                ...
                using (new Handles.DrawingScope(Color.blue))
                {
                    ...
                    if (Event.current.type == EventType.Repaint)
                    {
                        Handles.SphereHandleCap(0, shipControlModule.transform.TransformPoint(shipControlModule.shipInstance.centreOfDragZMoment), Quaternion.identity, 0.25f * gizmoScale, EventType.Repaint);
                    }
                }
                #endif
                #endregion
        ...
        {
            if (GUILayout.Button(SSCEditorHelper.gizmoFindBtnContent, buttonCompact, GUILayout.MaxWidth(20f)))
            {
        ...
        protected void CreateDamageMenuArray()
        {
        ...
        /// <summary>
        /// Draw the aerodynamic settings in the inspector
        /// </summary>
        /// <param name="isArcadeModel"></param>
        /// <param name="isPhysicsModel"></param>
        protected virtual void DrawAeroDynamicsSettings(bool isArcadeModel, bool isPhysicsModel)
        {
            ...
            if (GUILayout.Button(calculateDragPropertiesButtonContent))
            {
                ...
                if (allShipMeshRenderers.Length > 0)
                {
                    ...
                    int numOrigShipCols = originalShipColliders.Length;

                    bool[] origShipColsEnabled = new bool[numOrigShipCols];

                    for (ci = 0; ci < numOrigShipCols; ci++)
                    {
                        // Store the original state of the collider before disabling it
                        origShipColsEnabled[ci] = originalShipColliders[ci].enabled;
                        // If enabled, disable it now
                        if (origShipColsEnabled[ci])
                        {
                    ...
                    for (ci = 0; ci < numOrigShipCols; ci++)
                    {
                        // If it was enabled, re-enable it now
                        if (origShipColsEnabled[ci])
                        {
                    ...
                    // If required, force the cached drag variabled to be recalculated
                    if (Application.isPlaying) { shipControlModule.shipInstance.DirtyDragCache(); }
                }
                else
                {
            ...
            if (isArcadeModel)
            {
                ...
                {
                    // If required, force the cached drag variabled to be recalculated
                    shipControlModule.shipInstance.DirtyDragCache();
                }
            }
        ...
        /// <summary>
        /// Draw combat settings in the inspector
        /// </summary>
        protected virtual void DrawCombatSettings()
        {
            ...
            for (index = 0; index < numWeapons; index++)
            {
                ...
                weaponIsManualRechargeProp = weaponProp.FindPropertyRelative("isManualRecharge");
                ...
                if (weaponShowInEditorProp.boolValue)
                {
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        {
                            bool isMissile = weaponTypeProp.intValue == Weapon.FixedProjectileInt && weaponProjectilePrefabProp.objectReferenceValue != null && ((ProjectileModule)weaponProjectilePrefabProp.objectReferenceValue).IsMissile;

                            if (!isMissile)
                            {
                                weaponFiringButtonProp.intValue = 0;
                                Debug.LogWarning("Only fixed projectile weapons with Missiles (SSC Expansion Pack 1), or turrets support auto-fire");
                            }
                        }

                        if (weaponFiringButtonProp.intValue == Weapon.FiringButtonManualInt)
                        {
                            shipControlModule.SetWeaponManualFire(index + 1);
                        }
                    }
                    ...
                    if (weaponTypeProp.intValue == Weapon.FixedBeamInt || weaponTypeProp.intValue == Weapon.TurretBeamInt)
                    {
                        ...
                        EditorGUILayout.PropertyField(weaponIsManualRechargeProp, weaponIsManualRechargeContent);
                    }
                    else
                    {
...
#if SCSM_SSCXP1
                        if (shipControlModule.shipInstance.weaponList[index].IsMissile)
                        {
                            EditorGUILayout.PropertyField(weaponProp.FindPropertyRelative("turretMoveDelay"), weaponTurretMoveDelayContent);
                        }
#endif
                    ...
                    {
                        if (weaponProjectilePrefabProp.objectReferenceValue != null && ((ProjectileModule)weaponProjectilePrefabProp.objectReferenceValue).IsGuidedToTarget)
                        {
                            EditorGUILayout.PropertyField(weaponIsAutoTargetingEnabledProp, weaponIsAutoTargetingEnabledContent);
                        }
            ...
            EditorGUILayout.PropertyField(shipInstanceProp.FindPropertyRelative("modelId"), modelIdContent);
        ...
        /// <summary>
        /// Draw the Control settings in the inspector
        /// </summary>
        /// <param name="isArcadeModel"></param>
        /// <param name="isPhysicsModel"></param>
        protected virtual void DrawControlSettings(bool isArcadeModel, bool isPhysicsModel)
        {
            ...
            if (limitPitchAndRollProp.boolValue)
            {
                ...
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(shipInstanceProp.FindPropertyRelative("pitchDamping"), pitchDampingContent);
                if (EditorGUI.EndChangeCheck() && EditorApplication.isPlaying)
                {
                    shipControlModule.shipInstance.ReinitialisePitchRollMatchVariables();
                }
                ...
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(shipInstanceProp.FindPropertyRelative("rollDamping"), rollDampingContent);
                if (EditorGUI.EndChangeCheck() && EditorApplication.isPlaying)
                {
                    shipControlModule.shipInstance.ReinitialisePitchRollMatchVariables();
                }
                ...
                EditorGUILayout.PropertyField(shipInstanceProp.FindPropertyRelative("autoCentrePitch"), autoCentrePitchContent);
                ...
                if (stickToGroundSurfaceProp.boolValue || avoidGroundSurfaceProp.boolValue)
                {
                    ...
                    if (stickToGroundSurfaceProp.boolValue)
                    {
                        ...
                        if (isArcadeModel && useGroundMatchSmoothingProp.boolValue)
                        {
        ...
        /// <summary>
        /// Draw Physics setttings in the insepector
        /// </summary>
        /// <param name="isArcadeModel"></param>
        protected virtual void DrawPhysicsSettings(bool isArcadeModel)
        {
            if (shipInstanceProp != null)
            {
                ...
                EditorGUILayout.PropertyField(gizmoScaleProp, SSCEditorHelper.gizmoScaleContentIndent);
                ...
                EditorGUILayout.BeginHorizontal();
                bool isManualCentreOfMass = manualCentreOfMassProp.boolValue;
                ...
                if (manualCentreOfMassProp.boolValue)
                {
                    ...
                    if (GUILayout.Button(resetCentreOfMassButtonContent))
                    {
                        ...
                        if (!shipControlModule.TryGetComponent(out shipRigidbody)) { Debug.LogWarning("ERROR: The RigidBody is missing on " + shipControlModule.name + ". Cannot reset the Centre of Mass."); }
                        else
                        {
                            ...
                            centreOfMassProp.vector3Value = shipRigidbody.centerOfMass;
                                ...
                                shipRigidbody.automaticCenterOfMass = !manualCentreOfMassProp.boolValue;
#endif
                ...
                {
                    // Reset the centre of mass to what is calculated by colliders
                    // v1.4.5 Fix not fetching rbody every inspector refresh
                    if (shipRigidbody != null || shipControlModule.TryGetComponent(out shipRigidbody))
                    {
                        Vector3 _centreOfMass = centreOfMassProp.vector3Value;

                        /// TODO - fix bug in that this shouldn't run every 10 frames while mouse is (moving?) over inspector
                        //Debug.Log("[DEBUG] ResetCenterOfMass T:" + Time.time);
                        shipRigidbody.ResetCenterOfMass();

                        // #SMS v1.4.5 reduce impact of constant update by only changing property when required
                        if (shipRigidbody.centerOfMass != _centreOfMass)
                        {
                            centreOfMassProp.vector3Value = shipRigidbody.centerOfMass;
                ...
                // If changing manual set centre of mass, update the rigidbody with the change in 2022.2+
                if (manualCentreOfMassProp.boolValue != isManualCentreOfMass)
                {
#if UNITY_2022_2_OR_NEWER
                        ...
                        if (shipRigidbody != null)
                        {
                            ...
                            if (manualCentreOfMassProp.boolValue)
                            {
                                shipRigidbody.centerOfMass = centreOfMassProp.vector3Value;
                ...
                EditorGUILayout.PropertyField(shipInstanceProp.FindPropertyRelative("isCheckMomentValid"), isCheckMomentValidContent);
                ...
                if (isArcadeModel)
                {
                    EditorGUILayout.PropertyField(shipInstanceProp.FindPropertyRelative("counteractGravity"), counteractGravityContent);
                ...
                #region Timing and cached Move Data

                timingPhysicsLoopProp = shipInstanceProp.FindPropertyRelative("timingPhysicsLoop");
                timingUpdateLoopProp = shipInstanceProp.FindPropertyRelative("timingUpdateLoop");
                moveDataUpdateProp = shipInstanceProp.FindPropertyRelative("moveDataUpdate");

                EditorGUILayout.LabelField(timingHeaderContent, helpBoxRichText);

                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(timingPhysicsLoopProp, timingPhysicsLoopContent);
                if (EditorGUI.EndChangeCheck())
                {
                    shipControlModule.shipInstance.SetTimingPhysicsLoop((Ship.SSCTiming)timingPhysicsLoopProp.intValue);
                }

                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(timingUpdateLoopProp, timingUpdateLoopContent);
                if (EditorGUI.EndChangeCheck())
                {
                    shipControlModule.shipInstance.SetTimingUpdateLoop((Ship.SSCTiming)timingUpdateLoopProp.intValue);
                }

                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(moveDataUpdateProp, moveDataUpdateContent);
                if (EditorGUI.EndChangeCheck())
                {
                    shipControlModule.shipInstance.SetMoveDataUpdate((Ship.SSCMoveDataUpdate)moveDataUpdateProp.intValue);
                }
        ...
        /// <summary>
        /// Draw the thruster settings in the inspector
        /// </summary>
        /// <param name="isArcadeModel"></param>
        /// <param name="isPhysicsModel"></param>
        protected virtual void DrawThrusterSettings(bool isArcadeModel, bool isPhysicsModel)
        {
            if (isPhysicsModel)
            {
        ...
        /// <summary>
        /// Draw debug settings in the inspector
        /// </summary>
        protected virtual void DrawDebugSettings()
        {
            ...
            if (isDebuggingEnabled && shipControlModule != null)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(debugShipIdContent, labelFieldRichText, GUILayout.Width(defaultEditorLabelWidth));
                EditorGUILayout.LabelField((isShipInitialised ? shipControlModule.GetShipId.ToString() : "-"), GUILayout.MaxWidth(rightLabelWidth));
                EditorGUILayout.EndHorizontal();
                ...
                #region Debugging - Aero
                debugIsShowAero = EditorGUILayout.Toggle(debugIsShowAeroContent, debugIsShowAero);

                if (debugIsShowAero)
                {
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(debugAeroRefAreasContent, labelFieldRichText, GUILayout.Width(defaultEditorLabelWidth));
                    EditorGUILayout.LabelField(isShipInitialised ? SSCEditorHelper.GetVector3Text(shipControlModule.shipInstance.dragReferenceAreas, 1) : "---", GUILayout.MaxWidth(rightLabelWidth));
                    EditorGUILayout.EndHorizontal();
                }
                    ...
                    if (numWeapons > 0)
                    {
                        for (int wpIdx = 0; wpIdx < numWeapons; wpIdx++)
                        {
                            ...
                            if (weapon != null)
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                EditorGUILayout.LabelField(debugWeaponEstRangeContent, GUILayout.Width(defaultEditorLabelWidth - 3f));
                                EditorGUILayout.LabelField(SSCEditorHelper.GetFloatText(weapon.estimatedRange, 1));
                                EditorGUILayout.EndHorizontal();

                                EditorGUILayout.BeginHorizontal();
                                EditorGUILayout.LabelField(debugWeaponIsAutoTargetingContent, GUILayout.Width(defaultEditorLabelWidth - 3f));
                                EditorGUILayout.LabelField(weapon.isAutoTargetingEnabled ? "Yes" : "No");
                                EditorGUILayout.EndHorizontal();

                                EditorGUILayout.BeginHorizontal();
                                EditorGUILayout.LabelField(debugWeaponIsMissileContent, GUILayout.Width(defaultEditorLabelWidth - 3f));
#if SCSM_SSCXP1
                                EditorGUILayout.LabelField(weapon.IsMissile ? "Yes" : "No");
#else
                                EditorGUILayout.LabelField("No (Requires SSC Expansion Pack 1)");
#endif
                                EditorGUILayout.EndHorizontal();
                                ...
                                if (weapon.HasTarget)
                                {
                                    ShipControlModule weaponShipTarget = weapon.GetTargetShip();
                                    DamageRegion weaponDRTarget = weapon.GetTargetShipDamageRegion();
                                    GameObject weaponTarget = weapon.GetTarget();
                                    LocationData locationTarget = weapon.GetTargetLocation();

                                    EditorGUILayout.BeginHorizontal();
                                    EditorGUILayout.LabelField(debugTargetShipContent, labelFieldRichText, GUILayout.Width(defaultEditorLabelWidth - 3f));
                                    EditorGUILayout.LabelField(weaponShipTarget == null ? "---" : weaponShipTarget.name, GUILayout.MaxWidth(rightLabelWidth));
                                    EditorGUILayout.EndHorizontal();
                                    ...
                                    EditorGUILayout.LabelField(weaponDRTarget == null ? "---" : weaponDRTarget.name, GUILayout.MaxWidth(rightLabelWidth));
                                    EditorGUILayout.EndHorizontal();

                                    EditorGUILayout.BeginHorizontal();
                                    EditorGUILayout.LabelField(debugTargetGameObjectContent, labelFieldRichText, GUILayout.Width(defaultEditorLabelWidth - 3f));
                                    EditorGUILayout.LabelField(weaponTarget == null ? "---" : weaponTarget.name, GUILayout.MaxWidth(rightLabelWidth));
                                    EditorGUILayout.EndHorizontal();

                                    EditorGUILayout.BeginHorizontal();
                                    EditorGUILayout.LabelField(debugTargetLocationContent, labelFieldRichText, GUILayout.Width(defaultEditorLabelWidth - 3f));
                                    EditorGUILayout.LabelField(locationTarget == null ? "---" : locationTarget.name, GUILayout.MaxWidth(rightLabelWidth));
                                    EditorGUILayout.EndHorizontal();
                                }
                                else
                                {
                                    EditorGUILayout.BeginHorizontal();
                                    EditorGUILayout.LabelField(debugTargetShipContent, labelFieldRichText, GUILayout.Width(defaultEditorLabelWidth - 3f));
                                    EditorGUILayout.LabelField("---", GUILayout.MaxWidth(rightLabelWidth));
                                    EditorGUILayout.EndHorizontal();
        ...
        protected virtual void DrawTabs()
        {
            ...
            bool isArcadeModel = shipControlModule.shipInstance.shipPhysicsModel == Ship.ShipPhysicsModel.Arcade;
            bool isPhysicsModel = shipControlModule.shipInstance.shipPhysicsModel == Ship.ShipPhysicsModel.PhysicsBased;
            ...
            {
                DrawPhysicsSettings(isArcadeModel);
            }
            ...
            {
                DrawControlSettings(isArcadeModel, isPhysicsModel);
            }
            ...
            {
                DrawThrusterSettings(isArcadeModel, isPhysicsModel);
            }
            ...
            {
                DrawAeroDynamicsSettings(isArcadeModel, isPhysicsModel);
            }
