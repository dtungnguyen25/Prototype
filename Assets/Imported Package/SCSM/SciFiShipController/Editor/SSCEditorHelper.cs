...
namespace SciFiShipController
{
    public class SSCEditorHelper
    {
        ...
        /// <summary>
        /// Output a message if Expansion Pack 1 is not installed in the project.
        /// </summary>
        public static void SSCXPack1NotInstalled()
        {
            EditorGUILayout.HelpBox("SSC Expansion Pack 1 is not installed. It is available from the Unity Asset Store", MessageType.Error);
        }
        ...
        public readonly static GUIContent gizmoScaleContentIndent = new GUIContent(" Gizmo Scale", "Scale the gizmos in the scene view.");
        public readonly static GUIContent gizmoScaleContent = new GUIContent("Gizmo Scale", "Scale the gizmos in the scene view.");
        public readonly static GUIContent btnResetContent = new GUIContent("R", "Reset to default value(s)");
        public readonly static GUIContent gotoEffectFolderBtnContent = new GUIContent("F", "Find and highlight the sample Effects folder");
        public readonly static GUIContent gotoDestructsFolderBtnContent = new GUIContent("F", "Find and highlight the sample Destructs folder");
        public readonly static GUIContent gotoProjectileFolderBtnContent = new GUIContent("F", "Find and highlight the sample Projectiles folder");
        public readonly static GUIContent gotoBeamFolderBtnContent = new GUIContent("F", "Find and highlight the sample Beam folder");
        ...
        /// <summary>
        /// Draw a curve with a preceeding preset selection list and a default curve reset option.
        /// Returns true if the curve was reset.
        /// </summary>
        /// <param name="curveProperty"></param>
        /// <param name="curveContent"></param>
        /// <param name="defaultPreset"></param>
        /// <param name="buttonCompact"></param>
        /// <param name="defaultEditorLabelWidth"></param>
        /// <returns></returns>
        public static bool DrawSSCCurveWithPreset(SerializedProperty curveProperty, GUIContent curveContent, SSCCurve.CurvePreset defaultPreset, GUIStyle buttonCompact, float defaultEditorLabelWidth)
        {
            bool wasReset = false;

            EditorGUI.BeginChangeCheck();
            SSCCurve.CurvePreset curvePreset = (SSCCurve.CurvePreset)EditorGUILayout.EnumPopup(" Preset", SSCCurve.CurvePreset.None);
            if (EditorGUI.EndChangeCheck())
            {
                curveProperty.animationCurveValue = SSCCurve.GetCurveFromPreset(curvePreset);
            }

            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(curveContent, GUILayout.Width(defaultEditorLabelWidth - 24f));
            if (GUILayout.Button(btnResetContent, buttonCompact, GUILayout.Width(20f)))
            {
                curveProperty.animationCurveValue = SSCCurve.GetCurveFromPreset(defaultPreset);
                wasReset = true;
            }
        ...
        public static void DrawSSCGetHelpButtons(GUIStyle buttonCompact)
        {
            ...
            if (GUILayout.Button(SSCEditorHelper.btnPatreonContent, buttonCompact)) { Application.OpenURL(SSCEditorHelper.urlPatreon); }
        ...
        /// <summary>
        /// Draw an integer popup in the inspector.
        /// </summary>
        /// <param name="indent"></param>
        /// <param name="prop"></param>
        /// <param name="guiContent"></param>
        /// <param name="keys"></param>
        /// <param name="guiContents"></param>
        /// <param name="defaultEditorLabelWidth"></param>
        public static void DrawSSCIntPopup(float indent, SerializedProperty prop, GUIContent guiContent, int[] keys, GUIContent[] guiContents, float defaultEditorLabelWidth)
        {
            if (indent > 0)
            {
                EditorGUILayout.BeginHorizontal();
                SSCEditorHelper.DrawSSCLabelIndent(indent, guiContent, defaultEditorLabelWidth - indent - 4f);
                prop.intValue = EditorGUILayout.IntPopup(prop.intValue, guiContents, keys);
                EditorGUILayout.EndHorizontal();
            }
            else
            {
                prop.intValue = EditorGUILayout.IntPopup(guiContent, prop.intValue, guiContents, keys);
            }
        }
        ...
        /// Usage:
        ///   EditorGUILayout.BeginHorizontal();
        ///   DrawSSCLabelIndent(10f);
        ///   ...
        ///   EditorGUILayout.EndHorizontal();
        /// </summary>
        /// <param name="indent"></param>
        public static void DrawSSCLabelIndent(float indent)
        {
        ...
        /// Usage:
        ///   EditorGUILayout.BeginHorizontal();
        ///   DrawSSCLabelIndent(10f, myGUIContent);
        ///   ...
        ///   EditorGUILayout.EndHorizontal();
        /// </summary>
        /// <param name="indent"></param>
        /// <param name="guiContent"></param>
        public static void DrawSSCLabelIndent(float indent, GUIContent guiContent, float maxWidth = 0f)
        {
            DrawSSCLabelIndent(indent);
        ...
        /// <param name="guiLabelContent"></param>
        /// <param name="defaultEditorLabelWidth"></param>
        public static void DrawSSCMinMaxSlider(SerializedProperty minProp, SerializedProperty maxProp, GUIContent guiLabelContent, float minLimit, float maxLimit, float defaultEditorLabelWidth)
        {
            ...
            else if (minValue > maxLimit) { minValue = maxLimit; maxValue = maxLimit; }
            DrawSSCLabelIndent(2f);
            EditorGUILayout.MinMaxSlider(ref minValue, ref maxValue, minLimit, maxLimit);
            DrawSSCLabelIndent(2f);
            maxValue = EditorGUILayout.FloatField(maxValue, GUILayout.Width(40f));
            if (maxValue > maxLimit) { maxValue = maxLimit; }
            else if (maxValue < minLimit) { minValue = minLimit; maxValue = minLimit; }
            GUILayout.EndHorizontal();
        ...
        /// <summary>
        /// If the property is an element of an array (or list), this will return the 
        /// zero-based index position. Returns -1 if it is not an array or list element.
        /// </summary>
        /// <param name="prop"></param>
        /// <returns></returns>
        public static int GetSSCPropertyArrayIndex(SerializedProperty prop)
        {
            int index = -1;

            int startIndex = prop.propertyPath.LastIndexOf('[');
            int endIndex = prop.propertyPath.LastIndexOf(']');

            if (startIndex > 0 && endIndex > startIndex)
            {
                index = Convert.ToInt32(prop.propertyPath.Substring(startIndex + 1, endIndex - startIndex - 1));
            }
        ...
        /// <param name="defaultEditorLabelWidth"></param>
        public static void DrawSSCPropertyIndent(float indent, SerializedProperty serializedProperty, GUIContent guiContent, float defaultEditorLabelWidth)
        {
            EditorGUILayout.BeginHorizontal();
            DrawSSCLabelIndent(indent, guiContent, defaultEditorLabelWidth - indent - 3f);
            EditorGUILayout.PropertyField(serializedProperty, GUIContent.none);
            EditorGUILayout.EndHorizontal();
        }

        /// <summary>
        /// Draw an indented PropertyField with a label and reset button. The action will call the
        /// reset method in the calling class.
        /// DrawSSCPropertyWithReset(0f, handHoldOffsetProp, handHoldOffsetContent, buttonCompact, defaultEditorLabelWidth, ResetHandHoldOffset);
        /// </summary>
        /// <param name="indent"></param>
        /// <param name="serializedProperty"></param>
        /// <param name="guiContent"></param>
        /// <param name="buttonCompact"></param>
        /// <param name="defaultEditorLabelWidth"></param>
        /// <param name="resetAction">The name of the parameter-less function to call when reset</param>
        public static void DrawSSCPropertyWithReset(float indent, SerializedProperty serializedProperty, GUIContent guiContent, GUIStyle buttonCompact, float defaultEditorLabelWidth, System.Action resetAction)
        {
            EditorGUILayout.BeginHorizontal();
            float labelWidth = defaultEditorLabelWidth - 24f;
            if (indent > 0f)
            {
                DrawSSCLabelIndent(indent);
                labelWidth -= indent + 4f;
            }
            EditorGUILayout.LabelField(guiContent, GUILayout.Width(labelWidth));
            if (GUILayout.Button(btnResetContent, buttonCompact, GUILayout.MaxWidth(20f)))
            {
                resetAction();
            }
            EditorGUILayout.PropertyField(serializedProperty, GUIContent.none);
        ...
        public static bool DrawOffsetTopBottom(SerializedProperty offsetYProp, SerializedProperty heightProp, float labelWidth)
        {
          ...
          /// <summary>
        /// Draw a single-select layer dropdown in the inspector
        /// </summary>
        /// <param name="indent"></param>
        /// <param name="serializedProperty"></param>
        /// <param name="labelGUIContent"></param>
        public static void DrawSSCLayer(float indent, SerializedProperty serializedProperty, GUIContent labelGUIContent)
        {
            GUILayout.BeginHorizontal();
            if (indent > 0)
            {
            ...
            serializedProperty.intValue = EditorGUILayout.LayerField(labelGUIContent, serializedProperty.intValue);
        ...
        /// <summary>
        /// Returns a vector2 as text for debugging with 0,1,2 or 3 decimal places
        /// </summary>
        /// <param name="v2"></param>
        /// <param name="decimalPlaces"></param>
        /// <returns></returns>
        public static string GetVector2Text(Vector2 v2, int decimalPlaces)
        {
            float multiplier = decimalPlaces == 0 ? 10 : decimalPlaces == 1 ? 100f : decimalPlaces == 2 ? 1000f : 10000f;
            string sFormat = decimalPlaces == 0 ? "0" : decimalPlaces == 1 ? "0.0" : decimalPlaces == 2 ? "0.00" : "0.000";

            float x = Mathf.RoundToInt(v2.x * multiplier) / multiplier;
            float y = Mathf.RoundToInt(v2.y * multiplier) / multiplier;

            return x.ToString(sFormat) + ", " + y.ToString(sFormat);
        }
        ...
        /// <summary>
        /// Refresh the IntPopup data for the available gauge purposes.
        /// </summary>
        public static void RefreshGaugePurposes (SerializedProperty displayGaugePurposesProp, ref int[] gaugePurposeKeys, ref GUIContent[] gaugePurposeContents)
        {
            SSCGaugePurposes gaugePurposes = (SSCGaugePurposes)displayGaugePurposesProp.objectReferenceValue;

            gaugePurposeKeys = SSCLookup.GetKeys(gaugePurposes);

            string[] purposeValues = SSCLookup.GetValues(gaugePurposes);

            int numValues = purposeValues == null ? 0 : purposeValues.Length;

            if (numValues > 0)
            {
                gaugePurposeContents = new GUIContent[numValues];

                for (int vIdx = 0; vIdx < numValues; vIdx++)
                {
                    gaugePurposeContents[vIdx] = new GUIContent(purposeValues[vIdx]);
                }
            }
            else
            {
                gaugePurposeContents = new GUIContent[] { new GUIContent("Default") };
            }
        }
        ...
        public static readonly string urlGetSupport = "https://discussions.unity.com/t/724196";
        ...
        public static readonly string urlPatreon = "https://www.patreon.com/scsmmedia";

        //public static readonly string btnTxtAssetPage = "Asset Page";
        public static readonly string btnTxtGetSupport = "Get Support";
        public static readonly GUIContent tutorialsURLContent = new GUIContent("Videos", "Go to our website for a link to Video Tutorials about SSC concepts");
        ...
        public static readonly GUIContent btnPatreonContent = new GUIContent("Patreon", "Get the latest info on new features. Free Tier available.");
        ...
        /// <summary>
        /// Attempt to save a scriptableobject to the Assets folder. In UniqueRenameMode, the object
        /// will be created in the currently selected project folder, and will be highlighted in Rename Mode.
        /// If Unique Rename Mode is false, CreateAsset WILL OVERWRITE any existing asset of the same name.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        ...
        /// <param name="prefix"></param>
        /// <param name="isUniqueRenameMode"></param>
        /// <returns></returns>
        public static T SaveAsset<T>(T asset, string prefix, bool isUniqueRenameMode) where T : ScriptableObject
        {
            if (isUniqueRenameMode)
            {
                ProjectWindowUtil.CreateAsset(asset, prefix + typeof(T).Name + ".asset");
        ...
        /// <summary>
        /// Check to see if the scene is running for use with Cinematic methods
        /// </summary>
        /// <returns></returns>
        public static bool CheckRuntimeCinematic()
        {
            ...
            {
                SSCEditorHelper.PromptGotIt("Cinematic", "Run the scene to use cinematic");
                return false;
            }
        }
