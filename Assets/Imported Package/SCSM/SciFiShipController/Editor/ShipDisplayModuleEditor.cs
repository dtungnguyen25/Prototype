...
namespace SciFiShipController
{
    ...
    public class ShipDisplayModuleEditor : Editor
    {
        ...
        private int[] gaugePurposeKeys = null;
        private GUIContent[] gaugePurposeContents = { new GUIContent("Default") };
        ...
        private readonly static GUIContent isHideCursorOnInitContent = new GUIContent(" Hide Cursor on Init", "Hide the Cursor when HUD is first Initialised");
        ...
        private readonly static GUIContent sourceShipContent = new GUIContent(" Source Ship", "The ship in the scene that will supply the data for this HUD e.g., altitude, speed, heading, attitude etc.");
        ...
        private readonly static GUIContent lockDisplayReticleToCursorContent = new GUIContent(" Lock Reticle to Cursor", "Should the Display Reticle follow the cursor or mouse position on the screen? NOTE: This does not currently work with a Worldspace canvas");
        ...
        private readonly static GUIContent showAltitudeMetricContent = new GUIContent(" Show Altitude Metric", "Show the Altitude metric (m) on the HUD if the Altitude indicator is show. [Has no effect outside play mode]");
        ...
        private readonly static GUIContent showAirspeedMetricContent = new GUIContent(" Show Air Speed Metric", "Show the air speed metric (kmh) on the HUD if the air speed is show. [Has no effect outside play mode]");
        private readonly static GUIContent airspeedAsKnotsContent = new GUIContent(" Air Speed as Knots", "Display the Air Speed on the HUD in knots (kn)");
        private readonly static GUIContent airspeedOffsetContent = new GUIContent(" Air Speed Offset", "Add or subtract from the actual airspeed. This can be useful to give the impression the ship is travelling very quickly through space or during a warp when ship is stationary.");
        ...
        #region GUIContent - Cinematic
        private readonly static GUIContent displayCinSettingsContent = new GUIContent("Display Cinematic Settings");
        private readonly static GUIContent cinDampingContent = new GUIContent(" Damping", "Damp the movement of the cinematic effect to make it progress in a non-linear manner.");
        private readonly static GUIContent cinDelayContent = new GUIContent(" Delay", "Initial delay when starting the cinematic start or stop process.");
        private readonly static GUIContent cinInOnStartContent = new GUIContent(" Enable In On Start", "Smoothly enable the cinematic effect when display first starts");
        private readonly static GUIContent cinOutOnStartContent = new GUIContent(" Enable Out On Start", "Smoothly disable the cinematic effect when display first starts");
        private readonly static GUIContent cinInDurationContent = new GUIContent(" In Duration", "The length of time in seconds, it takes to fully enable the cinematic mask");
        private readonly static GUIContent cinOutDurationContent = new GUIContent(" Out Duration", "The length of time in seconds, it takes to fully disable the cinematic mask");
        private readonly static GUIContent cinBtnStartContent = new GUIContent("Start");
        private readonly static GUIContent cinBtnStartInstantlyContent = new GUIContent("Start Instantly");
        private readonly static GUIContent cinBtnStopContent = new GUIContent("Stop");
        private readonly static GUIContent cinBtnStopInstantlyContent = new GUIContent("Stop Instantly");
        private readonly static GUIContent cinBtnCancelContent = new GUIContent("Cancel");
        ...
        private readonly static GUIContent displayGaugePurposesContent = new GUIContent(" Gauge Purposes", "The optional ScriptableObject containing gauge purposes.");
        ...
        private readonly static GUIContent dgGaugePurposeContent = new GUIContent(" Gauge Purpose", "The purpose of this gauge. Typically it comes from SSCGaugePurposes.");
        ...
        private readonly static GUIContent debugIsCinematicOnContent = new GUIContent("Cinematic On?");
        ...
        private readonly static GUIContent debugTargetRadarLocationContent = new GUIContent("  Radar Location");
        ...
        private SerializedProperty isHideCursorOnInitProp;
        ...
        private SerializedProperty showAltitudeMetricProp;
        private SerializedProperty showAirspeedProp;
        private SerializedProperty showAirspeedMetricProp;
        private SerializedProperty airspeedAsKnotsProp;
        private SerializedProperty airspeedOffsetProp;
        ...
        // Cinematic
        private SerializedProperty showCinSettingsInEditorProp;
        private SerializedProperty cinematicInDurationProp;
        private SerializedProperty cinematicOutDurationProp;
        private SerializedProperty cinematicInOnStartProp;
        private SerializedProperty cinematicOutOnStartProp;
        private SerializedProperty cinematicDampingProp;
        private SerializedProperty cinematicDelayProp;
        ...
        private SerializedProperty displayGaugePurposesProp;
        ...
        private SerializedProperty dgauGaugePurposeKeyProp;
        ...
        public void OnEnable()
        {
            ...
            showCinSettingsInEditorProp = serializedObject.FindProperty("showCinematicSettingsInEditor");
            ...
            isHideCursorOnInitProp = serializedObject.FindProperty("isHideCursorOnInit");
            ...
            showAltitudeMetricProp = serializedObject.FindProperty("showAltitudeMetric");
            showAirspeedProp = serializedObject.FindProperty("showAirspeed");
            showAirspeedMetricProp = serializedObject.FindProperty("showAirspeedMetric");
            airspeedAsKnotsProp = serializedObject.FindProperty("airspeedAsKnots");
            airspeedOffsetProp = serializedObject.FindProperty("airspeedOffset");
            ...
            cinematicDampingProp = serializedObject.FindProperty("cinematicDamping");
            cinematicDelayProp = serializedObject.FindProperty("cinematicDelay");
            cinematicInDurationProp = serializedObject.FindProperty("cinematicInDuration");
            cinematicOutDurationProp = serializedObject.FindProperty("cinematicOutDuration");
            cinematicInOnStartProp = serializedObject.FindProperty("cinematicInOnStart");
            cinematicOutOnStartProp = serializedObject.FindProperty("cinematicOutOnStart");
            ...
            displayGaugePurposesProp = serializedObject.FindProperty("gaugePurposes");
            ...
            SSCEditorHelper.RefreshGaugePurposes(displayGaugePurposesProp, ref gaugePurposeKeys, ref gaugePurposeContents);
        }
        ...
        private void ExpandList<T>(List<T> componentList, bool isExpanded)
        {
            ...
            if (numComponents > 0)
            {
                ...
                else if (compType == typeof(DisplayGauge))
                {
                    for (int cpi = 0; cpi < numComponents; cpi++)
                    {
                        (componentList[cpi] as DisplayGauge).showInEditor = isExpanded;
                    }
                }
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (showGeneralSettingsInEditorProp.boolValue)
            {
                ...
                EditorGUILayout.PropertyField(isHideCursorOnInitProp, isHideCursorOnInitContent);
                ...
                if (autoHideCursorProp.boolValue)
                {
                    ...
                    if (isHideCursorOnInitProp.boolValue && !shipDisplayModule.IsInitialised)
                    {
                        EditorGUILayout.HelpBox("It may not make sense to have Hide Cursor on Init enabled AND AutoHideCursor when the HUD is first initialised", MessageType.Warning);
                    }
                }
            ...
            if (showAltSpeedSettingsInEditorProp.boolValue)
            {
                ...
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(showAltitudeMetricProp, showAltitudeMetricContent);
                // Always visible in outside of play mode
                if (EditorGUI.EndChangeCheck() && !shipDisplayModule.IsEditorMode && Application.isPlaying)
                {
                    serializedObject.ApplyModifiedProperties();
                    if (showAltitudeMetricProp.boolValue) { shipDisplayModule.ShowAltitudeMetric(); }
                    else { shipDisplayModule.HideAltitudeMetric(); }
                    serializedObject.Update();
                }
                ...
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(showAirspeedMetricProp, showAirspeedMetricContent);
                // Always visible in outside of play mode
                if (EditorGUI.EndChangeCheck() && !shipDisplayModule.IsEditorMode && Application.isPlaying)
                {
                    serializedObject.ApplyModifiedProperties();
                    if (showAirspeedMetricProp.boolValue) { shipDisplayModule.ShowAirSpeedMetric(); }
                    else { shipDisplayModule.HideAirSpeedMetric(); }
                    serializedObject.Update();
                }

                EditorGUILayout.PropertyField(airspeedAsKnotsProp, airspeedAsKnotsContent);
                EditorGUILayout.PropertyField(airspeedOffsetProp, airspeedOffsetContent);
            ...
            #region Cinematic Settings

            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            SSCEditorHelper.DrawSSCFoldout(showCinSettingsInEditorProp, displayCinSettingsContent, foldoutStyleNoLabel, defaultEditorFieldWidth);

            if (showCinSettingsInEditorProp.boolValue)
            {
                EditorGUILayout.PropertyField(cinematicDampingProp, cinDampingContent);
                EditorGUILayout.PropertyField(cinematicDelayProp, cinDelayContent);

                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(cinematicInOnStartProp, cinInOnStartContent);
                if (EditorGUI.EndChangeCheck())
                {
                    if (cinematicOutOnStartProp.boolValue && cinematicInOnStartProp.boolValue)
                    {
                        cinematicOutOnStartProp.boolValue = false;
                    }
                }
                EditorGUILayout.PropertyField(cinematicInDurationProp, cinInDurationContent);
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(cinematicOutOnStartProp, cinOutOnStartContent);
                if (EditorGUI.EndChangeCheck())
                {
                    if (cinematicOutOnStartProp.boolValue && cinematicInOnStartProp.boolValue)
                    {
                        cinematicInOnStartProp.boolValue = false;
                    }
                }
                EditorGUILayout.PropertyField(cinematicOutDurationProp, cinOutDurationContent);

                EditorGUILayout.BeginHorizontal();

                if (GUILayout.Button(cinBtnStartContent, buttonCompact))
                {
                    if (SSCEditorHelper.CheckRuntimeCinematic())
                    {
                        shipDisplayModule.StartDisplayCinematic(false);
                    }
                    else
                    {
                ...
                if (GUILayout.Button(cinBtnStartInstantlyContent, buttonCompact))
                {
                    if (SSCEditorHelper.CheckRuntimeCinematic())
                    {
                        shipDisplayModule.StartDisplayCinematic(true);
                    }
                    else
                    {
                        EditorGUIUtility.ExitGUI();
                ...
                if (GUILayout.Button(cinBtnStopContent, buttonCompact))
                {
                    if (SSCEditorHelper.CheckRuntimeCinematic())
                    {
                        shipDisplayModule.StopDisplayCinematic(false);
                    }
                    else
                    {
                        EditorGUIUtility.ExitGUI();
                ...
                if (GUILayout.Button(cinBtnStopInstantlyContent, buttonCompact))
                {
                    if (SSCEditorHelper.CheckRuntimeCinematic())
                    {
                        shipDisplayModule.StopDisplayCinematic(true);
                    }
                    else
                    {
                        EditorGUIUtility.ExitGUI();
                ...
                if (GUILayout.Button(cinBtnCancelContent, buttonCompact))
                {
                    if (SSCEditorHelper.CheckRuntimeCinematic())
                    {
                        shipDisplayModule.CancelDisplayCinematic(false);
                    }
                    else
                    {
                        EditorGUIUtility.ExitGUI();
            ...
            if (showGaugeSettingsInEditorProp.boolValue)
            {
                ...
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(displayGaugePurposesProp, displayGaugePurposesContent);
                if (EditorGUI.EndChangeCheck())
                {
                    SSCEditorHelper.RefreshGaugePurposes(displayGaugePurposesProp, ref gaugePurposeKeys, ref gaugePurposeContents);
                }
                ...
                for (int dgIdx = 0; dgIdx < numDisplayGauges; dgIdx++)
                {
                    ...
                    if (dgauShowInEditorProp.boolValue)
                    {
                        ...
                        dgauGaugePurposeKeyProp = dgauProp.FindPropertyRelative("gaugePurposeKey");
                        ...
                        dgauGaugePurposeKeyProp.intValue = EditorGUILayout.IntPopup(dgGaugePurposeContent, dgauGaugePurposeKeyProp.intValue, gaugePurposeContents, gaugePurposeKeys);
            ...
            if (isDebuggingEnabled && shipDisplayModule != null)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(debugIsCinematicOnContent, labelFieldRichText, GUILayout.Width(defaultEditorLabelWidth - 3f));
                EditorGUILayout.LabelField(shipDisplayModule.IsDisplayCinematicOn ? "Yes" : "No", GUILayout.MaxWidth(rightLabelWidth));
                EditorGUILayout.EndHorizontal();
                ...
                if (isDebuggingShowTargets)
                {
                    ...
                    if (Application.isPlaying && sscRadar == null)
                    {
                        sscRadar = SSCRadar.GetOrCreateRadar(shipDisplayModule.gameObject.scene.handle);
                    }
                    ...
                    for (int tgIdx = 0; tgIdx < numTargets; tgIdx++)
                    {
                        ...
                        for (int slotIdx = 0; slotIdx < maxNumTargets; slotIdx++)
                        {
                            ...
                            if (displayTargetSlot != null)
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                EditorGUILayout.LabelField(debugTargetRadarLocationContent, labelFieldRichText, GUILayout.Width(defaultEditorLabelWidth - 3f));

                                if (radarItem == null || radarItem.radarItemType != SSCRadarItem.RadarItemType.Location)
                                {
                                    EditorGUILayout.LabelField("--", GUILayout.MaxWidth(rightLabelWidth));
                                }
                                else
                                {
                                    LocationData location = sscRadar.GetRadarItemLocation(radarItem);
                                    if (location != null)
                                    {
                                        EditorGUILayout.LabelField(string.IsNullOrEmpty(location.name) ? "(No Name)" : location.name, GUILayout.MaxWidth(rightLabelWidth));
                                    }
                                    else
                                    {
                                        EditorGUILayout.LabelField("--", GUILayout.MaxWidth(rightLabelWidth));
