...
namespace SciFiShipController
{
    ...
    public class SurfaceTurretModuleEditor : Editor
    {
        ...
        private readonly static GUIContent modelIdContent = new GUIContent("Model Id", "The unique model or type of surface turret. Values 1 to 100 are reserved for demo turrets. 0 = Not Set.");
        ...
        private readonly static GUIContent gravityDirectionContent = new GUIContent("Direction", "The direction in which gravity acts on the projectile in world space.");
        private readonly static GUIContent mediumDensityContent = new GUIContent("Medium Density", "If SSC Expansion Pack 1 is installed, the density (in kilograms per cubic metre) of the fluid the missile will travel through (usually air).");
        ...
        private readonly static GUIContent weaponFiringButtonAutoContent = new GUIContent("Auto Fire", "When a target is selected, the weapon will automatically attempt to fire at the target.");
        private readonly static GUIContent weaponFiringButtonManContent = new GUIContent("Manual Fire", "The weapon can be manually fired with FireIfReady() API");
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("modelId"), modelIdContent);
            ...
            if (GUILayout.Button(SSCEditorHelper.gotoEffectFolderBtnContent, buttonCompact, GUILayout.Width(20f))) { SSCEditorHelper.HighlightFolderInProjectWindow(SSCSetup.effectsFolder, false, true); }
            ...
            if (isDestroyOnNoHealthProp.boolValue)
            {
                ...
                if (GUILayout.Button(SSCEditorHelper.gotoDestructsFolderBtnContent, buttonCompact, GUILayout.Width(20f))) { SSCEditorHelper.HighlightFolderInProjectWindow(SSCSetup.destructFolder, false, true); }
            ...
            else { if (GUILayout.Button(SSCEditorHelper.gizmoBtnContent, toggleCompactButtonStyleNormal, GUILayout.MaxWidth(22f))) { weaponShowGizmosInSceneViewProp.boolValue = true; } }
            ...
            if (weaponShowInEditorProp.boolValue)
            {
                ...
                if (weaponTypeProp.intValue == Weapon.TurretBeamInt)
                {
                    ...
                    if (GUILayout.Button(SSCEditorHelper.gotoBeamFolderBtnContent, buttonCompact, GUILayout.Width(20f))) { SSCEditorHelper.HighlightFolderInProjectWindow(SSCSetup.beamsFolder, false, true); }
                ...
                else
                {
                    ...
                    if (GUILayout.Button(SSCEditorHelper.gotoProjectileFolderBtnContent, buttonCompact, GUILayout.Width(20f))) { SSCEditorHelper.HighlightFolderInProjectWindow(SSCSetup.projectilesFolder, false, true); }
                ...
                bool isAutoFire = weaponFiringButtonProp.intValue == Weapon.FiringButtonAutoInt;
                bool isManualFire = weaponFiringButtonProp.intValue == Weapon.FiringButtonManualInt;

                EditorGUI.BeginChangeCheck();
                isAutoFire = EditorGUILayout.Toggle(weaponFiringButtonAutoContent, isAutoFire);
                if (EditorGUI.EndChangeCheck())
                {
                    weaponFiringButtonProp.intValue = isAutoFire ? Weapon.FiringButtonAutoInt : Weapon.FiringButtonNoneInt;

                    if (isAutoFire) { surfaceTurretModule.SetAutoFire(); }
                }

                EditorGUI.BeginChangeCheck();
                isManualFire = EditorGUILayout.Toggle(weaponFiringButtonManContent, isManualFire);
                if (EditorGUI.EndChangeCheck())
                {
                    weaponFiringButtonProp.intValue = isManualFire ? Weapon.FiringButtonManualInt : Weapon.FiringButtonNoneInt;

                    if (isManualFire) { surfaceTurretModule.SetManualFire(); }
                }
                ...
                #if SCSM_SSCXP1
                if (surfaceTurretModule.weapon.IsMissile)
                {
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("mediumDensity"), mediumDensityContent);
            ...
            if (isDebuggingEnabled && surfaceTurretModule != null)
            {
                ...
                EditorGUILayout.LabelField(surfaceTurretModule.Health.ToString("F1"), GUILayout.MaxWidth(rightLabelWidth));
                EditorGUILayout.EndHorizontal();

                // Only display radar item data at runtime
                if (surfaceTurretModule.IsInitialised && surfaceTurretModule.RadarId >= 0)
                {
                    if (sscRadar == null) { sscRadar = SSCRadar.GetOrCreateRadar(surfaceTurretModule.gameObject.scene.handle); }
        ...
        /// <summary>
        /// Force the inspector to refresh each frame when debugging is enabled
        /// </summary>
        /// <returns></returns>
        public override bool RequiresConstantRepaint()
        {
            return isDebuggingEnabled && surfaceTurretModule != null;
        }
