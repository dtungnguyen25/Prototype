...
namespace SciFiShipController
{
    ...
    public static class SSCSetup
    {
        ...
        static SSCSetup()
        {
            // See SSCDemoSetup for adding the Small Ships layers.
        ...
        private static void DefineSymbols()
        {
            ...
            // Shared with Sci-Fi Ship Controller, Sticky3D Controller, Landscape Builder, and LB Seamless.
            ...
            // If the package exists, add the define if it is missing
            // Only support v0.2.4 and v1.2.3+
            // Version 0.5.1-preview.32 needs an upgrade from IJobForEach to IJobEntity. See ProjectileSystem.cs

            if (!string.IsNullOrEmpty(physicsPackageVersion) && (SSCUtils.CompareVersionNumbers(physicsPackageVersion, "1.2.3") >= 0 || SSCUtils.CompareVersionNumbers(physicsPackageVersion, "0.2.5") < 0))
            {
        ...
        private static void UpgradeProject()
        {
            ...
            // v1.4.8 as Celestials is now referenced in ShipCameraModule.cs. If demo folder isn't imported this will fail.
            MoveAsset("SSCSetup.UpgradeProject", demoScriptsFolder, sscFolder + "/Scripts/Misc", "Celestials.cs");
            MoveAsset("SSCSetup.UpgradeProject", demoScriptsFolder, sscFolder + "/Scripts/Misc", "SSCCelestial.cs");

            // SSC v1.5.4. X Pack 4 requires the door animator and control components.
            string techDemoFolder = sscFolder + "/Demos/TechDemo";

            // Skip if the door animator components already part of core functionality
            if (!System.IO.File.Exists(sscFolder + "/Scripts/Utilities/SSCDoorAnimator.cs"))
            {
                // Is techdemo2 installed in the project?
                if (System.IO.File.Exists(techDemoFolder + "/Scripts/SSCDoorAnimator.cs"))
                {
                    MoveAsset("SSCSetup.UpgradeProject", techDemoFolder + "/Scripts", sscFolder + "/Scripts/Utilities", "SSCDoorAnimator.cs");
                    MoveAsset("SSCSetup.UpgradeProject", techDemoFolder + "/Scripts", sscFolder + "/Scripts/Utilities", "SSCDoorAnimEvt1.cs");
                    MoveAsset("SSCSetup.UpgradeProject", techDemoFolder + "/Scripts", sscFolder + "/Scripts/Utilities", "SSCDoorControl.cs");
                    MoveAsset("SSCSetup.UpgradeProject", techDemoFolder + "/Scripts", sscFolder + "/Scripts/Utilities", "SSCDoorProximity.cs");
                    MoveAsset("SSCSetup.UpgradeProject", techDemoFolder + "/Scripts", sscFolder + "/Scripts/Utilities", "SSCDoorProximity.cs");
                    MoveAsset("SSCSetup.UpgradeProject", techDemoFolder + "/Editor", sscFolder + "/Editor", "SSCDoorAnimatorEditor.cs");
                }
                else
                {
                    Debug.LogWarning("[ERROR] In order to update this version of Sci-Fi Ship Controller project, the demo files must be imported. You can delete the demos folder after upgrading if you wish.");
                }
            }
        }
        ...
        /// <summary>
        /// Add defines to the Project PlayerSettings
        /// </summary>
        /// <param name="buildTargetGroup"></param>
        /// <param name="defines"></param>
        public static void SetDefineSymbols(BuildTargetGroup buildTargetGroup, string defines)
        {
            #if UNITY_2023_1_OR_NEWER
        ...
        /// <summary>
        /// Create new Unity Layers. Optionally overwrite existing layer if one already exists.
        /// </summary>
