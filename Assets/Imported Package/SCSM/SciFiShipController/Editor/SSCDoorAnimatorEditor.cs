...
namespace SciFiShipController
{
    ...
    public class SSCDoorAnimatorEditor : Editor
    {
        ...
        private readonly static GUIContent audioSourcesContent = new GUIContent("Audio Sources", "An optional array of audio sources. By default all doors share the same audio source attached to this gameobject. However, if you provide one audio source slot for each door, audio will play on the matching audio source. If the slot is empty, no audio will play for that door.");
        private readonly static GUIContent isVerifyParmsContent = new GUIContent("Verify Parameters", "Verify the animator parameters on awake. False for backward compatibility. May incur GC so only use testing.");
        ...
        private readonly static string isAudioSourceMismatchMsg = "The number of Audio Sources does not match the number of doors or sets of doors being controlled. This is indicated by the number of Open Param Names.";
        #endregion
        ...
        private SerializedProperty isVerifyParmsProp;
        ...
        public void OnEnable()
        {
            ...
            isVerifyParmsProp = serializedObject.FindProperty("isVerifyParms");
        ...
        public override void OnInspectorGUI()
        {
            ...
            SSCEditorHelper.DrawArray(audioSourcesProp, audioSourcesContent, defaultEditorLabelWidth, "Audio Source");

            if (audioSourcesProp.arraySize > 0 && audioSourcesProp.arraySize != openParamNamesProp.arraySize)
            {
                EditorGUILayout.HelpBox(isAudioSourceMismatchMsg, MessageType.Warning);
            }
            ...
            EditorGUILayout.PropertyField(isVerifyParmsProp, isVerifyParmsContent);
