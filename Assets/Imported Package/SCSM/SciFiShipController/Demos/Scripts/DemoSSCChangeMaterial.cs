...
namespace SciFiShipController
{
    ...
    public class DemoSSCChangeMaterial : MonoBehaviour
    {
        ...
        public void GetGroup1Material (int materialElementIndex)
        {
            if (!isInitialised)
            {
                Debug.LogWarning("DemoSSCChangeMaterial.GetGroup1Material() on " + name + " in " + gameObject.scene.name + " is not initialised!");
            }
            else if (materialElementIndex >= 0 && materialElementIndex < numMaterialsArray[0] && isGroupValid[0])
            {
        ...
        public void GetGroup2Material (int materialElementIndex)
        {
            if (!isInitialised)
            {
                Debug.LogWarning("DemoSSCChangeMaterial.GetGroup2Material() on " + name + " in " + gameObject.scene.name + " is not initialised!");
            }
            else if (materialElementIndex >= 0 && materialElementIndex < numMaterialsArray[1] && isGroupValid[1])
            {
        ...
        public void GetGroup3Material(int materialElementIndex)
        {
            if (!isInitialised)
            {
                Debug.LogWarning("DemoSSCChangeMaterial.GetGroup3Material() on " + name + " in " + gameObject.scene.name + " is not initialised!");
            }
            else if (materialElementIndex >= 0 && materialElementIndex < numMaterialsArray[2] && isGroupValid[2])
            {
        ...
        /// <summary>
        /// Initialise the component. Call this if in your game code if initialiseOnStart is not true.
        /// </summary>
        public void Initialise()
        {
            ...
            {
                #if UNITY_EDITOR
                Debug.LogWarning("ERROR DemoSSCChangeMaterial - could not find mesh renderer for " + name + " in " + gameObject.scene.name);
                #endif
            }
            else
            {
                ...
                {
                    #if UNITY_EDITOR
                    Debug.LogWarning("ERROR DemoSSCChangeMaterial - could not find any materials on the mesh renderer (" + objectMeshRenderer.name + ") for " + name + " in " + gameObject.scene.name);
                    #endif
                }
