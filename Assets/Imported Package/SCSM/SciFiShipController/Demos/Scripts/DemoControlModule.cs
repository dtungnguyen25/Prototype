    ...
    public class DemoControlModule : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            for (int sqIdx = 0; sqIdx < numSquadrons; sqIdx++)
            {
                ...
                {
                    ShipControlModule shipCM = squadronShipList[sqIdx][shIdx];
                    if (shipCM != null)
                    {
                        ...
                        if (shipCM.IsInitialised)
                        {
                            shipCM.ShipRigidbody.interpolation = RigidbodyInterpolation.Interpolate;
                            if (useRadar) { shipCM.EnableRadar(); }
                        }
                        ...
                        SampleAttack sampleAttack = shipCM.GetComponent<SampleAttack>();
                        if (sampleAttack == null) { sampleAttack = shipCM.gameObject.AddComponent<SampleAttack>(); }

                        ShipAIInputModule shipAI;
                        if (shipCM.GetShipAIInputModule(out shipAI, true) && sampleAttack != null)
                        {
                            // TODO - Ship Radius should be calculated
                            // Comment this out if set in a prefab
                            // if battleship make 20 (actual est 17.8)
                            // Making the radius a little bigger for each ship may improve obstacle avoidance.
                            shipAI.shipRadius = shipCM.ModelId == 12 ? 20f : 7f;

                            sampleAttack.attackStateID = attackStateID;

                            if (!sampleAttack.IsInitialised)
                            {
                                shipAI.movementAlgorithm = ShipAIInputModule.AIMovementAlgorithm.PlanarFlightBanking;
