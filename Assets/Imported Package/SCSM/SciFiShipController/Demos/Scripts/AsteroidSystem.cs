...
namespace SciFiShipController
{
    ...
    #endif
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Get all the entities with a LocalTransform (ECS 0.51: Translation and Rotation) and Asteroid components
            #if UNITY_2022_2_OR_NEWER
            // ECS 1.0
        ...
        public static void CreateAsteroid(Vector3 position, Quaternion rotation, Vector3 scale, float rotationRate, Vector3 rotationDirection, Entity prefabEntity)
        {
            ...
            if (asteroidWorld.IsCreated)
            {
                ...
                // Add pre-rendering scale component for ECS 1.2.3
                if (!entityManager.HasComponent(entity, typeof(PostTransformMatrix)))
                {
                    entityManager.AddComponent(entity, typeof(PostTransformMatrix));
                }
                ...
                entityManager.SetComponentData(entity, new PostTransformMatrix() { Value = float4x4.Scale((float3)scale) });
