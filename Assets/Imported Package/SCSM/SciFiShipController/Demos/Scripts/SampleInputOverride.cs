...
namespace SciFiShipController
{
    ...
    public class SampleInputOverride : MonoBehaviour
    {
        ...
        [Tooltip("If enabled, the player can still control speed up to the targetSpeed")]
        public bool speedLimiterMode = false;
        #endregion
        ...
        void Update()
        {
            if (isInitialised && shipControlModule.ShipMovementIsEnabled())
            {
                ...
                // Should the player be able to control the speed up to the limit?
                if (speedLimiterMode)
                {
                    // Get the original input directly from the PlayerInputModule
                    ShipInput shipInputRaw = playerInputModule.GetShipInput;

                    if (targetSpeed > currentSpeed * 0.95f)
                    {
                        shipInput.longitudinal = Mathf.Clamp(deltaSpeed / (targetSpeed * 0.2f) * shipInputRaw.longitudinal, -1f, 1f) * accelerationRate;
                    }
                    else
                    {
                        shipInput.longitudinal = shipInputRaw.longitudinal;
                    }
                }
                else
                {
